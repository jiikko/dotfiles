...
func TestDiffUnknownLanguagePassesThrough(t *testing.T) {
	in := []string{
	...
	out := Diff(in)
	want := sgr.Green + "+" + sgr.Reset + "opaque payload line"
	if out[3] != want {
...
func BenchmarkDiff(b *testing.B) {
	...
	for range 5000 { // glogx の maxDiffLines (diff の板の上限)
...
func TestDiffHunkContentNotHeader(t *testing.T) {
	in := []string{
		"diff --git a/a.go b/a.go",
		"+++ b/a.go",
		"@@ -1,2 +1,2 @@",
	...
	out := Diff(in)
	if !strings.HasPrefix(out[3], sgr.Red+"-") {
		t.Errorf("hunk 内の --- 始まり削除行がヘッダー扱い: %q", out[3])
	}
	if !strings.HasPrefix(out[4], sgr.Green+"+") {
...
func TestDiffMultiFileResetsState(t *testing.T) {
	in := []string{
		"diff --git a/a.go b/a.go",
		"+++ b/a.go",
		"@@ -1 +1 @@",
	...
	if !strings.HasPrefix(out[5], sgr.Bold) {
