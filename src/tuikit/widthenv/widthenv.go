// Package widthenv は「tuikit の幅モデルが支持しない env」の検出とその文言を 1 箇所に集める。
//
// x/ansi は method.go の init で RUNEWIDTH_EASTASIAN を読み、East Asian Ambiguous
// (罫線 ─│・ブロック要素 ░▒▁・矢印・… 等) を幅 2 として数える。tuikit の layout (Panel の枠・
// 影、Scrollbar、引き出しの区切り線) はこれらのグリフを「グリフ数 = 表示幅」の前提で
// strings.Repeat で埋めて組むので、この env が真だと枠が要求幅の 2 倍近くまで膨らむ
// (実測 2026-08-15, glogx: 80 桁指定のパネルが 157〜158 桁。issue 054)。
//
// 支持しない判断をしたので、せめて**黙って壊れない**ようにする: 使う側は起動直後に Message で
// 警告を出し、テストは幅系がまとめて落ちる代わりに TestMain で 1 本の説明付き失敗にする。
//
// 独立パッケージにしてあるのは、幅に依存するパッケージのテストがどれも TestMain から呼ぶため。
package widthenv
...
const Message = Name + " が設定されています。この env には対応していません " +
...
//
// 幅に依存するパッケージの TestMain は**全て**これを呼ぶこと (tuikit の termwidth / layout /
// widthenv と、使う側の幅に依存するパッケージ)。1 つでも呼び忘れると、そのパッケージだけが「幅 1 を前提にした assert が
