...
func (s *Sim) Save() ([]byte, error) {
	st := simState{Now: s.now, Cards: s.cards, Limit: s.limit, UsageOff: s.usageOff, Scripts: map[string]scriptState{}, Resource: s.resource,
...
func (s *Sim) Restore(data []byte) error {
	...
	*s = Sim{now: st.Now, cards: st.Cards, limit: st.Limit, usageOff: st.UsageOff, scripts: scripts, resource: st.Resource,
