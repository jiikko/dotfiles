...
func (s *Sim) seed() {
	...
	s.cards = []card.Card{
		...
		{ID: "C-003", Title: "tmux-toast の通知が重なる", Request: "toast が 2 つ同時に出ると重なって読めない",
			Repo: "dotfiles", Owner: "PM-A", State: card.Planned, Since: s.ago(12 * time.Minute), Points: 3,
		...
		{ID: "C-004", Title: "av1ify のログ出力を整理", Request: "av1ify のログがうるさい", ParentID: "C-010",
			Repo: "dotfiles", Owner: "PM-A", State: card.Planned, Since: s.ago(8 * time.Minute), Points: 1,
		...
		{ID: "C-005", Title: "pro-con のカンバン幅の調整", Request: "カンバンの列が狭いときに崩れないようにして",
			...
			LastProgress: s.ago(1 * time.Minute), Points: 5, WorkFrom: s.ago(15 * time.Minute),
		...
		{ID: "C-006", Title: "実機 E2E で再生テスト", Request: "実機で seek のテストを回して",
			...
			LastProgress: s.ago(2 * time.Minute), Points: 2, Worked: 4 * time.Minute,
		...
		{ID: "C-007", Title: "zsh の起動が遅い", Request: "新しい zsh が開くのに 1 秒くらいかかる",
			Repo: "dotfiles", Owner: "PM-A", Session: "3feb5f01", State: card.Waiting, Since: s.ago(9 * time.Minute), Points: 2, Worked: 21 * time.Minute,
		...
		{ID: "C-008", Title: "glogx の起動時間短縮", Request: "glogx 開くのちょっと遅い",
			Repo: "dotfiles", Owner: "PM-A", Session: "3feb5f02", State: card.Review, Since: s.ago(25 * time.Minute), Points: 8, Worked: 72 * time.Minute,
		...
		{ID: "C-010", Title: "av1ify の失敗時に再開できない", Request: "av1ify が途中で落ちたらやり直しになる",
			Repo: "dotfiles", Owner: "PM-A", State: card.Done, Since: s.ago(90 * time.Minute), Points: 3, Worked: 48 * time.Minute,
...
func (s *Sim) Snapshot() backend.Snapshot {
	for i := range s.cards { // 作業中の時間の切り替わりを閉じる (本物では記録の書き手が書くときに閉じる。store.settleWork)
		s.cards[i].SettleWork(s.now)
	}
