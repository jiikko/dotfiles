...
func (s *Sim) seed() {
	...
	s.cards = []card.Card{
		...
		{ID: "C-005", Title: "pro-con のカンバン幅の調整", Request: "カンバンの列が狭いときに崩れないようにして",
			...
			LastProgress: s.ago(1 * time.Minute), Points: 5,
		...
		{ID: "C-006", Title: "実機 E2E で再生テスト", Request: "実機で seek のテストを回して",
			...
			LastProgress: s.ago(2 * time.Minute), Points: 2,
		...
		{ID: "C-007", Title: "zsh の起動が遅い", Request: "新しい zsh が開くのに 1 秒くらいかかる",
			Repo: "dotfiles", Owner: "PM-A", Session: "3feb5f01", State: card.Waiting, Since: s.ago(9 * time.Minute), Points: 2,
		...
		{ID: "C-010", Title: "av1ify の失敗時に再開できない", Request: "av1ify が途中で落ちたらやり直しになる",
			Repo: "dotfiles", Owner: "PM-A", State: card.Done, Since: s.ago(90 * time.Minute), Points: 3,
