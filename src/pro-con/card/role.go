...
// Handling は役のどれかが今カード c を手に取っていれば、その仕事の名前 (分解中 / 回答中 / レビュー中)。取っていなければ空。
// 手に取っている = 知らせ済みで、役が turn の途中か起動・再開の最中 (idle の役は知らせたカードが残っていても手を止めている)。
// 人の番 (r で決める) のカードは役の手元に残っていても手に取っていない (人に回した質問・起こさない役の仕事)。
func (c Card) Handling(r Roles, ss []RoleState) string {
	if t := c.Turn(r); t == TurnHuman || t == TurnNone {
		return ""
	}
	for _, s := range ss {
		if (s.Phase != RoleBusy && s.Phase != RoleLaunch) || !slices.Contains(s.Cards, c.ID) {
			continue
		}
		switch c.State {
		case Requested:
		...
		case Review:
			return "レビュー中"
		case Planned, Running, Done: // 役の列を離れた (次の Tick で知らせ済みから外れる)
		}
	}
	return ""
}
