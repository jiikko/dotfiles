...
// 読む費用が作ったカードの総数に比例して増える。終えたカードは書庫 (JSONL) へ移し、記録には動いているカードと
// 完了して間もないカードだけを置く。書庫は `card show` / `card list --all` が読む (438 の「記録からは消さない」を書庫で守る)。
// 完了から 1 週間たったカードは書庫からも消す (issue 497 のユーザーの決定。dispatcher が 1 時間に 1 回、片付けの印を残してから
// 書き直す = purge.go。それまでは足していくだけ)。
//
// 失敗モード:
...
func appendArchive(dir string, cs []card.Card) error {
	return appendLines(filepath.Join(dir, ArchiveFile), cs)
}
