...
4. **PG の質問に答える**。答える前に `pro-con card show <カード>` で依頼の原文・履歴・PG の出力を読む
   ...
   **人間に聞くべき質問は答えずに人間に回す**: 依頼の原文と issue から決まらない好み・方針 (見た目・スコープの広げ方・どちらの案を採るか)、
   ...
   カードは質問待ちの列のまま残り、人間が画面か `pro-con card answer <カード> "<回答>"` で答える (人間の番の目印は 452 で付く。今は履歴だけ)。
   人に回した質問がまた知らされたら (履歴に「人に回した」がある)、何もしない
5. **PG が終えたカード (レビューの列) は、取り込みの係がレビューする。PM は触らない**。分担は 2026-09-25 のユーザーの決定 (437):
   依頼の分解 (役目 1〜3) と PG の質問への回答 (役目 4) は PM、レビュー・差し戻し・完了・master への取り込みは取り込みの係 (人間か、人間の代わりの Claude の session)。
   取り込みの係は diff と実行結果を読み、よければ完了にする。PG の「終わった」は証拠ではない (415 論点 4)
   `pro-con card close <カード> --issue <repo>#<番号>`
   人間の追加オーダーが PG へ届いていないカードは閉じられない (dispatcher が PG へ戻して届ける。届いてレビューの列に戻ってから閉じる。issue 438)。
