...
func TestKillStaleByRunMarker(t *testing.T) {
	...
	imposter := start(exec.Command("/bin/sh", "-c", "sleep 63; true", RunMark+id("mine")))
	// `/bin/bash` でも `-c` ではない形 (照合の f[3] を外すと撃たれる)
	notC := start(exec.Command("/bin/bash", "-x", "-c", "sleep 64; true", RunMark+id("mine")))
	// グループの先頭ではない `/bin/bash -c … 印` (先頭は別の bash。照合の pid = pgid を外すとグループごと撃たれる)
	notLeader := start(exec.Command("/bin/bash", "-c", runCommandEnv+"='sleep 65' /bin/bash -c 'eval \"$"+runCommandEnv+"\"' "+RunMark+id("mine")+"; true"))
	waitMarker(t, id("other")) // 否定の確認の前提: 撃たれうる形で ps に出ている
	waitMarker(t, id("mine"))
	waitPS(t, "/bin/sh -c sleep 63; true "+RunMark+id("mine")) // 偽物も ps に出ている
	waitPS(t, "/bin/bash -x -c sleep 64; true "+RunMark+id("mine"))
	waitPS(t, "/bin/bash -c eval \"$"+runCommandEnv+"\" "+RunMark+id("mine")) // 先頭ではない方 (mine 本体と同じ形)
	killStale(id("mine"))
	select {
	case <-mine:
...
func waitMarker(t *testing.T, runID string) {
	t.Helper()
	waitPS(t, "/bin/bash -c eval \"$"+runCommandEnv+"\" "+RunMark+runID)
}
