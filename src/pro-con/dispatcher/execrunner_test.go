...
func TestKillStaleByRunMarker(t *testing.T) {
	...
	notLeader := start(exec.Command("/bin/bash", "-c", runCommandEnv+"='sleep 65' /bin/bash -c '"+runScript+"' "+RunMark+id("mine")+"; true"))
	...
	waitPS(t, "/bin/bash -c "+runScript+" "+RunMark+id("mine")) // 先頭ではない方 (mine 本体と同じ形)
	killStale(id("mine"))
	select {
	case <-mine:
...
func waitMarker(t *testing.T, runID string) {
	t.Helper()
	waitPS(t, "/bin/bash -c "+runScript+" "+RunMark+runID)
}
