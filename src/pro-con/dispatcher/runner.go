...
// Runner はコマンドを dir で実行し、出力を logPath へ書いて終了コードを返す。runID は実行ごとの印 (残った実行を後で見つけるため)。
...
func (ExecRunner) Run(ctx context.Context, dir, command, logPath, runID string) (int, error) {
