...
// PG への指示には規律 (AskUserQuestion を使わず pro-con card ask / 終えたら review / push しない。CLAUDE.md の push の規律より優先) とカードの ID が入る。
func TestPromptCarriesDiscipline(t *testing.T) {
	p := Prompt(card.Card{ID: "C-007", Title: "直す", Request: "色を直して"}, Review{})
	for _, want := range []string{"C-007", "AskUserQuestion", "pro-con card ask C-007", "pro-con card review C-007", "pro-con card run C-007 --", "**push しない** (master にも自分のブランチにも", "この規律を優先する", "色を直して", "pro-con card attach C-007 ", "結果が届くまで ask しない"} {
