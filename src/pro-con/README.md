...
| (PM の様子) | ゲージの「PG n/m」の隣に `PM n/m` (動いている PM の数 / 上限。今は 1) と様子 (作業中 + 手元のカード・idle・入力待ち・起動中・起こせない + 理由・落ちた・止めた・未起動・off)。依頼の列で PM が今分けているカード (質問待ちでは答えている質問) は、バッジに水色で `PM 分解中` (`PM 回答中`)。担当 (詳細・`card list` / `card show`) は記録の Owner ではなく今手を動かす者 (`card.Assignee`: 分解済みは `PG 待ち`)。様子は dispatcher が Tick ごとに `dispatcher-state.json` の `role_states` へ書き、止まった dispatcher の最後の様子は出さない (issue 476) |
