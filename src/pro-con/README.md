...
bin/pro-con config set limit 3 | set usage off | set pm 1 | set review codex | unset limit | show  # 止めずに PG の枠 (同時に動かす PG の上限) と PM の数を変える (issue 456)。受付の箱に置き、dispatcher が状態の置き場の settings.json に書いて次の Tick から使う (起動し直しても続く)
                         # `set usage off` で利用枠の絞りを止める (既定 on。枠を気にせず使うとき。尽きた PG は 429 で止まり watchdog が拾う。画面の設定のタブのチェックボックスと同じ)
                         # 🚨 上限の優先: 利用枠の絞り (80% で 1 本 / 95% で 0 本。usage が on の間) > 設定 (config set limit) > dispatcher の --limit > 既定 2。--limit は「設定が無いときの値」で、unset limit で戻る (画面が起こす dispatcher は --limit を付けない)
...
bin/pro-con ps [--json]  # pro-con が起動したプロセスを役ごとに出す (dispatcher / supervisor / 見張り / PM / PG / テストの係 / 画面。pid・経過・状態・カード・コマンド。カードと session の食い違いは状態の列に「食い違い: 」で出す。着手待ちの列で同じ session の再開を待つ PG は、列の名前ではなく「再開待ち (PG の空き待ち)」/「再開待ち (利用枠)」)。読むだけ: 状態の置き場に書かず、dispatcher の lock も画面の印 (presence) も触らない。生きているかは ps を 1 回読んで決める (busy / idle は出さない)。pro-con の外の session は出さない
...
- **クラッシュ・再起動の後は、起動したときに自動で復旧する** (コマンドも引数も無い。画面が起こしても手で起動しても同じ。issue 483)。
  ...
  **マシンの起動時刻 (`kern.boottime`) より前に始まり、`claude agents --json --all` に pid 無し・止まった state で残る**ものだけを
  待たずに再開する (作業中のカードは着手待ちへ戻して同じ session を再開、テストの係の結果を待つカードは頼み直してから。落ちた回数に数え、上限なら人の番へ)。
...
| s | 設定画面を開閉 (issue 456)。右から全幅の板が入ってきて、中を「設定 / プロセス / ディスク / ログ」のタブに分ける (tab / shift+tab で切り替え、j / k で行を選ぶ、s / q / esc で閉じる)。**設定**: PG の枠 (同時に動かす PG の上限) と PM の数を ← → (h / l) で 1 ずつ、敵対的レビューの担い手 (claude / codex。issue 514) を ← → で巡って変える。「利用枠を見て PG を絞る」はチェックボックス (Enter か ← → で入れ替える。`pro-con config set usage on|off` と同じ)。受付の箱に置き (`pro-con config set` と同じ)、dispatcher の次の Tick から効く (止めずに変わる)。適用されるまで「適用待ち」。利用枠の絞りは、チェックが入っている間この上限より優先。PM は今は 1 だけ。下に見る所の要約。**プロセス**: `pro-con ps` と同じ出どころの役ごとの一覧 (pid・経過・状態・カード・session・今のコマンド)。出すのは動いているものと、カードと食い違う PG (カードは作業中なのに session が止まっている / カードは完了なのに動いている。黄色の `!`) だけで、止まった PG (終わったカードのもの・質問待ち等) は出さず数えもしない (issue 497)。止まっている役 (PM・取り込み・テストの係) は 1 行に畳み、enter で開く。画面が 2 つ以上 (または join) なら下段に開いている画面の一覧 (モード・名前・開いた時刻・端末・pid)。**ディスク**: `pro-con du` と同じ、pro-con が作った物の使用量 (合計・置き場ごとの大きさと割合と数・大きい順の内訳 3 つ。enter でその置き場の内訳を全部。worktree と transcript は完了したカードに印)。**ログ** (issue 512): `pro-con log` と同じ出どころ (events.jsonl) から、プロセスの出来事 (dispatcher が起きた・抜けた・入れ替わった / supervisor が起こし直した / 見張り / PM・取り込み・PG の起動・再開・止めた・落ちた / 画面が開いた・閉じた) を新しいほど下に 1 行ずつ (時刻・役・カード・出来事)。同じ出来事 (数字だけ違う) の 2 分以内の繰り返しは 1 行に畳み、行末に「… ×N 回、最後 hh:mm (最初 hh:mm)」。c でカードの出来事 (適用・待たせた・テストの係の実行 等) も混ぜる。選んでいる行には下線を引き、y でその行 (時刻・役・カード・出来事・畳んだ回数。色なし) を、Y でその行の出来事の文だけをクリップボードへ写す。開いたときは最新を選び、一番下にいる間は足された出来事に付いていく。開いている間に足された分は変化の知らせで読み進める (描くたびに読み直さない)。🚨 見る所は描くたびに読まない (ディスクは数秒かかる): 開いたとき・プロセスのタブへ移ったとき・r で裏で 1 回読み、読んだ時刻を見出しに出す。`--view` では設定のタブを出さない (見る所だけ)。pro-con の外の session は名前も本数も出さない。画面は `claude agents` を自分で読まない |
| / | カードを絞り込む (issue 532)。最下段に検索の欄が出て、打つたびに題名・カード ID・issue 番号 (`#532` / `dotfiles#532`)・依頼の原文で絞る (大文字小文字を区別しない。空白で区切ると全部を含むもの。一致の判定は `card list --grep` と同じ `card.Matches`)。一致しないカードは隠して上へ詰め、題名の一致した語を橙の太字に、レーンの見出しを (一致/全部) にする。打っている間もカンバンは暗くしない。enter で確定すると絞り込みは残り、件数の行の頭に `/ <語> n/N 枚` を出してボードのキーが絞った結果に効く (/ で語の続きを直せる)。esc (確定後は q も。詳細を開いていれば先に詳細を閉じる) で絞り込みをやめる。空のまま enter でもやめる。絞るのは今のタブの中で、件数の行の列ごとの枚数と x の対象は絞る前のまま。🚨 絞っている間は K / J (見えている隣と入れ替わらない) と x (隠れた完了も片付く) を断る。実装は `ui/search.go` |
...
| K / J | 選択中のカードを 1 つ上 / 下のカードと入れ替える (issue 470。レーンの中は上ほど優先度が高い)。選択はカードについていく。端では止めて知らせる (巻かない)。repo のタブではその repo のカードの中の隣と入れ替える。書くのは dispatcher (画面は受付の箱に置くだけ。速く続けて押しても、押した回数だけ動く)。着手待ちの列は上から PG を起動する (回答・差し戻しを受けた再開は並びより先。バッジに ↻再開が先)。依頼の列は PM が上から分ける。`--after` の前のカードが終わっていなければ、上げても起動しない (履歴に理由)。詳細を開いている間の J / K は隣のカードへ送る |
...
| (送る前の確認) | 送る欄 (n / i の補足 / + / w / r と回答フォーム) は enter で送らず、画面の中央の枠に送る中身を折り返して全部出す。y / enter で送り、他のキーでは書いた中身のまま欄へ戻る (日本語入力の確定の enter が届いても、それ 1 つでは送らない。issue 517) |
...
| (PM の様子) | ゲージの「PG n/m」の隣に `PM n/m` (動いている PM の数 / 上限。今は 1) と様子 (作業中 + 手元のカード・idle・入力待ち・起動中・起こせない + 理由・落ちた・止めた・未起動・off)。依頼の列・質問待ちの PM の番のカードには PM の段階を出す (issue 480): `PM 知らせ待ち` (PM がまだ知らない) / `PM 知らせた` (知らせを渡している最中か、同じ turn でまだ扱っていない) / `PM 分解中 ▸ <最後の道具の呼び出し> <経過>` (水色。今の turn で扱っている 1 枚だけ。質問待ちは `PM 回答中`) / `PM 入力待ち` / `PM 知らせた (手を止めた)` (turn を終えたのに残っている)。扱っているカードは、PM の transcript で最後の起動・再開の後に、道具の呼び出しの対象 (コマンド・ファイル) に知らせ済みのカードの ID が出た最後のもの。積んだ・その場で閉じたカードは列を離れる (`card list` / `card show` にも同じものを出す。`--json` の鍵は `roleStep`)。担当 (詳細・`card list` / `card show`) は記録の Owner ではなく今手を動かす者 (`card.Assignee`: 着手待ちは `PG 待ち`)。様子は dispatcher が Tick ごとに `dispatcher-state.json` の `role_states` へ書き、止まった dispatcher の最後の様子は出さない (issue 476) |
| (処理中の印) | PG が turn の途中のカード (PG の status が busy) は、バッジの頭に回る印 (⠋⠙⠹…) を出す。回すカードがある間だけ 100ms ごとに描き直す (`ui/spinner.go`) |
| (待ちの地) | 待っているカード (作業中の列でテストの係の結果待ち = 実行中も含む / 着手待ちの列で再開待ち) は回る印を出さず、地をそのカード固有の色のまま明度 ×0.55 に暗くする (RGB で出す。truecolor の通らない端末では近い 256 色に落ちる)。まだ起動していない着手待ちのカードは暗くしない (issue 455) |
...
| ctrl+z | 画面を止めて裏に回す (シェルへ戻る。どの画面からでも)。`fg` で描き直して戻る。止まっている間も dispatcher と PG は別のプロセスで動き続ける。外からの SIGTSTP・前面を外された SIGTTIN でも同じく端末を戻してから止まる (issue 518) |
| a | PG の session を開く (今は模擬) |
| r | 質問待ちのカードに回答する。PG が選択肢つきで聞いた質問 (`card ask --json`。issue 493) なら、画面の中央に回答フォームを開く: 問いを縦に全部並べ、1 つ選ぶ問いは (•) / ( )、複数選べる問いは [x] / [ ]。推奨 (オレンジの「推奨」) は初めから選んである。j / k (↑ / ↓) で移り、space で選ぶ / 外す、tab / shift+tab で次 / 前の問い。どの問いにも「その他 (自由に書く)」、最後に「補足 (任意)」の欄があり、その行では打った文字がそのまま入る (↑ / ↓・tab で抜ける)。enter で送る前の確認へ (答えていない問いがあれば送らずにそこへ移る)、esc で取り消し。答えは問いごとに `N. 見出し: 選んだ名前` の行を並べた自由文として、今の回答 (`card answer`) で届く |
| + | 追加オーダー (tab で 追記 / 方針変更 / 別件)。方針変更は送る前の確認に「PG を止めて、指示を差し替えて再開します」を添える。届いたかは詳細に「未達 / 届いた」で出る |
...
- **切り替えの演出 (issue 509。`ui/switchfade.go`)**: 旧版は画面の色を端末の背景色へ寄せて 400ms で暗くし、中央の枠に
  「新版へ切り替え中…」を出してから終わる。**alt screen を抜けずに exec する** (`upgrade.Screen` が、切り替えのときだけ
  bubbletea の終了の `\e[?1049l` と `\e[?25h` を落とす) ので、切り替えの間もシェルの画面は見えない。新版は旧版の最後の
  暗い 1 枚を受け取り (引き継ぐ状態の `frame`)、最初の読み込みが届くまで (上限 1 秒) それを出してから 400ms で明るく戻す。
  exec に失敗したら明るい画面に戻して通知する。新版が画面を出す前に終わる (引数の誤り・panic) ときは、エラーを書く前に
  alt screen を抜ける (`main.go` の `altGuard`)。見本は `samples/509-upgrade-transition/`
  - 🚨 画面の出力を包んでいるので、端末を明け渡して子を走らせるときは `execOnTerminal` を通す (子の stdout を `os.Stdout` に
    固定する。包んだ出力のまま渡すと os/exec がパイプを挟み、attach の claude やエディタの stdout が端末でなくなる)
...
- 偽の PM: 依頼の列のカードを、その場で着手待ちにする (e2e#1)
