...
| (PM の様子) | ゲージの「PG n/m」の隣に `PM n/m` (動いている PM の数 / 上限。今は 1) と様子 (作業中 + 手元のカード・idle・入力待ち・起動中・起こせない + 理由・落ちた・止めた・未起動・off)。依頼の列・質問待ちの PM の番のカードには PM の段階を出す (issue 480): `PM 知らせ待ち` (PM がまだ知らない) / `PM 知らせた` (知らせを渡している最中か、同じ turn でまだ扱っていない) / `PM 分解中 ▸ <最後の道具の呼び出し> <経過>` (水色。今の turn で扱っている 1 枚だけ。質問待ちは `PM 回答中`) / `PM 入力待ち` / `PM 知らせた (手を止めた)` (turn を終えたのに残っている)。扱っているカードは、PM の transcript で最後の起動・再開の後に、道具の呼び出しの対象 (コマンド・ファイル) に知らせ済みのカードの ID が出た最後のもの。積んだ・その場で閉じたカードは列を離れる (`card list` / `card show` にも `progress` として同じものを出す)。担当 (詳細・`card list` / `card show`) は記録の Owner ではなく今手を動かす者 (`card.Assignee`: 分解済みは `PG 待ち`)。様子は dispatcher が Tick ごとに `dispatcher-state.json` の `role_states` へ書き、止まった dispatcher の最後の様子は出さない (issue 476) |
