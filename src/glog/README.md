...
| `d` | **コミットの diff をポップアップ表示** (`git show --stat --patch`。ほぼ全画面のモーダルで less 流儀にスクロール: `j`/`k`/`Enter` 行送り・`Space`/`f`/`b`/`C-d`/`C-u` 半ページ・`g`/`G`。末尾では最終行を表示したまま止まる。閉じるのは `q`/`h`/`Esc`/`d`。SHA ごとにセッション内キャッシュ) |
