...
//
// 🚨 ドットの縦横比: 端末セルは概ね 横 1 : 縦 2 なので、それを 横 2 x 縦 4 に割った 1 ドットは
// ほぼ正方形になる。円をドット座標でそのまま描けば端末上でも円に見える (縦横の補正は不要)。
// 🚨 幅: braille は termwidth の受理表に載っていて幅 1 が保証されている (termwidth.acceptSymbol)。
...
func (b *braille) putText(row, col int, s, color string) {
	...
	for _, r := range s {
		...
		// 🚨 全角が最終桁に来たら書かない。2 セル目に「覆われている」印を打てないので、
