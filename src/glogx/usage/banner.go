...
//
// 🚨 収録外の文字が 1 つでもあれば nil を返す。呼び出し側が普通の字へ落ちる。欠けた字を
...
var bigPixels = map[rune][pixelRows]string{
	...
	'D': {"### ", "#  #", "#  #", "#  #", "### "},
	'M': {"#  #", "####", "####", "#  #", "#  #"},
	// 未消費の枠に出す "NO DATA" 用 (issue 197 / ユーザー指示 2026-09-03)。
	// 日本語の説明文だけだと「盤が無い = 壊れた」に見えるので、他のカードの数字と
	// 同じ字形で状態を出す。O / D は上で収録済み
	'N': {"#  #", "## #", "# ##", "#  #", "#  #"},
	// 🚨 英大文字の O は数字の 0 と**別の字**。字形は同じでよいが、収録し忘れると
	// bigLines が nil を返して語ごと描けない (実測 2026-09-03: 'O' が無くて "NO DATA" が
	// 全幅でも AA にならず、原因の特定に往復した)
	'O': {"####", "#  #", "#  #", "#  #", "####"},
	'A': {"####", "#  #", "####", "#  #", "#  #"},
	'T': {"####", " ## ", " ## ", " ## ", " ## "},
	// 語を分かち書きするための空白。🚨 bannerPixels には在るが bigPixels には無かった
	// (数字と単位しか描いていなかったため)。無いと bigLines が nil を返し、空白を含む
	// 語は丸ごと描けない (実測 2026-09-03: "NO DATA" が全幅でも AA にならなかった)
	' ': {"    ", "    ", "    ", "    ", "    "},
}
...
//
// 🚨 termwidth.Of を介さず字数から計算している。幅の単一出典 (CLAUDE.md) の例外だが、
