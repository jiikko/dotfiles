...
// %B (フルメッセージ) はセパレータを含みうる唯一のフィールドなので最後に置く。境界文字が
// メッセージに入った病的ケースの扱いは非対称: \x1e (record sep) はレコードを割ってフィールド数
// 不足になり解析エラーで落ちる。\x1f (field sep) は SplitN の上限が本文 (--stat/-p) の \x1f を
// 1 フィールドへ吸収する idiom のため検出できず、当該メッセージが最初の \x1f で無言に切り詰め
// られる (残りは本文側へ漏れる) — 単一デリミタ方式では「メッセージの \x1f」と「本文の \x1f」を
// 区別できないための既知の制限。0x1F を含むコミットメッセージは実運用でほぼ発生しない。
const (
	recordSep    = "\x1e"
	fieldSep     = "\x1f"
...
func LoadLogDisplay(opts *Options, colored bool) ([]string, error) {
	...
	// VS16 付き絵文字 (⚠️ 等) の幅が Terminal/tmux と食い違い再描画で行がガタつくのを防ぐ
	// (dropEmojiVS16 の説明参照)。verbatim 表示は git 出力をそのまま流すのでここで正規化する。
...
func LoadCommits(opts *Options, colored bool) ([]Commit, error) {
	...
	// VS16 付き絵文字を text presentation (幅 1) へ正規化してから解析する。Subject/Message/Body
	// が下流の幅計算・描画で Terminal/tmux と幅一致し、再描画のガタつきを防ぐ (dropEmojiVS16 参照)。
