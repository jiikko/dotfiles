...
| `i` | **issues viewer を全画面で開く** (toggle。開くとき右から左へ流し込む演出。repo 直下と `<sub>/issues` の `*.md` をファイル名のカテゴリ別タブで一覧し、カテゴリごとに色を振る (`bug` は赤・`feat` は緑・未知の語は語のハッシュで安定に割る)。`Tab`/`h`/`l` でタブ、`Enter` で本文を markdown 整形して表示 (右から飛び出す引き出し。画面の 8 割を占め、左に一覧の先頭が残る。閉じるときは逆再生)、`p` で issue 番号・`y` でパス・`Y` で参照 (番号+タイトル+パス)・`N` で次に採番すべき番号をコピー、`a` で表示する状態を巡回 (既定は open のみ → +pending → +done)、本文で `u` を押すと URL 一覧のピッカーが出て、打った文字で絞り込み (`ctrl+n`/`ctrl+p` で移動・`Enter` で開く・`Esc` で戻る)、`v` で nvim、`r` で再読込、`q`/`i` で閉じる。**viewer を出したまま `C-g` で glogx ごと閉じると、次の起動は同じ画面から再開する** (タブ・状態フィルタ・カーソル・開いていた本文とそのスクロール位置。30 分以内・同じ repo のときだけで、演出は出さない。git log 一覧から閉じたときは記憶しない)。状態は `pending/` `done/` などディレクトリで判定し、件数 0 のカテゴリタブは右へ寄せる。規約は [docs/issues-viewer-spec.md](../../docs/issues-viewer-spec.md)) |
