...
//
// 🚨 rune 数ではなく表示幅で切る。呼び出し側 (tui.go) は m.width から引き算した残り桁を渡すので、
...
func truncate(s string, max int) string {
	...
	tail := "…"
	if max == 1 {
		// 🚨 1 桁しかないときは "…" で潰さず中身を優先する (元の実装からの意図的な挙動。
...
// closeSGR は装飾が開いたままの文字列に reset を足す。🚨 reset は幅 0 なので桁の約束を壊さない。
...
//
// 🚨 自前で走査して 1 rune = 1 桁と数えてはいけない (以前はそうしていた)。全角は 2 桁、
