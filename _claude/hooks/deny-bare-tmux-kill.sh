...
# 🚨 性能ガード (これが無いとゲートが黙って消える)。以降の検査は unquote の 1 文字ループと
...
# 🚨 この前置きフィルタは **遅延だけの fast path** で、判定は変えない (下のセグメント単位の
...
# 🚨 ここから先は入力長にコストが乗る。極端に大きい入力は timeout (settings.json: 10) に
...
# 🚨 ここを sed でやらないこと: 引用符と括弧を含む正規表現を sed -E に渡すと BSD sed が
...
while IFS= read -r seg; do
  ...
  # 🚨 重い解析はトークンを含むセグメントだけに入れる。unquote は 1 文字ループ + $(...) の
  ...
  # 🚨 大入力では unquote を諦めて生セグメントを走査する。unquote は 1 文字ループで、bash の
