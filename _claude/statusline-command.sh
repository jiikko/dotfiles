...
#
# 🚨 shebang は bash 必須 (sh 不可)。パス整形が bash 専用の置換に依存している:
...
# 🚨 フィールドごとに jq を起動しないこと: statusline は再描画ごとに走るため、以前は
# 1 レンダーで jq を 10 プロセス起動して実測 26ms (レンダー全体 75ms) を払っていた。
# 🚨 jq の出力順とこの read 順は 1:1 の契約。フィールドを足すときは両方を同じ位置に足す。
# 🚨 各フィールドは `// ""` で必ず 1 行出すこと (`// empty` だと値が無い行が消えて以降が全部ズレる)。
...
# 🚨 bash の $(( )) は "08" (8 進数として解釈され失敗) / "5E+1" / 全角数字で **fatal**
...
to_int() {  # 結果は REPLY (整数化できなければ空)。$(...) を使わないため出力しない
  REPLY=${1%%.*}
  # 🚨 数字の判定に範囲 [!0-9] を使わないこと。locale の照合順では全角数字が 0-9 の
...
# 🚨 結果は stdout でなく REPLY で返す。statusline は再描画ごとに走るため、$(...) は
...
# 🚨 epoch から日時への変換はプラットフォームで別物: BSD (macOS) は `date -r <epoch>`、
...
pace_row() {
  ...
  case $pr_kind in
    ...
    # 🚨 7d は絶対時刻を出さない (pr_efmt が空)。残り日数で十分で、"8月26日19:18" は
  ...
  # 持ち、整数演算だけで済ませる。
  # 🚨 カレンダー基準 (ローカルの 0 時 / 毎時 00 分に揃えた格子) にはしない。以前は
  ...
  # 🚨 空白は括弧の**外**に置く。括弧の中に入れると「空のスロット」に見えて、その窓が
  ...
  # 🚨 帯の中では前借り (赤) / 使い残し (シアン) を出さない。1 カラム = 50/ncells pt なので、
  #   乖離が数 pt でもカラム境界を跨げば半日分の赤が出てしまい、「想定通り」のラベルと
  #   矛盾して見える (実測 2026-08-23)。
  # 🚨 逆に帯の外では、塗りが窓の右端に clamp されると想定線と同じカラムに入り、超過が
  ...
  # magenta にする。
  # 🚨 100% 到達は乖離に関わらず赤 + 「上限」にする。乖離が +18pt でも「先行 (黄)」で
  #   済ませない (上限に届いている事実の方が重い)。
  # 🚨 状態の語は全て 2 文字 (4 カラム) に揃える。語の幅が変わると、その後ろの残り時間・
  #   予算が行ごとに横へずれる (実測: 余裕 4 カラム / 想定通り 8 / 余らせ過ぎ 10)。
  # 🚨 帯の中 (適正) も語を出す。空にすると乖離 pt の直後がその行だけ残り時間になり、
  #   他の状態の行と縦が揃わない (実測: 5h に「余裕」があるのに 7d には何も無い、と読めた)。
  # 🚨 アドバイスは「語で言えないこと」だけを持つ: 帯の中は語 (適正) で足りるので出さない。
  ...
  # 🚨 このガードは出力を変えない (空の書式で呼んでも fmt_epoch は空を返す) が、外すと
  ...
  # 🚨 区切りは中黒でなく空白 1 つ。項目 (残N / N%/日 / N日分の…) は形が違うので
  ...
  # 🚨 数値は桁を固定して右詰めにする (使用率・想定率は 3 桁、乖離 pt は符号込み 4 桁)。
