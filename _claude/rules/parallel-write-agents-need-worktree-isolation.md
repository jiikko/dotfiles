...
- **他人の worktree / 未コミット差分を「放置された残骸」と判断する前に、そのセッションが
  ...
  「誰かが今も動いているか」の証拠ではない。実例 2026-09-03: 予約セッションの worktree 2 つを
  `git log` と未コミット差分だけで「途中で止まった」と診断し、片方を削除した。実際は相手は
  running で、その差分はレビュー中の作業だった (内容が同一だったので実害は出なかった)。
  `ListAgents` を 1 回叩けば分かった
