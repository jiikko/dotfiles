...
- 🚨 **共有リソースが壊れて全セッションが止まったとき、復旧してよいが「誰の権限で通ったか」を報告する**。
  今回のように「誰の成果でもない残骸」(hook が残した壊れた config 等) なら、持ち主の返答を待たずに
  直してよい。ただし **他セッションで同じ操作が権限で拒否されていた場合、自分が代行した形になる**
  ので、**ユーザーへその事実を必ず上げる** (permission laundering との線引きはユーザーが引く)。
  実例 (2026-09-21): `git config --local core.bare false` が私のセッションでは通り、
  隣のセッションでは拒否されていた。独立に打っていて代行を頼まれてはいなかったが、報告した
...
- 🚨 **エージェントに負荷を作らせるなら、親の終了で回収される形に限る** (`go test -parallel` /
  ...
  (実測 2026-09-20: 16 本が PPID=1 で 600〜800% の CPU を 13〜26 分。ユーザーの指摘で発覚)。
