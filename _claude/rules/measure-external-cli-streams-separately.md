...
- **外部ソース (upstream のソースコード・ドキュメント・WebFetch の要約) は「仮説」であって
  ...
  - **それを読んで自分が書いた fake の振る舞いも同じく仮説**。外部 CLI の状態遷移 (再開・再起動・停止で id / pid / 時刻が
    どう変わるか) を模す fake は、書く前に本物で 1 回測る (実測 2026-09-25 dotfiles 427: fake の再開は同じ session を続ける前提で、
    本物の `claude --bg --resume` は別の id を立てていた。その前提の上で敵対レビューを 5 周重ねた)
