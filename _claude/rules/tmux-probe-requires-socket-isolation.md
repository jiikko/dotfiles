...
- hook は Bash ツールのコマンド文字列を静的検査するため、**散文（コミットメッセージ・echo・ドキュメント引用）に「tmux → kill-server/kill-session」の並びや「pkill と tmux の同居」が同一行にあると偽陽性 deny になる**。回避は言い換え（「ソケット未指定の kill-server」等、小文字 `tmux` を同じ行に置かない）。[`no-comment-line-starting-with-shellcheck.md`](no-comment-line-starting-with-shellcheck.md) と同族の罠
