...
実例は `~/dotfiles/_claude/rules-rationale/claude-md-maintenance.md` に置く（起動時には読まれない。ルールを疑う・改訂するときに読む）。
...
| **共通 helper / 共通層を X に新設・抽出する** | **実装前に** `X/CLAUDE.md` を読む。抽出元のコードだけ見て書くと、その領域が既に潰した divergence を再輸入する。**規約を読むより先に、同じファイル / 同じ lib の中を helper 名と用途で grep する**。**あわせて `grep -rln '<触るファイル名>' issues/` を打ち、そのファイルを名指ししている open issue を読む** — 「今は手を入れるな」という判断が issue 側にだけ書かれていることがある |
| ファイル rename / 削除 | 旧名を `grep -r "<old-name>" **/CLAUDE.md` で点検して更新 |
| **ディレクトリ階層・置き場所の契約を変えた** (サブディレクトリを 1 段足す、状態ディレクトリを増やす等) | **深さや置き場所を前提にした検査・スニペット**を grep する: `find -maxdepth` / `ls <dir> <dir>` の列挙 / `*/` の段数を決め打ちした glob。spec・rule・hook を揃えても、`-maxdepth 2` の検査 1 本が新しい段を**黙って対象外**にする。**その置き場所へ実際にファイルを動かす手順書** (skill / hook / スクリプト) も同じ変更で直す — 探すのは文言ではなく**動作** (`git mv` / `mv` / `os.Rename`)。宣言 (spec・rule・README) と手順は語彙が違うので、「完了は global へ」で grep しても「issue ファイルを `issues/done/` に移動する」には当たらない |
| **error code / status / 返却形の契約を変えた** | **その契約を assert している手順書**を同じ変更で直す (skill / E2E シナリオ / runbook)。コードとテストは compiler とテストが守るが、**手順書は誰も守らない**。探すのは文言ではなく**旧の値そのもの** (`grep -rn '<旧 error code>' .claude/skills/ docs/`)。上の行がディレクトリの契約、こちらは**値の契約** |
