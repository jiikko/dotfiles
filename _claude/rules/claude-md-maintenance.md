...
| **ディレクトリ階層・置き場所の契約を変えた** (サブディレクトリを 1 段足す、状態ディレクトリを増やす等) | **深さや置き場所を前提にした検査・スニペット**を grep する: `find -maxdepth` / `ls <dir> <dir>` の列挙 / `*/` の段数を決め打ちした glob。spec・rule・hook を揃えても、`-maxdepth 2` の検査 1 本が新しい段を**黙って対象外**にする (実例 2026-09-05: `issues/epic/<name>/` を足したとき、番号一意テストだけが旧深さのまま緑を出し続けた) |
