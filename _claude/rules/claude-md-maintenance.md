...
| **共通 helper / 共通層を X に新設・抽出する** | **実装前に** `X/CLAUDE.md` を読む。抽出元のコードだけ見て書くと、その領域が既に潰した divergence を再輸入する (実例 2026-08-21 obaket 537: `Storage/HTTP/CLAUDE.md` が「5xx 判定は `HTTP5xxErrorClassifier` に委譲」と明文化していたのに、2 provider の共通部だけ見て 5xx 分岐を再実装した)。共通化は既存規約が最も効く場面なのに、規約の参照が最も抜けやすい。**規約を読むより先に、同じファイル / 同じ lib の中を helper 名と用途で grep する** — 実物の重複はそこにあることがある (実例 2026-08-28: 同じファイルの 100 行上にある `tt_mtime_of` に気づかず、順序が逆で壊れる版を新設した) |
| ファイル rename / 削除 | 旧名を `grep -r "<old-name>" **/CLAUDE.md` で点検して更新 |
| **ディレクトリ階層・置き場所の契約を変えた** (サブディレクトリを 1 段足す、状態ディレクトリを増やす等) | **深さや置き場所を前提にした検査・スニペット**を grep する: `find -maxdepth` / `ls <dir> <dir>` の列挙 / `*/` の段数を決め打ちした glob。spec・rule・hook を揃えても、`-maxdepth 2` の検査 1 本が新しい段を**黙って対象外**にする (実例 2026-09-05: `issues/epic/<name>/` を足したとき、番号一意テストだけが旧深さのまま緑を出し続けた)。**その置き場所へ実際にファイルを動かす手順書** (skill / hook / スクリプト) も同じ変更で直す — 探すのは文言ではなく**動作** (`git mv` / `mv` / `os.Rename`)。宣言 (spec・rule・README) と手順は語彙が違うので、「完了は global へ」で grep しても「issue ファイルを `issues/done/` に移動する」には当たらない (実例 2026-09-06: issue 291 で group issue の完了先を変えたとき、宣言 5 箇所は揃えたのに issue-sync skill の Step 6 だけが旧契約のまま残った) |
...
- ✓ シンボル名 / ファイルパスを grep で追える形で書く。**行番号で位置を pin しない** (1 行の増減で無言にドリフトする。file 名 + symbol 名で書く)。**この項は CLAUDE.md に限らず issue / docs / コメントにも効く** (どれも参照先より長生きする)
