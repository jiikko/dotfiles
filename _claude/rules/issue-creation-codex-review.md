...
- **依頼は「妥当性の確認」ではなく「反証」で投げる**。「この issue の主張は誤りだという前提で、事実と食い違う箇所を挙げて」と敵対側に立たせる (肯定形で聞くと追認が返ってくる)
...
- **codex を使わない環境では、観点を分けたサブエージェント (read-only) の反証レビューで代替する**。
  「レビューして」ではなく**観点を分けて反証させる**のが要点で、①壊す ②素通り (false green)
  ③回帰 のように分けると、1 体に全部見せるより指摘が具体的になる。実績: dotfiles 2026-08-22
  (3 体・重複除去 14 件 → 対応 11 / 記録 3) と 2026-08-27 (P1 3 件がいずれも本物)。
  **「外部レビューを通せない環境だから省略する」は採らない** (省略するなら、その旨を issue に
  明記して「反証されていない仕様」として扱う)

## なぜ

根拠・起源・実例は `~/dotfiles/_claude/rules-rationale/issue-creation-codex-review.md` に置く（起動時には読まれない。ルールを疑う・改訂するときに読む）。
...
issue のレビュー依頼。この issue の記述は間違っているという前提で、現コードと突き合わせて反証してほしい。

## 聞きたいこと
- 事実誤認 (file:line / カウント / 関連 issue 参照)
- 対応方針の見落とし / 断定が現コードと一致しているか / 重要度判断の妥当性
- この issue が「既に修正済み」「そもそも再現しない」可能性はないか (根拠つきで)

問題のあるものだけ重要度順 (P1/P2/P3) に。反証できなかった主張は「反証できなかった」と明記して。
---
EOF
