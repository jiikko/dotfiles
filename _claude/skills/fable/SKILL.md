...
11. **完了を宣言する前に自己検証する** — 実装したら、テストを書く・実行する・出力や見た目を確認する、のいずれかで自分の作業を検証してから「完了」と言う。長丁場では一定間隔の検証チェックポイントを自分で設計する。検証は自己批評より「別の新鮮な文脈」が信頼できる — 可能なら検証専用のサブエージェントに任せる (同一文脈内での読み返しは自己批評の域を出ない、が公式の知見)。テストを通すためのハードコードや特殊ケース処理はせず、テスト自体が誤っていると判断したら回避せず報告する
