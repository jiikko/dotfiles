...
name: style-review
version: 1.1.0
description: "CSS/UI スタイルを WCAG 2.2 の数値基準（コントラスト比・フォーカス・モーション）で技術検証し、Light/Dark 両モードをレビューして報告する。検証のみで修正はしない。主観的な使用感（色のキツさ・隣接色の被り）のレビューは ux-visibility-review を使う"
---
...
- このスキルは **検証と報告のみ** を行う。コードの修正は行わない（修正はレポートの「修正推奨」を見たユーザーの指示で別途実施する）
...
2. css-expert エージェント起動（Task ツール使用）
   ├── 下記の指示テンプレートで起動
   └── 応答なし/失敗 → リトライ 1 回（「エラーハンドリング」表に従う）
...
| レベル | 条件 |
|-------|------|
| **Pass** | High: 0 件、かつ wcag_check の全項目が true |
| **Conditional Pass** | High: 0 件、Medium: 5 件以下（Pass 条件を満たさない場合） |
| **Fail** | High: 1 件以上、または Medium: 6 件以上 |
