...
> **Minimum モード**: 3エージェントのみを**直列実行**。Phase 4.1/4.2 は省略。
> **Standard モード**: 必須6エージェントを**並行実行**。
> **Maximum モード**: 全エージェント（条件付き含む）を**並行実行**。
> **Ultra モード**: Phase 4.3（反復並列思考）を実行。Phase 4.1/4.2 は省略。
