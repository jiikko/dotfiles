...
```markdown
□ CLAUDE.md のルールに準拠
  - プロジェクトの CLAUDE.md に定義された禁止パターンに違反していないか
  - コーディング規約（命名、スレッド制約等）を遵守しているか
...
```bash
# 実装後、必ず実行（make が無いプロジェクトでは同等コマンド: npm run build 等）
make build
```
...
実装完了後、以下の5観点のセルフレビューを全モード共通で順に実施する。
...
**BLOCKER 条件**（プロジェクトの CLAUDE.md で禁止されている場合のみ。禁止規定がなければ WARNING 扱い）:
- `!` 強制アンラップが存在
- `[0]`, `[n]` の配列直接アクセスが存在
...
> **注意**: `make` ターゲットが無いプロジェクトでは同等コマンド（例: `npm run lint/build/test`）に読み替える。対応するコマンド自体が存在しない項目はスキップし、完了レポートの「最終確認」に「該当コマンドなし」と明記する。
...
- 専門家相談でも BLOCKER が解消しない場合: 自前修正を続けず、状況（試行履歴・残存 BLOCKER）を整理してユーザーに報告し、判断を仰ぐ
