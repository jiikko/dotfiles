...
| 入力パターン | タイプ | フロー |
|-------------|--------|--------|
| ファイル/ディレクトリパス | レビュー | Phase -1 → 4 → 4.1 → 4.2 → 4.5 → 5 → 5.5 → 完了 |
| それ以外（タスク説明） | 実装 | Phase -1 → 0 → 1 → 1.1 → 1.5 → 2 → 3 → 3.5 → 4 → 4.1 → 4.2 → 4.5 → 5 → 5.5 → 完了 |
...
> **詳細**: `_common/modes.md` を参照（スコアリングシステム、各フェーズ動作テーブル）
...
> **注意**: Phase 0 で読み込んだ内容は Phase 2/3 でも有効です。Phase 2/3 で専門家相談（agents.md）やスキル検出（skill-triggers.md）が必要な場合は、Phase 0 で既に読み込み済みの内容を使用してください。
