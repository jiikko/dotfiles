...
> **🚨 ここに書くのはエージェント名と役割だけ**。プロンプト本文は forge v2 (`forge.js`) の汎用 `investigatePrompt()` / `reviewPrompt()` が生成するので、変えたいときは `forge.js` を編集する (かつてここに載せていた per-agent プロンプトは実行時に使われておらず、削除した。履歴は `git log` にある)。実運用で意味を持つのは、この後の「条件付き必須」「追加エージェント（検出パターン）」「言語別置換」テーブル (skill 層が `extraAgents` を決めるのに使う)。「(model: main 継承)」は固定値ではなく、その時のセッションモデルを指す。
