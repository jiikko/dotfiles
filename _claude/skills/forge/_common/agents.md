...
> **⚠️ 以下の per-agent「Phase 1/4 用プロンプト」は実行時には使われない（履歴・参考）**。forge v2 (`forge.js`) は各エージェントを `agentType` として起動し、プロンプト本文は forge.js の汎用 `investigatePrompt()` / `reviewPrompt()` が生成する。**ここのプロンプトを編集しても動作は変わらない**。実運用で意味を持つのは「エージェント名・役割」と、後述の「条件付き必須」「追加エージェント（検出パターン）」「言語別置換」テーブル（skill 層が `extraAgents` を決めるのに使う）。プロンプトを変えたい場合は `forge.js` を編集すること。モデルは forge.js が `model` を指定しない（= セッションの main モデルを継承する）ため、「(model: main 継承)」表記は固定値ではなくその時のセッションモデルを指す。
...
**プロジェクトルール**: プロジェクトの CLAUDE.md や規約ドキュメントにコーディングルールが定義されていれば、それに準拠しているか検証してください。
...
**プロジェクトルール**: プロジェクトの CLAUDE.md や規約ドキュメントに UI/フレームワーク固有のルールが定義されていれば、それに準拠しているか検証してください。
