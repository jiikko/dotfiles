...
|-------|--------------|---------|--------------|--------------|------|
| **Minimum** | 3 | 直列 | 省略 | なし | 軽量タスク、単純な修正 |
...
| タスク特徴 | 推奨モード | 判定基準 |
|-----------|----------|---------|
| 単一ファイル、小規模変更 | Minimum | 影響ファイル数 ≤ 3、修正行数 ≤ 50 |
...
| フェーズ | Minimum | Standard | Maximum | Ultra |
|---------|---------|----------|---------|-------|
| Phase 0 | 実行 | 実行 | 実行 | 実行 |
| Phase 1 | 3直列 | 6並行 | 全員並行 | 全員並行 |
| Phase 1.1 | **省略** | 実行 | 実行 | **省略** |
| Phase 1.5 | 簡略 | 標準 | 拡張 | 拡張 |
| Phase 2-3 | 実行 | 実行 | 実行 | 実行 |
| Phase 3.5 | **省略** | 実行 | 実行 | 実行 |
| Phase 4 | 3直列 | 6並行 | 全員並行 | 全員並行 |
| Phase 4.1 | **省略** | 実行 | 実行 | **省略** |
| Phase 4.2 | **省略** | 実行 | 実行 | **省略** |
| Phase 4.3 | - | - | - | **実行** |
| Phase 4.5 | 実行 | 実行 | 実行 | 実行 |
| Phase 5 | 実行 | 実行 | 実行 | 実行 |
| Phase 5.5 | **省略** | **省略** | 実行 | 実行 |
