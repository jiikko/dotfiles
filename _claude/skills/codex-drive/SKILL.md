...
name: codex-drive
version: 2.1.0
description: codex を設計の壁打ちからメイン実装者まで主役にし (設計 read-only → 実装 codex exec -s workspace-write)、Claude はオーケストレーション (要件確定・spec 多重読解・スコープ分割・設計/成果物の検閲・敵対的レビュー・ミューテーション検証・観測駆動デバッグ・commit/push・反復・要件照合) に徹するワークフロー。大きめの実装/移植/プロトコル実装で、余っている codex トークンを使い切りたい時に使う。「codex に書かせて」「codex メインで実装」「codex に作らせて」「codex-drive」「/codex-drive」で発火。typo・数行修正には使わない (それは Claude が直接やる)。
---
...
トークンは 2 方向に厚く使う (v2.0.0):

1. **独立した視点を増やす**: spec 独立読解 2 本 + 相互照合 (`[S]`)、設計案は独立 4 本 (`[D1]`) + クロス批評
   (`[D1.5]`)、方針が割れうる実装は競作 (`[2p]`)、発見型レビュー 3 観点 (`[3.5]`)、敵対的レビューは攻め口別に
   並列で枯れるまで (`[3.6]`)、壊せなかった不変条件はテストで固定 (`[3.7]`) し、そのテストの検知力を
   ミューテーションで機械検証 (`[3.8]`)。
2. **Claude 検閲の前処理に使う**: 律速は Claude の精読なので、「指摘の再現手順の構築」(`[3.55]`)・
   「digest 同士の突き合わせ」(`[S]`)・「敗者剖検」(`[2p]`) のような**下ごしらえを codex にやらせ、
   Claude は判定済みの材料 + 原文 spot check で最終判断だけを握る**。検閲を薄めるのではなく、
   検閲 1 件あたりの単価を下げて総検閲量を増やす方向にトークンを使う。

本数を増やせない箇所は **effort とモデルの底上げ**で贅沢する: **effort 既定 high**・
機械判定できる定型のみ medium・上位モデル (`sol`) 既定。
...
- **Claude が批評込みで統合する**: 重複を畳み、軸ごとの trade-off と各案への批評を 1 枚に並べ、推奨案
  (+ 他案から取り込む部分) を作る。批評で initial 案が全滅したら、その批評内容を出発点制約に足して
  D1 をもう 1 巡する (最大 2 巡)。
...
- **全出力が揃ったら重複をマージし、`[3.55]` の裏取りへ渡す**。裏取り結果を受けて Claude が統合・検閲する
  (`subagent-model-tiering.md`)。レビュー出力を無検閲で採用しない。
...
- **codex** (workspace-write・`-C <worktree>`・`gpt-5.6-luna`・medium — 変異の注入は機械作業) に指示する:
  「このマイルストーンの実装に、**現実的なバグを 3〜5 個、1 個ずつ独立の patch として**作れ
  (境界の off-by-one・条件の反転・エラー処理の握りつぶし・early return の欠落など、レビューで見逃しやすい類)。
  各 patch は `git diff` 形式で別ファイルに出力。テストファイルは触らない」
- **Claude が機械実行する**: worktree で各 patch を適用 → テスト実行 → **落ちるか**を記録 → patch を戻す
  (worktree なので `git checkout -- .` で安全に戻せる)。
  - **全変異が検知された** → テストの検知力が確認できた。worktree を削除して `[4]` へ
  - **検知漏れがあった** → その変異は「テストの穴」の実物。`[3.7]` に的として差し戻し
    (「この patch を落とすテストを書け」— 的が具体的なので循環テストになりにくい)。テスト追加後に漏れた変異だけ再検証
- **変異の質は Claude が一瞥する**: コンパイルが通らない変異・実装と無関係な変異は検知できて当然なので数に入れない。
