...
**例外は下表の「裁定・ゲート役」3 フェーズだけで、そこは `gpt-5.6-sol` + effort `medium` (= sol-mid) を使ってよい**
(ユーザー決定 2026-08-26)。
...
- **モデルはスキル側で明示する (フェーズで振り分けない)**: `-m <model> -c model_reasoning_effort=...`。省略すると
  ...
  - **例外: 「裁定・ゲート役」の 3 フェーズだけ `gpt-5.6-sol` + effort `medium` (sol-mid) を使ってよい**
    (ユーザー決定 2026-08-26)。判定条件は**フェーズ名で固定**であり、その場の難易度判定ではない:

    | フェーズ | 役割 | なぜ sol-mid か |
    |---|---|---|
    | `[S]` の**第 3 の codex** (相互照合) | 独立 2 本の読解の相違を裁定 | 1 本しか走らず、出した合意 digest が**以降の唯一の oracle**。ここの誤りは実装・テスト・レビュー全部に伝播する |
    | `[D3]` の**敵対観点 1 本** | 設計を破綻させるシナリオ構築 | 設計の穴は実装後に見つけるほど高くつく。承認ゲートの前の最後の関門 |
    | `[7]` の**敵対照合 1 本** | 未充足要件の探索 | 出口ゲートで**後工程が無い**。ここの見落としは誰も拾わない |

    共通するのは「**1 本しか走らず、間違えても後段で拾えない**」こと。逆に `[D1]` の 4 案・
    `[3.5]` の 3 観点・`[3.6]` の lens 別のように**冗長性が誤りを吸収する**フェーズは luna-max のまま。
  - **上げてよいのは上表の 3 フェーズだけ**。「難しそうだから」で他フェーズを sol にしない。
    それでも足りないと感じたら、モデルを上げる代わりに forge / cross-review へ escalate する
...
- **effort は全フェーズ `max` 固定** (= `gpt-5.6-luna` の最大 effort。ユーザー決定 2026-08-25):
  ...
  - **`medium` は sol と組でのみ使う** (= sol-mid)。上のモデル規約が挙げた 3 フェーズ専用で、
    `gpt-5.6-luna` に `medium` を組み合わせることはしない (luna を使うなら常に max)。
...
- **第 3 の codex** (read-only・**sol-mid** = `gpt-5.6-sol` + effort `medium`。裁定役の例外) に 2 本の digest を突き合わせさせ、**相違点と「どちらが spec に
...
- **Claude 自身も 1 視点として設計をレビューする**。codex の設計を codex だけでレビューすると同一モデルの
  ...
  別モデル視点を必ず 1 本混ぜる。
- **発見型 (A) は luna-max、敵対 (B) は sol-mid** (`gpt-5.6-sol` + effort `medium`。裁定・ゲート役の例外)。
...
- **codex の exit 0 を「実装した」の証拠にしない (空振りの検出)**。実装フェーズの codex は、方針を述べただけで
  ...
  **workspace-write の実装フェーズで、期待した成果物のパスに差分が無い**ときに限る。並行セッションの
  ...
  ⚠️ **空振りと判定する前に、その run が終了していることを確認する**。rc が確定する前に成果物の有無だけ見ると、
...
- **codex 敵対照合 1 本を必須で先行させる** (read-only・**sol-mid** = `gpt-5.6-sol` + effort `medium`。出口ゲートの例外)。「充足を確認して」ではなく
