...
トークンは**独立した視点を増やす方向**に厚く使う: 設計案は独立 4 本 (`[D1]`)、方針が割れうる実装は 2 本の競作
(`[2p]`)、敵対的レビューは攻め口別に並列で枯れるまで (`[3.6]`)、壊せなかった不変条件はテストで固定 (`[3.7]`)。
本数を増やせない箇所 (律速は Claude の検閲) は **effort とモデルの底上げ**で贅沢する: effort 既定 medium・
敵対系 high・設計判断を含む実装は上位モデル。
...
- **モデルはスキル側で明示し、フェーズで振り分ける**: `-m <model> -c model_reasoning_effort=...`。省略すると
  ...
  - **`gpt-5.6-sol` (上位)**: 設計壁打ち (`[D1]`〜`[D3]`)・**設計判断を含む実装・非自明な wire/protocol 実装**
    (`[2]`/`[2p]` のうち該当マイルストーン)・敵対的レビュー (`[3.6]`・D3 の敵対観点)・
    テスト強化 (`[3.7]`)・要件照合 (`[7]`)。**正解が客観確定しにくい / 反証を構築する / 判断を含む**フェーズに上位モデルを充てる。
  - **`gpt-5.6-luna`**: 機械的移植・rename・boilerplate の実装 (`[2]`)・発見型レビュー (`[3.5]`) など、
    設計が確定していて出力の正否を build/test/diff で機械判定できるフェーズ。
  - 迷ったら **sol に寄せる** (codex トークンは余っている前提。luna に落とすのは機械判定できる定型作業だけ)。
- **effort はタスクに合わせて選ぶ (既定 `medium`、trivial だけ落とし、敵対系は上げる)**:
  - `low`: 機械的置換・定型修正・grep 系調査・単純な boilerplate。**trivial と確信できるときだけ**ここに落とす。
  - `medium`: **既定はこれ**。実装・発見型レビュー・設計壁打ち・根本原因分析など、上下の条件に当たらないもの全部。
  - `high`: **敵対系の既定** (`[3.6]`・D3 の敵対観点)。反証は構築作業で、effort を削ると表層で止まる。
    加えて収束しない難所 (framework 内部挙動・並行不整合・二回外した後の分析)。high でも
  ...
  → コスト最適化より品質を取る (codex トークンは余っている前提)。「low で出して様子を見る」運用はしない。
    以降の実行コマンド例の effort はこの既定を反映した値であり、trivial なマイルストーンでのみ `"low"` に落としてよい。
...
[D1]  設計アウトライン  … codex 独立 4 本 (read-only) が各々 1 案を作る → Claude が統合。設計自由度が低ければ軽量パス (+ 敵対 1 本)
...
[2]   codex 実装       … codex exec -s workspace-write に指示。codex がファイルを書きプロジェクト標準の build/test まで回す
                        方針が割れうるマイルストーンは既定で [2p] 競作 (独立 2 本を別 worktree で走らせ勝者を採用)
...
- **他案の存在を各本に伝えない**。「他案と差別化して」と書くと、独立性を保つためにやっている分割が
  「相手を意識した案」に崩れる。
- 4 本揃ったら **Claude が統合する**: 重複を畳み、軸ごとの trade-off を 1 枚に並べ、推奨案 (+ 他案から取り込む
  部分) を作る。各案が実質同じ内容に収束したなら、それは「設計自由度が低かった」ことの証拠なので軽量パスに
...
- **Claude 自身も 1 視点として設計をレビューする**。codex の設計を codex だけでレビューすると同一モデルの
  ...
  別モデル視点を必ず 1 本混ぜる。
- 敵対側 (B) の effort は **`high`** (敵対系の既定)、発見側 (A) は `medium`。モデルはどちらも `gpt-5.6-sol`。
...
- **モデルはマイルストーンの性質で振る** (大原則のモデル振り分け): 機械的移植・boilerplate なら例のとおり `gpt-5.6-luna`、
  **設計判断を含む実装・非自明な wire/protocol 実装なら `gpt-5.6-sol` に格上げ**する。
...
- effort は既定どおり `medium` (テスト設計は構築作業だが的が `[3.6]` で確定済み)、モデルは上位側 (`gpt-5.6-sol`)。
  書き込みが必要なので `-s workspace-write`。
...
- ✓ 設計案出し [D1] は独立 4 本に 1 案ずつ作らせ (他案の存在を伝えない)、Claude が統合して推奨を作る
