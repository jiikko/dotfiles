...
- **レビューは「探す」ではなく「壊す」**: 実装者が codex なら、レビューも codex に任せた時点で同一モデルの相関盲点が残る。
  各マイルストーンは発見型レビュー `[3.5]` に加えて **敵対的レビュー `[3.6]` (red team) を commit 前の最終ゲート**に通す。
...
[3.6] 敵対的レビュー    … commit 直前の最終ゲート。「壊れている」前提で red team を回し、発火条件つきの反証だけ採る
...
ここは「問題を探す」発見型パスで、commit 直前の最終ゲートは `[3.6]` の敵対的レビューが担う (2 段構え)。
...
### 3.6. 敵対的レビュー（red team・commit 直前の最終ゲート）

`[3.5]` の指摘対応が終わり **build/test が green に戻った状態**で、commit の直前に 1 本回す。
...
- このタスク固有の攻め口を末尾に追記する。`[0]` の「既知の弱点」表がそのまま攻撃対象:
...
- 出力は `[3.5]` とぶつからない別パスへ。`[3.5]` の 2 本と**同時に走らせない** (3.5 の指摘を直した後のコードを攻めるため)。

```bash
stamp="$(date +%Y%m%d-%H%M%S).$$"
command codex exec review -m gpt-5.6-luna -c model_reasoning_effort="medium" \
  --ephemeral -o "<scratchpad>/codex-drive.$stamp.adversarial.md" </dev/null \
  "<codex-review「敵対的モード」テンプレート全文 + 上記のタスク固有の攻め口>" 2>&1 | tail -40
```
...
| 出力 | 扱い |
|---|---|
| 発火条件が具体的 → 再現できた | 修正対象。`[2]` に戻して構造的に直す (直したら 3.6 を 1 回だけ再実行) |
...
- **再実行は最大 2 回**まで。それでも新規の反証が出続ける場合は、マイルストーンの切り方か設計前提を疑い
  `[6]`／D2・D3 に戻す (反証を潰し続けるモグラ叩きにしない)。
...
- ✓ commit 直前に [3.6] 敵対的レビューを最終ゲートとして通し、発火条件つきの反証だけを修正に採る
...
- ✗ [3.6] を黙って省略する / [3.5] と同時に走らせる (3.5 の修正後のコードを攻める) / 反証のモグラ叩きを 3 回以上続ける
