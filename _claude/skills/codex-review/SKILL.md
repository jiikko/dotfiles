...
- **worktree など別 tree をレビューさせるときは `cd` する。`codex exec review` は `-C` を受け付けない**
  (`codex exec -s read-only` は取れるので取り違えやすい)。selector を使わない (プロンプト指定の) 実行なら
  `bin/codex-run` がこの差分と出力・rc の保存をまとめて引き受ける
