...
- **複数マシンで同じ repo を触るので、着手する issue は `issues/next/` へ移して claim し、その移動だけを即 push する**（claim は push されて初めて他マシンから見える。着手前に `git fetch` して既に next に居ないかを見る）。詳細は [`claim-issue-in-next-and-push.md`](rules/claim-issue-in-next-and-push.md)。PostToolUse hook（`_claude/hooks/next-claim-push.sh`）が `issues/next/` への移動を検出して push を促す
