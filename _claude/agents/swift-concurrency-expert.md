...
### 6. Actor Reentrancy Diagnosis (Project-Specific)

**What is Actor Reentrancy?**
Actor reentrancy occurs when an actor method suspends (at an `await` point), allowing other calls to the same actor to execute before the original method completes. This can lead to unexpected state changes.
...
actor Counter {
    ...
    // ❌ DANGEROUS: count may change between check and modification
    func incrementIfLessThan(_ limit: Int) async {
        if count < limit {           // (1) Read: count = 5, limit = 10
            await someAsyncWork()    // (2) Await: another call sets count = 15
            count += 1               // (3) Write: count becomes 16, exceeds limit!
        }
    }

    // ✅ SAFE: Modify state before await
    func incrementIfLessThanSafe(_ limit: Int) async -> Bool {
        guard count < limit else { return false }
        count += 1                   // Modify BEFORE await
        await someAsyncWork()        // Now it's safe - state already updated
        return true
    }

    // ✅ ALTERNATIVE: Capture-check-verify pattern
    func incrementWithVerify(_ limit: Int) async -> Bool {
        let originalCount = count    // Capture current state
        guard originalCount < limit else { return false }

        await someAsyncWork()

        // Verify state hasn't changed unexpectedly
        guard count == originalCount else {
            // State changed during await - decide how to handle
            return false
        }
        count += 1
        return true
...
**When to Suspect Reentrancy Bugs**:
| Symptom | Possible Reentrancy Issue |
|---------|--------------------------|
| State appears to "jump" unexpectedly | Another call modified state during await |
| Invariants violated after async ops | State checked before await, modified after |
| Race-like behavior in actor code | Multiple calls interleaving at await points |
| Logic works in sync tests, fails in async | Test doesn't trigger reentrancy |

**Debugging Strategy**:
```swift
actor DebugableActor {
    var state: Int = 0

    func suspiciousMethod() async {
        let beforeAwait = state
        print("[\(Self.self)] Before await: state = \(beforeAwait)")

        await someAsyncWork()

        let afterAwait = state
