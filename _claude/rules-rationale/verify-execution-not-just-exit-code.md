...
## 「隔離環境での失敗」を実装のバグとして確定させかけた (2026-09-08, dotfiles 332/333)

Ruby の LSP が効かない調査で、最初のプローブを `nvim -l script.lua` で書いた。
**`-l` はユーザー設定を読まない**ので `attached=false clients=` が返り、それを
「LSP が attach していない」という事実として一度手元で確定させた。次の一手
(`--headless -c "lua dofile(...)"`) で覆った。

本文が「隔離環境での成功は本番の成功ではない」しか書いていなかったのは、緑の側だけを
警戒していたため。赤は「壊れている証拠」の顔で来るので、ハーネスを疑う契機が生まれにくい。
[`verify-interactive-prompt-with-pty-driver.md`](verify-interactive-prompt-with-pty-driver.md) が
同じ趣旨 (「対話の確認が失敗したら実装を疑う前にハーネスを疑う」) を持っているが、
あちらの発動点は対話プロンプトに限定されていて、headless の検証には掛からなかった。

同じセッションでもう 1 つ: 6 分ポーリングで誤答のままだった run を根拠に
「214 秒待っても必ず誤答」と報告したが、別の run では正答が返った (索引の状態依存)。
本文の「条件を変えずに同じ HEAD で 2 回回す」は**切り分け**の手順として書いてあり、
**症状の恒常性を主張するとき**にも当てはまることが読み取れていなかった。

## 単独 green を集約経路の緑と読みかけた 2 件 (2026-09-06/07, dotfiles の監査修正)

ルール本文の「個別に直接叩いた結果を集約経路の証拠として使わない」は**守った** (どちらも
`make test` で見つけた) が、「単独 green を見た時点で報告しかけた」瞬間が 2 回あった。
どちらも単独では出ない形なので、報告していれば嘘になっていた。

- `zshlib/_av1ify.zsh` の `always` ブロック — 単独の `zsh -n` は通るが、`make test` の
  shellcheck が SC1072 を出す (集約経路にしかその lint が居ない)
- 中断テスト — 単独では緑。`make test` の**並列実行下でだけ** flaky だった
  (孤児になったスタブが `$out` を書き直す)

## 「手動棚卸し target」には退行検出の主体が居ない (2026-09-08, dotfiles 329)

`make test-assert-reaches-exit` が集約経路にも CI にも配線されていないのを見て
「配線漏れ」と書きかけたが、**`Makefile` のコメントが「`test` からは呼ばない」と明記していた**
(各テストを 3 回走らせるので所要が数倍。`test-fresh` と同じ手動棚卸し扱い)。
`test-lint` の直上も「ここが並列でよいのは互いに独立した**静的**検査だけだから」と書いており、
全テストを 3 回実行する動的検査を入れると lint が数十分になる。

つまり配線漏れではなく「**動的検査の置き場所が repo に無い**」。ただし結果として
「誰も自動では回さない = 退行検出力ゼロ」になる点は同じで、§4 (その機構が CI で実際に走るか)
の不履行にあたる。`test-fresh` も同じ形なので、1 target の問題ではなくカテゴリの問題。

🚨 この節自体が実例でもある: 私は `Makefile:478` に無いことは実測したのに、
