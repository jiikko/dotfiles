...
## 「派生どうしが違う値を出すことを確かめる」の起源 (dotfiles retro 368 項目 2, 2026-09-12)

見本スクリプト `a3.lua` / `b.lua` / `a-inline.lua` が `vim.fn.expand("%:p:h")` で兄弟ファイルを
`dofile` していた。これは**スクリプトの dir ではなく開いているバッファの dir** なので、見本を
任意のファイルに当てられるようにした瞬間に外れた。

外れ方が最悪だった: `dofile` がエラーになるとチャンクが中断し、**本体の設定がそのまま残る** →
出力は「本体の値」= 一見もっともらしい。`a3` が `a` と同じ値を出していたので気づいたが、
気づかなければ「案 A3 は案 A と見分けが付かない」と報告していた。

同じ節の canary が *観測が変化を捉えられない* 形を扱うのに対し、これは
**観測装置が黙って無効化され、無効化された結果が正常な出力と区別できない**形。

## A-B の「崩した片腕」が整形で打ち消され、対照が空振りした (dotfiles 410, 2026-09-21)

`_claude/settings.json` のキー順を正規化する SessionStart hook を入れた際、「hook が実際に
発火するか」を確かめるために A-B を組んだ。片腕 (崩した入力) の作り方はこう書いた:

    jq -S 'to_entries | reverse | from_entries' "$S" > tmp && cat tmp > "$S"

`-S` は**出力時にキーをソートし直す**ので `reverse` を打ち消す。ファイルは 1 バイトも変わらず、
`git status` は clean のまま。そのまま headless セッションを起こして「戻っている」を見ても、
入力が最初からソート済みなので**何も確かめていない**。

気づけたのは `git status --porcelain` が想定に反して空だったから。しかも最初の周は崩しに
**揮発キーの追加も混ぜていた**ため、「dirty → clean になった」という*別の理由*で緑が付き、
順序については何も言えていない状態で「発火を確認した」と書きかけた。

直した形: 崩した直後に `keys_unsorted` を読んで逆順になっていることを目視し、`git status` が
`M` を出すことを確認してから headless を起こす。さらに「別の config を見せた hook では直らない」
という**負の対照**を挟んだ (これが無いと「勝手に直った」と区別できない)。

教訓の一般形はルール本文へ。発動点が [`mutation-verify-new-tests.md`](../rules/mutation-verify-new-tests.md)
手順 1.6 (変異の diff を目視する) と同型だが、あちらは**コードへの変異**が対象で、live な A-B の
入力づくりには効かなかった (このセッションは当該ルールを読んだ状態で踏んでいる)。

## ルール本文から移した実例 (2026-09-24 の整理で本文から外したもの)

- **`&&` のつなぎ** (2026-09-23 obaket 909): obaket の push が pre-push の lint gate で落ちたのに、アンブレラの submodule
  参照を bump して push し、数分間 remote に無い commit を指させた
- **stdout / stderr の両方を読む** (2026-09-04): err だけ見て shellcheck 通過と読んだが、指摘も「✗ 失敗したターゲット」も stdout に出ていた
- **`pgrep -f` の自己マッチ** (2026-08-26 実測)。`until ! pgrep -f` の永久待ち (2026-09-08 swift-smbee): 完了済みの
  background を十数ターン待ち続け、ユーザーの指摘で気づいた
- **`&&` と `;`** は retro 386
- **全行 diff**: 96 → 96 ファイルのまま `✓` 行が 68 件消えていた (dotfiles issue 133、本ファイル上部)
- **非対話 CLI** (2026-09-04): `docker builder prune --filter ... </dev/null` が rc=0 + プロンプトだけを stdout に出して終了した。
  `-f` を付けるまで 1 バイトも減らない配線だった。**設定の読み戻し** (2026-09-23 retro 411): Terminal が未導入フォントを代替フォントへ黙って置き換えた
- **集約テストが赤いまま** (2026-09-20): `make test` の staticcheck 行が半日赤いままで、8 issue ぶんの検証をその上で回した。
  原因はガード 1 行のバグ (`command -v` が version manager の shim を掴む) で、着手したら 30 分で直り緑に戻った
- **隔離環境での成功** (2026-09-05 dotfiles 265): 隔離サーバで toast が出るのを確認して「出る」と報告したが、本体は `behind 1` で
  古い版が動いていた。2 往復した。**隔離環境での失敗** (2026-09-08 dotfiles 332): `nvim -l script.lua` はユーザー設定を読まないのに、
  その結果で「LSP が attach していない」を一度事実として確定させた
- **キャッシュされた緑** (2026-09-14, retro 374): 違反を植えても `ok (cached)` rc=0、`-count=1` で FAIL
- **切り分けの変数** (2026-09-04): `TMPDIR` の差し替えで go のビルドキャッシュも空になり、変数が 2 つ動いた。
  抽出方法の違い: `grep -oE '"svce"...'` と素の `grep -c` で件数が 30 と 49 に割れた
- **使い捨ての不在判定** (2026-09-21 obaket 888): 起動中アプリが古いかを `strings <binary> | grep '<新 route>'` の 0 件で判定しかけたが、
  既存 route で試したら全部 0 件で手法自体が無効だった。結論は偶然正しく、そのまま報告していたら正しい結論を誤った根拠で出していた
