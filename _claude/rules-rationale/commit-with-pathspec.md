...
## 推測した帰属を第三者へ伝え、訂正されるまで気づかなかった (2026-09-06, dotfiles)

issue 番号 309 の衝突を調べていて、**timestamp だけを根拠に「309 は dotfiles-71 のもの」と
第三者のセッションへ伝えた**。実際は第 3 のセッションの作業で、訂正された。

本文の「推測した帰属を第三者へ伝えない」に正面から違反しているが、違反している自覚が
無かったのが要点。**帰属の推測は「調べた結果」の顔で出てくる** (timestamp を見た = 根拠がある、
と感じる) ので、伝える直前に「これは commit が触ったファイルか、本人の返答か」を
問い直す以外に止め方が無い。同じセッションで番号の衝突自体も 2 回起きている (309 / 291)。


## ルール本文から移した実例 (2026-09-24 の整理で本文から外したもの)

- **submodule の pathspec** (2026-09-19): 親から submodule の README を commit して空振りした
- **worktree からの merge / push** (2026-09-02〜03 SnapTrim): 2 回踏んだ
- **除外による漏れ** (2026-09-11 obaket 752): 変異検証用の worktree がビルドできず、そこで初めて発覚して amend した
- **時刻は帰属に使えない** (2026-09-02): `23:30 → 23:52 → 23:47` と逆転していた
- **推測した帰属の伝播** (2026-09-04): 4 回外し、誤情報を受けた別セッションがさらに別のセッションへ確認しに行った

## push の再試行のループが失敗した rebase を隠した (dotfiles retro 448, 2026-09-25)
