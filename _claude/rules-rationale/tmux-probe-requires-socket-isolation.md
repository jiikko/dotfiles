...
## commit message をファイル経由で渡す回避の実例 (dotfiles, 2026-09-15)

`deny-bare-tmux-kill.sh` の偽陽性で commit が 1 度止まった。heredoc のコミットメッセージ本文は
引用符に入らないので、「tmux の停止コマンド」を説明した散文がそのまま検査対象になる。

`git commit -F <ファイル>` にすると、本文は**コマンド文字列に現れない**ので偽陽性が起きない。
**検出の迂回ではない**: hook の目的は「実際に本番サーバを撃つコマンド」を止めることで、
そちらは従来どおり検査対象に残る。以後この形を既定にした (本文が長い commit では特に楽)。


## ルール本文から移した実例 (2026-09-24 の整理で本文から外したもの)

- **human に回す前に隔離サーバで測る** (2026-08-27): 「conf を reload したら仕掛けた sleeper が死ぬか」を human に回しかけたが、隔離サーバで 1 分で実測できた (生存する)
- **tmux を立てる必要があるか** (2026-07-30): 測りたかったのは `bin/glogx` ラッパーと Go バイナリ直の差で、tmux を一切使わずに測れた
- **shim が唯一の層** (2026-09-11): 事故はテストスクリプト内部の `-L default` 付きの kill で、文字列を見る hook では捕まえられなかった
