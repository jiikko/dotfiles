...
| キー | 動作 |
|---|---|
| `C-t g` / `C-g` | **git 操作 popup**（`C-g` = Ctrl+g なら prefix 不要）。いま見ているペインの cwd の repo に対して fzf のインクリメンタル操作で status/diff/add/commit（nvim や Claude のペインからでも効く。`Tab`/`Enter` = stage⇄unstage、`C-a` = 全 add、`C-o` = commit、`C-p` = push、`C-d` = diff 全画面、`Esc` = 閉じる。clean 時はサマリ画面になり `p` = push） |
