...
| キー | 動作 |
|---|---|
| `C-t g` / `M-g` | **git 操作 popup**（`M-g` = Option+g なら prefix 不要）。いま見ているペインの cwd の repo に対して fzf のインクリメンタル操作で status/diff/add/commit（nvim や Claude のペインからでも効く。`Tab`/`Enter` = stage⇄unstage、`C-a` = 全 add、`C-o` = commit、`C-d` = diff 全画面、`Esc` = 閉じる） |
