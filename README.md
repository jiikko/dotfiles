...
| `C-t g` | window 跨ぎ | **fzf popup** で選んだ window のアクティブペインをここへ持ってくる (get) |
| `C-t G` | window 跨ぎ | 現在のペインを fzf popup で選んだ window へ送る (give) |
...
`C-t a` で入ると**修飾キーなしの単キー**で移動し放題になるモーダル（copy-mode の移動版）。
歯磨き等の片手運用向け。モード中はステータスバー左端に 🧭 NAV バッジが点滅する。

| キー | 動作 |
|---|---|
| `C-t a` | nav モードに入る |
| `h/j/k/l`（右手） / `w/a/s/d`（左手） | ペイン移動（端でループしない） |
| `n/p`（右手） / `Tab`/`Shift-Tab`（左手） | 次 / 前のウィンドウ |
| `1`〜`9` | ウィンドウ直行 |
| `z` | ズームトグル（モード維持） |
| `f` | fzf window picker（開くとモードは抜ける） |
| `Esc` / `q` | モードから出る |

未バインドのキーは 1 打消費して root に戻るだけなので、入りっぱなしでも誤爆は連鎖しない。
