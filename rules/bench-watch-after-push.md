...
- `trusted=no` の run の数字を「デグレ/改善の根拠」として報告しない。**同じコミットで Bench を
  再実行** (`gh run rerun <run-id>`) し、trusted=yes の run で判定する
- 再実行でも同じ数字が出たら環境ではなくコードなので、下の 1〜3 へ進む
- 起源: run 30683781313 は較正器 ×1.5 の混雑下で `render_large_patch` が換算後も +25% 残り、
  コードに機構が無いのに悪化と読めた。同コミットの再実行で 0.940 (基準 0.950) に戻り flake と確定
