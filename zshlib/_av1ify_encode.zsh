...
# 共有上のパスへ向ける。
#
# 🚨 キーは**絶対パス (symlink 解決済み) を小文字化した文字列の SHA-256**。
#   - basename では足りない: 別ディレクトリの同名ファイルが同じロックになり、
#     関係ないファイル同士が直列化する
#   - 小文字化するのは、APFS が既定で大小を区別しないため。A.mkv と a.mkv は
#     同じファイルなのでキーも同じでなければならない (大小を区別する FS では
#     別ファイルが直列化しうるが、安全側に倒す)
#
...
__av1ify_lock_dir_for() {
  local in="${1:A}"          # 絶対パス + symlink 解決 (./x や //x や 別名を同一視)
  local key="${in:l}"        # 大小を畳む (上のコメント参照)
  local h
  h="$(printf '%s' "$key" | shasum -a 256 2>/dev/null | cut -d' ' -f1)"
  [[ -n "$h" ]] || return 1
...
__av1ify_lock_acquire() {
  ...
  if ! whence -p lockman >/dev/null; then
    # 今日までと同じ「排他なし」の挙動。黙って進めない — 何が守られていないかを出す。
    if (( ! __AV1IFY_LOCK_WARNED )); then
      __AV1IFY_LOCK_WARNED=1
      print -ru2 -- "${_C_YELLOW}⚠️ lockman が PATH にありません。同じファイルへの二重実行を防げません${_C_OFF}"
    fi
    return 0
  fi

  if ! __av1ify_lock_dir_for "$in"; then
    print -ru2 -- "${_C_YELLOW}⚠️ ロックキーを作れません (shasum 不在?)。排他なしで続行${_C_OFF}"
    return 0
  fi
  local dir="$REPLY"
  if ! mkdir -p -- "$dir" 2>/dev/null; then
    print -ru2 -- "${_C_YELLOW}⚠️ ロックディレクトリを作れません: $dir (排他なしで続行)${_C_OFF}"
    return 0
  fi
...
__av1ify_one() {
  ...
  if ffmpeg "${args_common[@]}" "${args_audio[@]}" -- "$tmp"; then
    # 🚨 finalize は元ファイルをゴミ箱へ移す。ここまでに lease を失っていたら
    # (自動更新の失敗・TTL 超過)、他プロセスが同じファイルを引き継いでいる可能性が
    # あるので、**元ファイルには触らずに** 失敗で抜ける。
    if ! __av1ify_lock_still_held; then
      print -ru2 -- "${_C_RED}🚨 排他を失いました。元ファイルは保持します: $in${_C_OFF}"
      [[ -e "$tmp" ]] && rm -f -- "$tmp"
      __AV1IFY_CURRENT_TMP=""
      REPLY="$in"
      return 1
