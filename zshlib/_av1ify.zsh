...
#     `bin/binav1c foo/*.mkv(N)` のようにマッチ 0 件で引数が消えた呼び出しが
#     「クリップボードの中身を処理するか?」の確認に化ける (実測 2026-08-22)
#     🚨 対話シェルの `av1c` は _zshrc のラッパー経由で関数として動くので、このゲートは
#     通る (ユーザーの明示要求)。防波堤は av1c() のコメントに書いた 4 点だけになる。
#   -t 0 … リダイレクト・パイプ・CI では読まない (確認を取る相手がいない)
#
# 🚨 interactive 側は自動テストから真にできない (setopt interactive は実行時に変更不能で、
...
av1ify() {
  ...
  if (( ! __av1ify_internal )) && (( ! show_help )) && (( have_targets || use_clipboard )); then
    ...
    if [[ -n "${AV1_AUDIO_REENCODE_MARGIN:-}" ]] \
      ...
      print -r -- "エラー: 無効なAV1_AUDIO_REENCODE_MARGIN指定: ${AV1_AUDIO_REENCODE_MARGIN}（0より大きい10進数で指定してください。例: 2.0）" >&2
      return 1
    fi
  fi
...
機能:
  ...
  - 音声は「MP4へcopyできるか」「再エンコードで実際に削れるか」の2段で判定します。
    ...
    2. copyできるコーデック (既定でAAC, ALAC, MP3) でも、ソースのビットレートが
       「ターゲット × AV1_AUDIO_REENCODE_MARGIN (既定2.0)」を超えるなら再エンコードして圧縮します。
...
環境変数による設定:
  ...
  AV1_AUDIO_REENCODE_MARGIN (デフォルト: 2.0)
    ...
    (例: 既定では 96k × 2.0 = 192000bps 超のソースが再エンコード対象)
  ...
  AV1IFY_DENSITY_TOLERANCE_PCT (デフォルト: 5) / AV1IFY_DENSITY_FLOOR (デフォルト: 24)
    ...
    この 2 つは「ソースの nb_frames を期待値として信用するか」の判定にも
    (その半分の幅で) 使われるため、緩めると警告が出にくくなるだけでなく、
    尺と矛盾する nb_frames も採用されるようになります。
  ...
  AV1IFY_VIDEO_LOSS_TOLERANCE (デフォルト: 2.0)
    変換前後の「映像ストリームだけ」の尺の差がこの値[秒]以下であれば警告しません。
    音声の状態に関係なく判定するため、音声は満尺のまま映像だけが途中で打ち切られた
    (エンコード中のプロセス異常終了など) ケースを検知します (avsync/duration は
    音声が満尺だとこの種の欠落を見逃すことがあります)。超過時は vidloss タグが
    ファイル名に付与され、必ず再エンコードが必要な破損として扱われます。
...
#
# プリセットの中身はここだけに置く。bin/binav1c (Finder アクション用) と _zshrc の
# lazy-reload ラッパー (対話シェル用) の両方がこの関数を呼ぶ。フラグを両方に書くと
# 片方だけ変わる。
#
# 🚨 対話シェルで引数なしの av1c を打つとクリップボード入力が発火する。
