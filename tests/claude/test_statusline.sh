...
mkdir -p "$gnu_dir"
real_date="$(command -v date)"   # 🚨 PATH を汚す前に実体を絶対パスで解決する
case "$real_date" in
  ""|"$gnu_dir"/*)
...
# 🚨 five_hour を必ず入れる: epoch → 日時の変換 (fmt_epoch) を使うのは 2 行目の
#   rate_segment だけで、7d はペース行へ移ったため 7d だけの fixture ではこの経路を
#   1 度も通らない (テストが空振りする)。
gnu_json_body() {
  local now; now="$(date +%s)"
  # 🚨 境界値を使わない。now をテスト側と statusline 側で別々に取るため 1〜2 秒ずれる。
...
# 🚨 JSON の数値リテラル (5e1 等) は使わない: jq 1.6 は 50 に正規化し、jq 1.7 は
...
# 🚨 残り時間ラベル (残N日N時間 / 残N時間N分) を assert する fixture は、表示される
...
# 🚨 ただし**余白で回避できるのは「丸めの表示」だけ**。分岐の境界そのものを見るテストは
#   余白では直らないので、date shim + TZ 固定で **now を固定する**こと。
#   実例 (実測 2026-08-23): カレンダー格子のセル数は「窓の開始がローカル 0 時に揃うか」で
#   7 と 8 に変わる。`now + N` の fixture では、テストが now を取る瞬間と statusline が
#   取る瞬間の 1 秒差でセル数が入れ替わり、連続実行で成功と失敗が両方出た。
#   判定対象が境界そのものなら、余白をいくら積んでも境界は消えない。
...
# 🚨 ゲージ自体は出し続ける。以前は 2 行目の旧セグメント形式へフォールバックしていたが、
...
# 窓が終わっているので、塗った先は全部「使い残し」= 明るい青。実績 62% は 9 カラム目まで
# なので 6 番は明るい青になる
assert_contains "$(pace_raw 62 0)" $'\033[94m'"6" "リセット済みの未使用分は使い残し (明るい青)"
assert_lacks "$(pace_raw 62 0)" $'\033[4;1m' "リセット済みには現在位置の下線を引かない"
# 🚨 窓が終わっているので、乖離が想定帯の中に入っていても使い残しは出す。実績 91% は
#   13 カラム目まで塗られ、最後の 1 カラムが使い残しになる (帯の判定をすると消える)
assert_contains "$(pace_raw 91 0)" $'\033[94m'" " "リセット済みは帯に関係なく使い残しを出す"
...
# 色: 状態ごとに色が変わること (想定通り=緑 / 先行=黄 / 超過=赤 / 余裕=明るい青 /
...
assert_contains "$(pace_raw 17 "$(day 5)")" $'\033[94m 17%' "余裕は明るい青"
...
assert_contains "$(pace_raw 17 "$(day 5)")" $'\033[94m'"残" "余裕の残り時間・アドバイスは明るい青"
...
# 塗り: 想定内の消化は緑背景、前借りは赤背景、使い残しは明るい青。位置が used% と残り秒
# だけで決まるので、期待するカラムの文字まで指定して固定できる。
# 🚨 色だけを見る assert にしないこと。明るい青 (94m) は「余裕」の状態色にも使われるので、
...
assert_contains "$(pace_raw 10 "$(day 5)")" $'\033[94m'"2" "使い残しは明るい青で出る"
...
# 🚨 実績 100% / 想定 99% は乖離が +1pt しかなく **帯の中**なので、バーに赤は出ない
...
assert_lacks    "$(pace_raw 30 "$(day 5)")" $'\033[94m'"3" "帯の中では使い残しの明るい青を出さない"
...
assert_contains "$(pace_raw 3 514080)" $'\033[94m\033[4;1m'"2" \
...
# 🚨 想定帯は 7d と別で ±25pt。5 時間窓は本質的にバースト的で「1 時間目に 40% 使った」
#   = +20pt が常態になるため、±10pt では赤が出続けて信号にならない。
# 🚨 7200 (= ちょうど 2 時間) は境界値なので使わない (1 秒ずれると "1時間59分" になる)
...
if command -v go >/dev/null 2>&1; then
  ...
  # 🚨 skip は「合格」ではない。理由を出す (go 未導入の環境でだけ起きる)
