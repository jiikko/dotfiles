...
#
# 🚨 一本化しただけでは守られない。着手時の変異検証で、以下がすべて **green のまま通った**:
...
# 🚨 ここが緩むと、再起動を跨いで残った lock の pid が無関係な長命プロセスに再利用された
...
# 🚨 ここが緩むと、指紋の書式を変えた移行期に**生存 owner の lock を奪う** (誤奪)。
...
# 🚨 ここが本テストで最も重要。`ps -o lstart=` は **macOS が末尾を空白で埋め、Linux は埋めない**。
