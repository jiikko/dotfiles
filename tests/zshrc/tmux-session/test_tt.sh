...
OUT="$(HOME="$TMP_HOME" zsh -c '
  ...
  tmux() {
    case "$1" in
      ...
      show-options) # GC の adopted 判定用: -t =<name> @tt-adopted を _T_GC_ADOPTED_<name> から返す
        # =<name> 引数の検出は `case =*)` を使わない (zsh の EQUALS 展開で = 始まりに
        # マッチせず soname が空になる)。= を剥がして変化したら「= 始まり」とみなす。
        local _soa _soname="" _sov
        for _soa in "$@"; do [ "${_soa#=}" != "$_soa" ] && _soname="${_soa#=}"; done
        eval "_sov=\"\${_T_GC_ADOPTED_${_soname}:-}\""
        [ -n "$_sov" ] && print -r -- "$_sov" ;;
  ...
  source "'"$ZSH_LIB"'"
  reset_log() { _LOG_NEW=""; _LOG_NEWWIN=""; _LOG_SELECT=""; _LOG_KILL=""; _LOG_ATTACH=""; _LOG_T=""; _LOG_FLASH=""; }
...
proj 3 1"
  ...
  # (保護) 死 pid + 非attach + pristine でも @tt-adopted が立っていれば触らない。
  # rc=1/2 でユーザーが adopt して作業→端末を閉じた live hold を GC が誤 kill する回帰の防止。
  reset_log
  _T_GC_SESSIONS="__tt_hold_${DEADPID} 1 0"
  eval "_T_GC_ADOPTED___tt_hold_${DEADPID}=1"
  _tt_gc_stale_holds
  print "CASE:gc_adopted kill=[$_LOG_KILL]"
  eval "unset _T_GC_ADOPTED___tt_hold_${DEADPID}"
...
assert_eq_line  gc_adopted   "kill=[]"           "@tt-adopted の hold は死pid+pristineでも触らない (adopt 済み live 保護)"
