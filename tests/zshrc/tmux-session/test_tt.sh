...
OUT="$(HOME="$TMP_HOME" zsh -c '
  ...
  tmux() {
    case "$1" in
      has-session)
        if [[ "$2" == "-t" ]]; then
          [[ " $_T_SESSIONS " == *" $3 "* ]] && return 0 || return 1
        fi
...
printf '\n## _tt_impl の rc → 分岐\n'
assert_eq_line  srv_exist   "new=[] attach=[ proj] t=[]"  "既存サーバ+目的有: hold作らず目的にattach、新規作成しない"
...
assert_line_has rc0_exist   "attach=[ proj] t=[]"        "rc=0+目的有: 目的にattach、新規作成しない"
...
assert_eq_line  rc1_exist   "kill=[] t=[] attach=[ proj]" "rc=1+目的有: holdを畳まず・新規作成せず、目的にattach"
assert_line_has rc2_missing "t=[] attach=[ __tt_hold_"   "rc=2+目的無: 新規作成せず hold に attach"
