...
OUT="$(
  ...
  bash -c '
    ...
    : > "'"$TMP_HOME"'/save_runs"
    noexec_save="'"$TMP_HOME"'/noexec_save.sh"
    printf "#!/usr/bin/env bash\n" > "$noexec_save"   # 実行ビット無し
    _T_SAVE_SCRIPT="$noexec_save"                     # パスは設定済みだが実行不可 → 実行しない
    if tt_run_resurrect_save; then rc=0; else rc=1; fi
    printf "CASE:save_noexec rc=%s runs=%s\n" "$rc" "$(wc -l < "'"$TMP_HOME"'/save_runs" | tr -d " ")"
...
assert_eq_line save_noexec   "rc=1 runs=0"   "保存スクリプトが実行不可 (実行ビット無し) でも実行せず失敗を返す"
