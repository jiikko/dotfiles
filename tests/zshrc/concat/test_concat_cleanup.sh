...
# concat デフォルト削除動作 & --keep オプションのテスト
#
# 仕様:
#   - ファイル指定モード: デフォルトで結合成功後に元ファイルを削除
#   - ディレクトリモード: デフォルトで結合成功後に元ファイルを削除
#   - マルチグループモード: デフォルトで結合成功後に元ファイルを削除
#   - --keep 指定時: 元ファイルを残す
#   - --dryrun 指定時: 元ファイルを削除しない（--keep の有無に関わらず）
#   - 既存出力でスキップされた場合: 削除しない
...
# ============================================================
# Test 1: ファイル指定モード（デフォルト）で元ファイルが削除される
# ============================================================
printf '## Test 1: File-spec mode deletes source files by default\n'
...
assert_file_not_exists "$TEST_DIR/clip_001.mp4" "Source file 1 deleted by default"
assert_file_not_exists "$TEST_DIR/clip_002.mp4" "Source file 2 deleted by default"
assert_contains "$output" "元ファイルを削除中" "Output mentions deletion"
assert_contains "$output" "削除: clip_001.mp4" "Output confirms file 1 deletion"
assert_contains "$output" "削除: clip_002.mp4" "Output confirms file 2 deletion"
...
# 削除ログが出ていないこと
if [[ "$output" == *"元ファイルを削除中"* ]]; then
  printf '✗ --keep should not print deletion message\n'
  return 1
else
  printf '✓ No deletion message with --keep\n'
fi

# ============================================================
# Test 3: ディレクトリモードのデフォルト削除（既存挙動の維持）
# ============================================================
printf '\n## Test 3: Directory mode deletes by default (unchanged)\n'
...
assert_file_not_exists "$TEST_DIR/show_001.mp4" "Source file 1 deleted in directory mode"
assert_file_not_exists "$TEST_DIR/show_002.mp4" "Source file 2 deleted in directory mode"
...
# ============================================================
# Test 5: --dryrun は削除しない（--keep なしでも）
# ============================================================
printf '\n## Test 5: --dryrun does not delete source files\n'
...
assert_file_exists "$TEST_DIR/demo_001.mp4" "Source file 1 not deleted in dryrun"
assert_file_exists "$TEST_DIR/demo_002.mp4" "Source file 2 not deleted in dryrun"

# ============================================================
# Test 6: マルチグループモードで各グループの元ファイルが削除される
# ============================================================
printf '\n## Test 6: Multi-group mode deletes source files for all groups\n'
...
assert_file_not_exists "$TEST_DIR/alpha_001.mp4" "alpha_001 deleted"
assert_file_not_exists "$TEST_DIR/alpha_002.mp4" "alpha_002 deleted"
assert_file_not_exists "$TEST_DIR/beta_001.mp4" "beta_001 deleted"
assert_file_not_exists "$TEST_DIR/beta_002.mp4" "beta_002 deleted"
...
# ============================================================
# Test 8: 既存出力でスキップされた場合は削除しない
# ============================================================
printf '\n## Test 8: Skip case (existing output) does not delete source files\n'
...
# ============================================================
# Test 9: 結合失敗時は元ファイルを削除しない
# ============================================================
printf '\n## Test 9: Failed concat does not delete source files\n'
