...
- [ ] 項目 1 の切り出し (ルールに 1 行 or 却下) — 未着手。issue 163 の「走らせ方」には書いたが、
      `subagent-model-tiering.md` に一般化するかは未決 (2026-09-02 に 6 体並行で全滅した実例が増えたので、
      切り出す材料は揃っている)
- [x] 項目 2 の切り出し → `commit-with-pathspec.md` に節を追加 (実測つき)
- [x] 項目 5 の切り出し → `mutation-verify-new-tests.md` に 1 項追加
