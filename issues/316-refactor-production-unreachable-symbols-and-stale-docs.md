...
## 進捗 (2026-09-08) — 1/13 対応済み。残りは継続

commit `fix(316): DeleteReport.HasFailures を結果画面へ配線する`。

### ✅ `DeleteReport.HasFailures` — **配線漏れだった**（issue の「優先」判定は正しかった）

削除の結果画面は各行に `❌ できず` / `🚨 未完了` を出しているが、**末尾の合計行は
「解放しました: N」しか語らない**（`doctor_delete.go` の `deleteTotalsLine`）。
`HasFailures` の doc が言う「`err == nil` だけを見る呼び出し元が『全部消えた』と読み違えない
ための口」がまさにこの合計行で、そこから呼ばれていなかった。

→ 合計行の直後に「🚨 消せなかった / 未完了の項目があります (上の ❌ / 🚨 の行)」を出す。
**変異検証**: 呼び出しを外すと既存の結果画面テストが red（`🚨 消せなかった…` が無い）。

### 残り 12 行は未着手

判断が要るもの（`anchorCursor` の配線漏れ / 将来用の判定、`ime_tis_stub.go` を消すか lane を作るか、
`tt_capture_contents_on` を削除するか `guards.sh` へ寄せるか）と、
issue 317 と束ねるもの（`RenderLine` / `RenderTable` / `Window.Raw`）が混ざっている。
**一括削除の issue ではない**という冒頭の注意どおり、1 件ずつ判断が要るので分割して進める。
