...
- カテゴリ: audit
- 起票: 2026-09-02
- 状態: **open (部分実施。体 1/2/4/5/6 は完了、体 3 は未走行。フルは次の枠で通す)**
...
### ⚠️ この audit はまだ全部を通していない (2026-09-02 時点)

**今回やったのは 6 体のうち 5 体分で、しかもその 5 体も途中終了した残骸から拾ったもの。** この issue は open のまま残す。
...
- **次の 5h 枠が開いた直後に、体 3 を直列で走らせる**。並行させない (2026-09-02 に 6 体並行で全滅した)
- そのあと、体 4 の未計測項目 (実機の fd / プロセスグループ観察、起動時間 before/after) を埋める
- 起票済みの issue 167-183 の**反証レビューは P1 2 件 (167 / 178) にしか通していない**。P2 の主要なもの
  (170 / 171 / 175 / 176 / 177) は未反証。トークン消費が重いので、着手時にその issue だけ反証する運用に切り替える
- この issue を `done/` へ送れるのは、上の表の全体が「完了」になり、かつ受け入れ条件を全部満たしたとき
