...
## 保留 (pending へ。2026-09-04。ユーザー判断)

**claim だけして着手しなかった**ので、next から pending へ戻す。理由は規模:
本文自身が「実装は地味に重い」と書いており、UI 案 (`--per-host N` / `host=N` の併用、
URL として parse できない行の扱い、scheme / port 違いの統合) は具体的だが、
**着手すると 1 セッションでは終わらない**。中途半端に始めて next に寝かせると、
他マシンから「着手中」に見えたまま止まるので戻す。

### 再開の trigger

**bulk download で「一番弱いホストに合わせて global `-P` を絞る」場面に実際に当たったとき**
(本文の「モチベーション」節がその状況を書いている)。そのとき、当たったホストと絞った値を
この issue に足してから着手すると、既定値の設計が実測で決まる。

### 着手前に決めること (本文の UI 案から未決のまま残っているもの)

- `--per-host` の値と global `-P` の関係 (min を取るか、per-host が上限として独立するか)
- URL として parse できない行を global pool に混ぜたとき、per-host の枠を消費するか
