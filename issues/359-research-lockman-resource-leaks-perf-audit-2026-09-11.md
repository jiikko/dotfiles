...
- 却下 1 の再開 trigger の確認コマンドを、定期的に見る仕組みへ載せるかは未決 (今は本文のみ)
- 付随して見つけた README の乖離の切り出し (ユーザー判断待ち)
- 「軽微だが実在する」節の 1 件目 (`--on-lost` の値検証) は 356 / 357 の実装時に一緒に直す
- **反証レビューは 1 周のみ。** `_claude/rules/adversarial-review-own-safeguards.md` §7 は
  「指摘を直したら直した差分にもう 1 周回す」を求めるが、本件の修正は**issue 本文の
  書き換えだけでコードを 1 行も変えていない**ため 2 周目は回していない。
  356 / 357 / 358 の**実装**に入るときは、その差分に対して改めて敵対的レビューが要る
