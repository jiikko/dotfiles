...
📌 **2026-09-15: 356 / 357 は実装完了**（項目 4 の trigger も発火して対応済み）。残るのは
`--on-lost` の値検証 1 件だけ。
...
✅ **trigger が発火し、対応した (2026-09-15 / 357 の実装時)**。
`with` の `Renew` を包んだので、この節が予告したとおり tick ごとに goroutine が積む経路が
実際にできた。選んだのは**上限を置く**側: **最初の更新失敗で ticker を止める**
(`outcome` は sticky なので、それ以降の更新は結論を変えず、積む理由が無い)。
1 回の `with` で見捨てる goroutine は**最大 1 本**に収まる。
`TestRenewDoesNotPileUpGoroutinesWhenBlocked` が本数で判定し (時間ではない)、
`ticker.Stop()` を外す変異で RED を確認した (上限なし版はこの条件で 10 本以上積む)。
**この節の trigger を書き残していたことが、実装側の見落としを実際に止めた。**
...
(OK / lease 喪失 = 122 / 判定不能 = 125) に分けた。この節が指摘した
「一過性のヒカップ 1 回で rc=122」は、125 を返すようになった。
「テスト 0 件」も埋めた (経路 2 と `--io-timeout` の両方)。
...
- [ ] 「軽微だが実在する」節の 1 件目 (`--on-lost` の値検証) — **356 / 357 の実装では直していない**
      (2026-09-15)。あの 2 件の受け入れ条件に含まれておらず、まとめて入れると
      「この commit は何を主張しているか」が曖昧になるため独立させた。**この issue の
      done 条件はこれ 1 件のまま残る**
