...
|---|---|
| 子を回収済み (グループが空) | **ESRCH** (空振り) |
| 回収前 (**ゾンビだけ**が残る) | **EPERM** (空振り。macOS ではゾンビもメンバーとして残るので ESRCH にならない) |
| **生きたメンバーが 1 つでもある** | **成功** (届く) |

本文は「通常は ESRCH」と書いていたが、**回収前はむしろ EPERM** で、どちらも空振り。
つまり実害には「窓の中で pid が再利用され、**かつ再利用した側がそのグループのリーダー**」が要る
= 本文の見立てより**さらに狭い**。

**推測で防御を足さない**規約どおり、コードは変えずに**受容**する。代わりに
`_ = killGroup(...)` の 2 箇所 (`with.go` のシグナル転送 / `escalateGroupKill` の SIGKILL) に
**エラーを捨てている理由**を書いた (本文が指摘していた「転送の失敗自体が無音」は、
理由が書かれていないことが問題だったので、そこを埋める)。

再開の trigger は本文どおり据え置き。
...
- [x] 3 は**受容**。384 の実測で「失敗はすべて空振り (ESRCH / EPERM)、実害には pid 再利用
      **かつ**リーダー一致が要る」と分かったので、防御は足さずエラーを捨てる**理由**を
      コードに書いた。再開の trigger は本文に記載
