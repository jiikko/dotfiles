...
- [ ] `glog`実行直後にコミット履歴が表示される
- [ ] GitHub API取得後にCI状態が非同期で更新される
- [ ] Alt Screenを使用せず、最終表示がターミナル履歴へ残る
- [ ] `-n 1`、`-n1`、`--max-count=1`が動作する
- [ ] `--stat`が動作し、コミットヘッダーにだけCI状態が付く
- [ ] `-p` / `--patch`が動作し、patch本文を壊さない
- [ ] `--cached`がwrapper独自モードとして動作する
- [ ] 未対応引数を黙って無視しない
- [ ] `git log`の全引数対応を要件にしない旨がヘルプとREADMEに明記される
- [ ] GitHub API失敗時もGit履歴を表示できる
- [ ] stdoutが非TTYの場合、動的描画用ANSI制御を出さない
- [ ] GraphQLで表示対象コミットを一括取得する
- [ ] キャッシュとAPI取得がテスト可能な境界に分離されている
