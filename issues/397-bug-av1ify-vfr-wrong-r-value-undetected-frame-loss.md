...
- [ ] `__av1ify_frac_fps` / `__av1ify_detect_vfr` の awk に `LC_ALL=C` を付ける
- [ ] `__av1ify_frac_fps` の分母 0 は「変換不能 = 空」に倒す (`24/0` を `24.000` にしない)
- [ ] `-r` に丸めた 10 進でなく **`avg_frame_rate` の生の有理数** (`30000/1001`) を渡す (d と、
      NTSC が `2997/100` という非標準レートになる問題が同時に解消する)
- [ ] `--fps N` 指定時のキャップ判定が VFR ソースで迂回されないようにする (c)
- [ ] `_fps_changed` による**全か無かの抑制をやめ**、VFR 検出時は
      **期待フレーム数 = round(src_duration × avg_fps)** との密度比較に差し替える
      (最低でも「Δ が src の 50% を超えたら VFR でも NG」の上限ガード)
- [ ] e2e に**値の pin** を足す (`出力の r_frame_rate == ソースの avg_frame_rate`)。
      `-r 10` / `-r` 削除の変異で red になることを確認する
- [ ] 上記すべてを変異検証し、**ケース名ごとの pass/fail** で判定する ([issue 399](399-chore-av1ify-test-harness-detection-power.md))
