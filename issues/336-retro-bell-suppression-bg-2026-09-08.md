...
- **`make test` が 3 件失敗する (本セッションの変更とは無関係。base commit 187ab4d0 でも同じく失敗)**:
  - `make test-shellcheck`: `zshlib/_av1ify_encode.zsh:695` の `} </dev/null >/dev/null 2>&1 &!`
    を shellcheck が解析できず SC1035/SC1072/SC1073。`&!` は zsh 固有 (disown 付き background)。
    av1ify の lockman 作業 (187ab4d0 / 07e53917) を進めている側の担当
  - `tests/zshrc/av1ify/test_av1ify_clipboard.sh`: 「Splits on a non-breaking space」が失敗。同上
  - `tests/claude/test_dangling_symlinks.sh`: `~/.config/nvim/init.vim*.pre-dein-vim` の
    dangling symlink 4 件。環境の残骸で `./setup.sh` の再実行で消える (repo の変更ではない)
- **人の動作確認**: 実際に bg タスクを走らせた状態で入力待ちにしてベルが出ないことは、
  hook 経由でしか再現できないので未確認 (テストは隔離 tmux サーバでの unit 検証まで)。
  必要なら human issue を切る
