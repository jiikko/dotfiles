...
## この記録 issue を done にする条件

**派生 issue（310 / 311 / 312 / 313 / 314 / 315 / 316 / 317）がすべて片付いたとき**。

- 314 は解消（2026-09-09）。サンドボックス自己テストが拒否理由を sentinel で区別するようになり、
  「無関係な Fatal を拒否に畳む」変異が修正前は緑・修正後は red になることを実測した。
記録そのものは着手対象ではないが、**却下理由と「監査自身の全数勘定が破れていた」申し送りを
次の監査へ渡すのがこのファイルの役目**なので、派生が残っているうちに `done/` へ送ると
参照が潜って見つけにくくなる（324 と同じ扱い）。

2026-09-08 時点で派生 8 件はすべて open（このセッションで順に着手中）。

2026-09-09: **310 / 311 / 312 / 316 / 317 が done**。
- 310: hook のトリガを「語の一致」の grep からトークン解析へ寄せ、出典行と untrusted
  ヘッダを足した（回帰テスト 16 件 + 変異 3 本）
- 312: `lockman.Renew` の非対称を解消（`Release` と同じ `expired` / `holderTTL` を配線し、
  mode 定数に脅威モデルを書いた）
