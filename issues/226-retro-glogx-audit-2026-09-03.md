...
- [ ] 項目 1〜3 の切り出し (既存ルールへの追記) を実行するかの判断
- [ ] issue 222 の残件: 未導入 linter (production 6 / テスト 8) の導入、語彙の一元化と突合テスト、
      `src/doctor/Makefile` のコメント 1 行
- [ ] issue 219 (doctor キャッシュを `writeAtomic` へ寄せる) の着手
- [ ] CI で exhaustive が実際に退行を止めるかの確認は**未実施**
      (ローカルでは変異検証済み。CI 上では「lint が走って 0 issues」までしか見ていない)
