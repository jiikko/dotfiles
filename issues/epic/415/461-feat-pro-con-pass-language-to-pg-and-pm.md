...
- 2026-09-25 23:15 の観測: 461 を含む dispatcher (22:07 に入れ替え) が起こした C-021 (478) の PG の報告が英語だった。PG の claude のプロセスのコマンドラインに
  `--settings` は見つからなかった (bg の session は daemon が起こすので、渡した引数が ps に出ない可能性もある。確かめられていない)。
  C-013 の実測 (haiku・英語の質問に日本語で返した) と合わない。次の確かめ: PG の transcript の最初の system の設定に language が入っているか / 既定のモデルで効くか
- 2026-09-26 の実測 (session dotfiles-4c。直近 12 時間の PG の transcript 72 本): 応答の文 1,984 件のうち日本語 1,852 件 (93%)。英語は作業の途中の
  短い一言 (「Now write recover.go.」等) がほとんど。**各 session の最後の報告が英語だったのは 72 本中 3 本** (C-034 1 本・C-041 2 本。ほかに C-045 の要約の
  ような 2 本)。3 本ともレビューの差し戻し・merge の衝突の直しで再開された session だった。再開のときの渡し方の違いを疑うが未確認

## 決着 (2026-09-26)

- ユーザーの判断で done。本物の dispatcher で報告が日本語になるかの確認と、再開した session で英語になる件 (上の 3 本) は、dogfooding で見つけたら 440 に書く
