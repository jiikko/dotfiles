...
- [ ] 実端末でカクつきが減ったか (ドッグフーディングで確認。残るなら「見立てと未確認」の手順で本番のコマの時刻と所要をログに出す)
  - 観測の口を足した (commit「pro-con: 演出のコマの時刻と所要を、印があるあいだだけ記録する (494)」。`ui/framelog.go`、使い方は README の
    「演出のカクつきを観測する」)。置き場に `framelog.on` があるあいだ、届いたメッセージの時刻・種類・Update の所要と描画の所要を `framelog.tsv` へ追記する。
    テスト `TestFrameLogRecordsOnlyWhileMarked` (変異: 印を無視する / 印を消しても止めない → red)
- [x] 2: 行の作り直しを減らす (2026-09-26。commit「pro-con: 揺れの帯を 1 行 1 回の差し替えで重ねる (494)」):
  測り直すと、揺れのコマの `render` の 7 割が `overlayBump` だった (ヘッダの上に乗せるために画面全体へ重ねる形にしたとき、1 行ごとに
  `fit` と `splice` を 5 回ずつ行っていた)。元の帯の位置と移り先を合わせた範囲に空白と帯を並べ、1 行 1 回の差し替えにした (揺れの届かない行は触らない)。
  実データの写し (200×50) の揺れの 1 コマ: 2.1 ms / 0.70 MB / 4,080 allocs → 1.4 ms / 0.50 MB / 3,622 allocs。
  見た目が変わらないことは、4 方向 × 幅 3 通り (120 / 200 / 83。83 はボードが画面からはみ出して左右の端で切れる) × 盤面 2 通り × 32 コマの画面を
  書き換えの前後で文字ごとの見た目 (文字・色・太字・下線・反転) に直して突き合わせて確かめた (使い捨ての比べ方。バイトでは冗長な色の指定が減った 4,764 行だけ違う。
  この比べ方は途中で右端の「…」が消えた違いを検出している)。
  ほかの演出の 1 コマ (同じ条件): カードの移動 0.64 ms / 0.32 MB、選択の枠が滑る 0.27 ms / 0.13 MB。
  見送り: `splice` / `fit` を 1 回の走査にする (色の状態の持ち越しを自前で再現することになり、壊す危険の割に 1 コマ 1 ms 未満しか減らない)。
  再評価の trigger: 実端末でまだカクつく / 揺れの 1 コマが 5 ms を超える
