...
- [x] 実端末でカクつきが減ったか (ドッグフーディングで確認。残るなら「見立てと未確認」の手順で本番のコマの時刻と所要をログに出す)
  - 実測 (2026-09-26 18:43:45〜18:44:35、ユーザーが新版で端のバウンスとカードの移動を約 50 秒。画面 325×75、作業中の PG あり。`framelog.tsv` 2,062 行):
    演出のコマ (`frameMsg`) の間隔は 379 回のうち 374 回が 30〜40 ms、40〜50 ms が 4 回、最大 58 ms が 1 回 (その間に 7.1 ms の描画が 1 回)。ループが詰まって
    コマが飛ぶ形は出ていない。描画 (View) の所要は演出のコマの直後が p50 2.3 ms / p90 5.9 ms / 最大 9.3 ms、ほかのメッセージの直後が p50 1.1 ms / p90 2.2 ms
    (benchmark の 200×50 より画面が約 2.4 倍広い分重い)。Update はどの種類も p99 3 ms 以内。ユーザーの感想は「カクカクは減ったかも」
  - 測っていないもの: 端末・tmux が描き終えるまでの時間 (bubbletea の書き出しの後)。上下のバウンスが 1 行ずつ動いて見えるのは性能ではなく丸めの段数 (436)
  - 再開の trigger: まだカクつくと感じたら、README の「演出のカクつきを観測する」の手順でもう一度取り、`frameMsg` の間隔が 50 ms を超える所の前後を見る
