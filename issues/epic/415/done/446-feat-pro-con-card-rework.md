...
### 2026-09-25 (PM のレビューと取り込み)

- PM が diff とテストの係の記録を読んで master へ取り込んだ (cf67cf7e / 4f04cea9)。カード C-004 は完了
- ✓ **実機で確かめた** (2026-09-25 19:06): 本物の dispatcher (7c2ca201 を含むバイナリ) で、レビュー待ちの C-005 (447) を
  `pro-con card rework C-005 "<直してほしい点>"` で差し戻した。履歴に「差し戻した: <原文>」が切り詰めずに残り、同じ秒に
  「PG を再開した (session 9c548fad)」(同じ会話の続き) が付いた
- スコープ外として閉じる: 画面 (TUI) から差し戻すキー。PM は CLI で差し戻すので今は要らない。人間が画面から差し戻したくなったら起票する
