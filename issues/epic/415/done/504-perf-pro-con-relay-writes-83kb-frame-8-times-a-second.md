# 504 (perf): 画面の中継が、見る人がいなくても 83KB のフレームを毎秒約 8 回書き直す

起票日: 2026-09-26

## 概要

画面の中継 (`relay/relay.go` の `Writer.loop`。443 で導入) は、描き直しの中身が変わるたびに最短 100ms ごとに `<id>.frame` を書き直す。
「中身が同じ描き直しは書かない」(`sameContent`) で抑えるつもりだったが、作業中の PG が居るとスピナー (`ui/spinner.go` の `spinInterval` = 100ms) が
毎回画面の中身を変えるので、その抑えが効かない。読むのは `pro-con screen` / `pro-con ps` を人やエージェントが叩いたときだけ。

## 実測 (2026-09-26、作業中の PG 1 体・画面 1 つ)

| 何を | 値 |
|---|---|
| 5 秒間に書き直された回数 (`stat -f %Fm` を 20ms ごとに見た) | 42 回 (約 8 回/秒) |
| 1 枚の大きさ | 83 KB (`ansi` 40 KB + `plain` 21 KB を JSON に) |
| 1 回の書き出し (`ansi.Strip` + `json.Marshal` + `writeAtomic`、本物の 1 枚で benchmark) | 0.53 ms / 204 KB / 33 allocs |
| 合計 | 毎秒 約 4 ms の CPU・1.6 MB の確保・700 KB の書き込み (一時ファイルの作成 + rename が毎秒 8 回) |

CPU としては小さいが、画面の描画と同じプロセスで確保する (494 の GC の負荷に足される) のと、ディスクへ書き続ける。

## 対応方針 (案)

- 読む側が居るときだけ書く (読む側が要求の印を置く / 読み出しの最後の時刻を見る)、または中身が変わっても書く間隔を 1 秒に延ばす。
  🚨 443 の「最新の 1 枚を読める」の約束 (`pro-con screen` が今の画面を見る。e2e とエージェントの確認に使う) を崩さない形を選ぶ
- どちらにしても、効果は「5 秒間の書き直しの数」で before / after を測る

## 関連ファイル

- `src/pro-con/relay/relay.go` — `Writer.Put` / `loop` / `sameContent` / `minInterval`
- `src/pro-con/main.go` — `openRelay` (受け口の配線)
- `src/pro-con/screencmd.go` — 読む側

## 進捗

- [x] 実測 (上の表)
- [x] 反証レビュー (読み取り専用のサブエージェント 1 体): 反証されず
- [x] 変化が続くあいだは 1 秒ごとにまとめる (commit「pro-con: 画面の中継は変化が続くあいだ 1 秒ごとにまとめて書く (504)」):
  書き出しの後に空けている間に次の描き直しが来ていたら (スピナー・演出)、次からは `busyInterval` (1 秒) ごとに書く。来ていなければ次の変化 (操作) は
  今までどおり `minInterval` (100ms) で書く。最後の 1 枚は必ず書く約束はそのまま。「読む側が居るときだけ書く」は採らなかった:
  `pro-con screen` は置き場に何も書かない約束 (`TestScreenDoesNotWrite`) で、読む側から合図を出せない。
  実測 (100ms ごとに中身の違う 1 枚を 5 秒置き続ける、実時間): 書き出し 50 回 → 8 回。
  テスト: `TestWriterSpacesWritesWhileChangesContinue` (空けた間隔の並びを、差し替えた after で見る。壁時計で測らない)。
  `bin/mutate-verify` で 続いても延ばさない / 止まっても戻さない / 決めた間隔を使わない の 3 本が red
- 記録のみ (敵対的レビュー 2 周目 P3。未確認): 空けている間の最後の 1 枚が前に書いた 1 枚と同じ中身に戻る動き (2 コマの点滅・行き来) は「止まった」と読まれ、
  100ms の間隔に戻る。スピナーは 10 コマなので実害は小さい。2 コマの演出を足すときに見直す
- [x] 操作の結果が `pro-con screen` に出るまで最大 1 秒遅れる件 (スピナー・演出のように中身の変わる描き直しが続くあいだ) は受ける (2026-09-26):
  読む側は `pro-con screen` を上限つきで待つ形だけ (`tests/pro-con/test_e2e_screen_relay.sh` は `tt_wait_until` で条件が成り立つまで待つ。`--follow` は 200ms ごとに読み直す)。
  遅れは合否ではなく所要時間にしか効かない。修正の後の `make test` でこの e2e が `[ok]` だった (commit「pro-con: Seen の時刻を書く時点で入れることをテストで固定する (502)」の時点)。
  再評価の trigger: 1 秒の遅れで待ちの上限に当たる検査・手順が出たとき
