...
- [525](525-perf-pro-con-tick-rereads-record-after-busy-day.md): 完了して 24 時間以内のカードが溜まる日は、Tick 1 回が 25ms・11.7MB (478 の見送りの前提が崩れた)。
...
- `register` のカード × session の二重ループ / `dispatch` の順番待ち (`card.HeldBy` の線形探索) / `tagScreen`: どれも 525 の Tick の bench の中に含まれ、単独では目立たない
...
既知の 478 / 494 / 502 / 503 / 504 は走査の前に除いた (候補に数えていない)。525 は 478 の見送りの再評価で、重ねた起票ではない。
