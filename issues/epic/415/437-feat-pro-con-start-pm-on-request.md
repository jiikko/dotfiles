...
### 2026-09-26 (カード C-026 の PG): PG の質問を PM に知らせる

- **質問待ちの列の PG の質問 (`WaitQuestion`) も、依頼の列のカードと同じ仕組み (`tellPM`) で PM に知らせる**。知らせには質問の文を添える。
  二重に知らせない鍵 (`pmKey`) は、依頼 = カード ID、質問 = カード ID と質問待ちに入った時刻 (回答で列を離れてまた質問したら、離れたのを
  PM 側で見ていなくても別の鍵 = また知らせる)。`pm.json` の `Told` / `Telling` にこの鍵を持つ (前の版の `Told` はカード ID だけ = 依頼の鍵と同じ形で読める)
- 権限の確認と落ちて止めた PG は PM に知らせない (PM には答えられない)
- **PM が人に回すと決めた質問は `pro-con card handoff <カード> "<理由>"` で履歴に残すだけ** (列も質問も変えない。人の番の目印は 452)。
  人に回した質問は知らせる物から外す (`card.HandedOff`)。外さないと PM が片付けられない物が残り、PM が落ちるたびに (30 分に 3 回まで) 起こし直し、
...
- 残り / 未確認のリスク:
  ...
  - この版に上げた最初の Tick で、その時点の質問待ち (人が答えるつもりで置いていた古い質問も) を全部新しい質問として PM に知らせる
  - 本物の PM で質問を受けて答えるところはまだ走らせていない (dogfooding 440)
...
- 2026-09-25 PM のレビュー (2 回目): 差し戻しの分 (`--pm=on|off` と設定の `pm`、off の告知) は妥当。457 を入れた master と合わせた tree で、PM を終了で止めるテストと
  457 のテストが両方通る (PM の記録の行のカード ID は `PM` で、カードとしては存在しないので 457 の `unregistered` には入らない)。
  **pm-guide の役目 5 をユーザーの分担に書き直した** (レビュー以降は取り込みの係。PM はレビューの列のカードに触らない)。`make lint` 0 件・`go test -race` 15 パッケージ ok で master へ。
  PG はブランチの rebase の後に自分のブランチへ `-f` で push した (消えたのは自分の rebase 前の commit だけ。本人が報告)
- ✓ **本物で初めて動いた** (2026-09-25 21:45): 437 を含む dispatcher を `--pm=on` で起こすと、PM を起動して依頼の列の C-001 (ping。10 時間残っていた) を知らせ、
  PM が 9 秒後に「回答済み」で閉じた (役目 2)。同じ時にレビューの列に居た 4 枚には触らなかった (役目 5 の分担どおり)
- △ **PG の質問は PM に届かない** (2026-09-25 22:55 の C-020): 分担 (pm-guide の役目 4) では PG の質問に答えるのは PM だが、dispatcher が PM に知らせるのは依頼の列のカードだけ。
  質問待ちに入ったカードは PM が知らないまま残り、取り込みの係 (人間の代わりの Claude) が答えた。質問待ちに入ったカードも PM に知らせる (人に回すべき質問は PM が人の番にする。452) — C-026 で対応 (下の節)
- △ 同じ C-020 で、PG が `card run` で make test を頼んだ直後に `card ask` したので、作業中の列を離れた時点でテストの係が実行を取り消した (「結果を待つ PG が居なくなった」)。
  回答の文に「頼み直して」と添えて回した。質問しても頼んだ実行を残すか、ask の前に run の結果を待つよう PG への指示に書くか — C-026 で後者 (下の節)
