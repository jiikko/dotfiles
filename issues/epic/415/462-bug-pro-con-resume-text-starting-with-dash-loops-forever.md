...
- 2026-09-25 PM のレビュー: 差し戻しなし (カード C-017。自動の PM が分けた)。`make lint` 0 件・`go test -race` 15 パッケージ ok → master へ。
  残り: **PM の起動・再開 (`dispatcher/pm.go`) には同じ回数の上限が無いように見える** (PG の読んだだけの指摘。記録に PM の行が無い状態で起動が拒否されると上限なく繰り返しうる。未確認)
- 残りの「PM の起動・再開に回数の上限が無い」は 464 (カード C-024) で確かめて、拒否が 3 回続いたら PM を起こさない形にした。**464 で解消**
