...
**段階 ①②③④ はすべて実装済み・稼働中** (④ は 2026-09-03 に完了)。
実装として残っている本題は無く、残りは **8 章の未決事項 (ユーザー判断待ち)** と、
7 章の未チェック 2 件 (BTM のパース失敗 = 段階 1 で未実装 / 起動時間の実測) だけ。
...
#### S2 の実装 (2026-09-03。commit `825f26b1`)

7 段すべて入った。決まったこと:

- **キー**: `Space` = 選択 (ページ送りは `ctrl+d` / `pgdown` に譲った) / `d` = 下見 → 確認 →
  `y` で実行 / `D` は閉じるまま。dry-run は独立キーにせず、`d` の確認モーダルが兼ねる
- **確認は下見 (engine の `DryRun`) の結果をそのまま出す**。UI 側で「何が消えるか」を組み直さない
- **選べない行**は押す前に理由を出す (走査していない / 走査できていない / 対象 0 件 /
  `risk: confirm` で中身を見ていない)。engine も拒否するが、押した後に失敗するより親切
- **実行中はキーを飲み、Ctrl-C 2 回で ctx を cancel** する
- **結果は 7 値をそのまま出す**。`incomplete` を成功にも失敗にも畳まない。閉じると再スキャン
- `updateKeyReachable` が `doctorOv.ownsKeys()` で C / X に譲る
  (tui.go のコメントが「issue 148 ④ が最初の該当」と予告していた箇所)

表示で踏んだこと (サンプルで回してから本体へ入れた):

- 記号を先頭に置く形は採らない。`・` と絵文字が同じ列に来ると、幅が合っていても目には揃わない
- ⚠️ **`🗑` は実測で幅 1**、`🗑️` (VS16 付き) は 2。端末ごとに右端が動くので `🚮` に替えた
- 🚨 **`padTo` は溢れた分を切る**。素直に並べると選択が多いときに「y: 削除する n/Esc: やめる」が
  消え、操作の分からない確認プロンプトになる。末尾を先に確保する形にした

engine の積み残しを 1 件足した: `OnPhase` (plan の走査は `DryRun` でも通り数秒かかるので、
口が無いと確認プロンプトが出るまで無言で固まる)。
...
**④ は完了 (S1 削除エンジン / S2 UI 導線 / S3 入口ドキュメントと受け入れ条件)。**
実装の残りは無い。次に触るなら **8 章の未決事項**か、7 章の未チェック 2 件。
④ で決めた不変条件は「S1 で決まった不変条件」節が正本。
