...
|---|---|---:|---:|---|---|
| ~~`fatih/vim-go`~~ | Vimscript | 19,167 | 0 | （削除済み）GoDecls→telescope symbols / 保存時整形→conform goimports / `]] [[`・`af if ac ic`→treesitter-textobjects / `K`=hover→native LSP | **✅ 置換して削除済み**(2026-07) |
...
### 2. vim-go（旧 fatih/vim-go） — ✅ 置換して削除済み（2026-07）— 難易度: **中**（置換して削除）
- **背景**: 19k 行の Vimscript プラグインだが、明示的に keymap していたのは GoDecls のみ。`g:go_gopls_enabled=0`/`g:go_def_mapping_enabled=0` で LSP・定義ジャンプは元からネイティブ委譲済みだった。加えて vim-go の ftplugin が既定で以下を有効化していた: `K`→`:GoDoc`、テキストオブジェクト `af/if/ac/ic`・関数ジャンプ `]]`/`[[`（`go_textobj_enabled` 既定 1）、**保存時 gofmt + goimports**（`go_fmt_autosave`/`go_imports_autosave` 既定 1）、`:Go*` コマンド群。
- **戦略**: 全書き換えは論外。**used surface をネイティブ / 既存 Lua プラグインで満たして vim-go ごと削除**（vendoring はしない）。
- **実施結果**: lazy spec を削除（`_nviminit.lua` の該当ブロックは廃止経緯コメントに置換）、`_lazy-lock.json` の entry も除去。used surface は以下で置換した:
  1. **GoDecls**: `nvim/ftplugin/go.lua` で Go バッファ限定に、`<leader>gd`→telescope `lsp_document_symbols`（kind フィルタ付き）、`<leader>gD`→`lsp_dynamic_workspace_symbols` に再現。※置換前は fzf/ctrlp 未導入（lazy に `fzf.vim`/`ctrlp` なし）で `<Plug>(go-decls)` が**非稼働だった**ため、実挙動の喪失はなく体験はむしろ改善。
  2. **保存時整形**: conform の `formatters_by_ft.go = { "goimports" }` ＋ `format_on_save` に go を追加（旧 `go_fmt_autosave`/`go_imports_autosave` 相当）。goimports 実体は mason-tool-installer で導入。
  3. **`]] [[` / テキストオブジェクト `af if ac ic`**: nvim-treesitter-textobjects（treesitter の select/move モジュール）を有効化し、`nvim/ftplugin/go.lua` で Go 限定 buffer-local に再現（vim-go の scope を踏襲）。
  4. **ハイライト**: nvim-treesitter の go/gomod/gosum parser（`ensure_installed` に追記）。vim-go の syntax は元から非稼働で treesitter が唯一の highlighter だった。
  5. **`K`（hover）**: Neovim 0.11 の native LSP 既定（K=hover）が引き継ぐ。
  6. **`:Go*` コマンド群**（`:GoTest` 等）: 未使用のため引き継がない（必要になれば go.nvim / nvim-dap-go）。
- **副次利得**: `:GoUpdateBinaries`（起動時 build ステップ）と `g:go_*` フラグ群が不要になり spec が単純化した。

### 3. vim-rails — ✅ 削除済み（2026-07-10）— 移植も vendoring もせず廃止
- **決定**: 棚卸しの結果、**削除**（Lua 移植・代替導入なし）。`_nviminit.lua` の vim-rails lazy spec と `_lazy-lock.json` の entry を除去（`~/.local/share/nvim/lazy/vim-rails` の実体は次回 `:Lazy clean` で消える）。
- **判断根拠**:
...
- **戦略**: **全書き換えはプラグイン再著述に等しく非推奨**。config は offscreen popup + surround という**特徴的機能**を使っている（`_nviminit.lua` の `andymass/vim-matchup` spec: `matchup_matchparen_offscreen={method="popup"}` / `matchup_surround_enabled=1`）ため、素朴な代替では劣化する。
...
2. ~~**vim-go を used surface 置換で削除**~~ — ✅ 2026-07 に完了（19k 行の Vimscript + build ステップを一掃）。conform goimports で保存時整形、telescope symbols で GoDecls、treesitter-textobjects で `]] [[`・`af if ac ic` を再現。詳細は §2。
3. ~~**vim-rails**~~ — ✅ 2026-07-10 に削除済み（棚卸しの結果、移植も代替もせず廃止。def ジャンプは LSP 担当で喪失なし）。
4. **vim-matchup は対象外** — 最難・低ROI。Vimscript のまま残す（または組み込み matchparen+treesitter への割り切りを別途判断）。
