...
### 🚨 2026-09-10 更新: 起点だった 304 の判断が出て、10/10 が決着した

- **304** ✅ done。ユーザー承認を得て孤児サーバ 6 本を回収し、死んだ socket 537 個を削除した
- **305** ✅ done。① tmux socket は**掃除機構ではなく発生源を断つ**形で閉じた
  (`tmux kill-server` が socket を消さないので**正常終了のたびに 1 個ずつ漏れていた**。
  起票時の「trap が中断で走らないから残る」という前提が誤りだった)。
  ② Go テストの一時 dir は `src/doctor/testtmp` で決着済み

**この記録 issue の役目 (却下理由の保存) は果たしたので done へ送る。**
