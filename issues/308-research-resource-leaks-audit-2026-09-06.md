...
2026-09-09: **305 は ② Go テストの一時ディレクトリだけ解消**（`src/doctor/testtmp` に起動時掃除を新設。SIGKILL で残った dir が次回起動で回収されることを実測）。**① tmux socket は継続** — 消す対象がプロセスなので母集合を取り違えると本番サーバを殺す（2026-07-30 に事故の実績）ことと、既存 6 本の処遇が issue 304 の判断待ちであることが理由。他の派生（298 / 299 / 300 / 301 / 302 / 303 / 306 / 307）はこのセッションでは触っていない。
