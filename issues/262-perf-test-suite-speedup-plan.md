...
起票日: 2026-09-05
カテゴリ: perf
優先度: **中** (施策 E のみ。CI 全体の壁時計は 1 秒も縮まないので、下記「前提」を先に読む)
...
1. **E-1 (FIFO 同期)** — 効果最大 (~25s)、対象は 1 ファイルに集中、flake も同時に減る。
   変異検証のやり直しと 3 回連続 green が要るので、そこを見込んで着手する
2. **E-2 (条件ポーリング)** — 独立した小さい修正が 6 箇所。1 つずつ入れられる
3. **B (直列腕を 3 本目の job へ)** — 「Tests の再実行待ちが長い」という痛みが出てから
4. **C (直列腕を減らす)** — B の後。`test_schedule_keys.sh` の 27s を測るところから
5. **A (`NPROC` 文書化)** — 性能施策ではないので、他のついでに
