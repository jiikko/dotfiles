...
## EPERM の追試 (2026-09-19。実測。残タスクの本体)

**環境**: darwin 24.6 (arm64) / go 1.26。使い捨てプローブ (`tmp/384-eperm`。gitignore なので破棄済み) で
`Setpgid` の子を起こし、`kill(-pgid, SIGTERM)` の **errno を分けて** 測った。**3 回とも同じ結果**。

| # | 形 | `kill(-pgid, SIGTERM)` | 直接の子 |
|---|---|---|---|
| A | 対照 (setsid なし) | **成功** | TERM で死ぬ |
| B | 子自身が `setsid()` を試みる | **成功** | TERM で死ぬ |
| C | 子が `setsid` した孫を起こす (子は生存) | **成功** | 子は死ぬ / 孫は別 session で生き残る |
| D | 子が終了して回収済み (グループが空) | **ESRCH (3)** | — |
| E | 子が終了したが**未回収 (ゾンビだけ)** | **EPERM (1)** | — |
| F | ゾンビの子 + **同グループの生きた孫** | **成功** | 生きたメンバーが 1 つでもあれば届く |

### 結論 1: issue 本文の「子が `setsid()` すると kill が EPERM」は**誤り**だった

`Setpgid` で起こした子は**自分がグループリーダー**なので、**`setsid()` 自体が EPERM で失敗する**
(B の stderr: `child: setsid failed: operation not permitted (errno=1)`)。
つまり EPERM を受け取るのは**親の kill ではなく子の setsid** で、出典のレビュワーはおそらく
この 2 つを取り違えている。`setsid` した孫が居る形 (C) でも、子が生きていれば kill は成功する
(孫に届かないのは別問題で、`runWith` の `Setpgid` の注記が既に記録している到達範囲の限界)。

### 結論 2: EPERM は**再現した**。ただし「生きたメンバーが 1 つも無いとき」だけ

E (ゾンビだけのグループ) が EPERM を返す。macOS ではゾンビも**グループのメンバーとして残る**ので
ESRCH にならない。D (回収済み = 本当に空) は ESRCH。**F が示すように、生きたメンバーが
1 つでもあれば kill は成功する**ので、「生きているのに昇格が焼き切れる」形は**構成できなかった**。

### 結論 3: 焼き切れ自体は起きるが、測れた範囲では無害

E / D はどちらも `killGroup` がエラーを返す → `warnf` → `return` で **`escalate.Do` は消費済み**
= 以後は昇格しない。ただしどちらの errno でも**グループに生きたメンバーは居ない**ので、
昇格すべき対象がそもそも無い。**Once を消費したままにする**という判断 (上の「進捗」節) は、
この実測でも支持される。

### 結論 4: 🚨 現在の warn は「測れた範囲では偽」。それでも残す理由を書いた

`昇格できない (...)。**子は走り続けている可能性がある**` は、E / D のどちらでも**生きた子は居ない**ので
その場では偽陽性になる。消さなかったのは、**別 uid が所有する生きたメンバー**が居る形を
**測れていない**ため (root 権限が要る)。この 3 点 (測れた errno の意味 / 測れていない形 /
再開の trigger `ps -o pid,pgid,user -g <pgid>`) を `with.go` の当該 warn の直上に書いた。

**残る未確認**: 別 uid のメンバーが居るグループへの `kill(-pgid)` (setuid が要るので未実施)。

## 残タスク

- [x] 1 の方針を決めた (Once は消費したまま + 報告を厚くする。理由は `kill(2)` に一過性の失敗が無いこと)
- [x] 2 を直した (seam 付きで検査手段も用意した)
- [x] EPERM の実測の追試 → **上節**。EPERM は**ゾンビだけのグループ**で再現 (errno 1)。
      本文の「子が setsid すると EPERM」は誤りだった (EPERM を受けるのは子の setsid 自身)
...
- [x] EPERM の実測の追試 (2026-09-19) → **上節**。A〜F の 6 形を 3 回ずつ測った
- [x] **記録済み (2 周目 P2-1)**: SIGKILL 側の guard は挙動中立ではない。**直接の子が回収済みで、
      ...
      🚨 **2026-09-19 の実測 (形 F) がこの trade-off を裏付けた**: ゾンビの子 + 同グループの
      生きた孫でも `kill(-pgid)` は**成功する**ので、guard で降りると「届いたはずの生きた孫」を
