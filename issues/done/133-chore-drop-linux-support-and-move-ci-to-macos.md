...
| `make test-gnu` + `scripts/with_gnu_grep.sh` | grep の方言差を push 前に手元で出す | 正規表現を移植可能に保つ規律。macOS 単一では不要。opt-in なので CI には元から無い | **撤去** ([108](108-chore-no-gnu-grep-repro-for-tests.md) に撤去の追記) |
