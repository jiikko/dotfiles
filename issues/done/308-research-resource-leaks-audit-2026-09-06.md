...
### 🚨 2026-09-10 更新: 起点だった 304 の判断が出て、10/10 が決着した（305 は下記のとおり訂正: open）

- **304** ✅ done。ユーザー承認を得て孤児サーバ 6 本を回収し、死んだ socket 537 個を削除した
- **305** ⏳ **open のまま**（2026-09-11 実測で訂正。上の行は「done」と書いていたが誤り）。
  ① tmux socket は**掃除機構ではなく発生源を断つ**形で閉じた
  (`tmux kill-server` が socket を消さないので**正常終了のたびに 1 個ずつ漏れていた**。
  起票時の「trap が中断で走らないから残る」という前提が誤りだった)。
  ② Go テストの一時 dir は `src/doctor/testtmp` で決着済み。残っているのは本文末尾の
  残タスク（SIGKILL で trap が走らない形は「検出しない」と決めた形として明記済み / `/private/tmp` の
  殻は再起動で消えるので削除しない）で、**この記録 issue が依存する判断はもう無い**

**この記録 issue の役目 (却下理由の保存) は果たしたので done へ送る。** 本文で「残る」と書いた
2 点（① の `gitlog.go` doc の例外 1 行 / ⑨ の `runner.go` の trigger 1 行）は 2026-09-11 に入れた。
