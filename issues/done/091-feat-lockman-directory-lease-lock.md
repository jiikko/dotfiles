...
- **shared (read) lock は作らない**。排他のみ。「参照中のマーク」は排他ロックの保持で表す
- **`--meta-dir` は用意しない**。`.lockman/` 固定とする (共有でない場所を指されると排他が
  黙って消える footgun のほうが重い)。バックアップから外したい場合は除外パターンで対応する。
  将来必要になったら「全アクセス経路から同じ実体が見えること」を必須要件として再検討する
- **`--wait` の既定は「待たない」** (即 exit 3)。`--wait <duration>` を明示したときだけ待つ
- **サブキーは持たない**。1 ディレクトリ 1 lock。必要になれば `.lockman/lock.<key>` の形で
  後方互換に足せる (今は作らない)
- **ローカル FS 専用の速い経路は作らない**。経路で分岐させると、分岐した側だけ壊れる形の
  事故を招く。同じ手順で十分速い
...
置き場所は `.lockman/` 固定。外へ逃がすオプションは作らない — 下の「経路が混在しても
透過に効くこと」参照)。
...
- ❌ `flock(2)` / `fcntl` byte-range lock を**単独の根拠にしない**。SMB マウントでは
  実装依存で、マウント設定次第では黙って無効化される (macOS smbfs が実際に送るかは実測項目)
...
B. link(tmp/<token>.json, lock)          ← 存在すれば失敗する原子操作。勝者は 1 人
   ...
   ・🚨 **macOS の smbfs では ENOTSUP になる可能性が高い**。使えたら儲けもの、程度に
...
> rename のあいだを 60ms 広げると 10/10 で勝者が 2〜5 人になり、graveyard には**先に勝った
> 者の新しい lock** が入っていた。手順 2 の前に「観測した世代の id を O_EXCL で取る」調停を
> 足し、rename の直前に世代と mtime を再照合する形へ直してある。実装の正本は
> `src/lockman/lock.go` の `tryTakeover`。

### 生死の判定に使う TTL は「保持者が宣言したもの」

🚨 **奪いに来た側が渡す `--ttl` を生死の判定に使わない**。使うと
`lockman acquire "$dir" --ttl 30s` と打つだけで、30 分の lease を持つ相手を 30 秒で
追い出せる (= 排他が任意に破れる)。呼び出し側の `--ttl` は**自分が新しく作る lock の
TTL** にだけ効く。判定の出典は `lock` の中身に書かれた保持者の TTL。

実装ではこれを一度取り違え、**macOS のローカル FS では速すぎて再現せず、CI (Linux) の
「引き継ぎの勝者が 4 人」で露見した**。lock の中身の TTL は秒の整数で持たないこと:
1 秒未満が 0 に丸められ、fallback の既定値 (30 分) に黙って化ける。
...
- 🚨 **メタの置き場所を変えるオプションを作らない**。片方のマシンだけローカルの別
  ディレクトリを指した瞬間、**両者が別々のロックを取って全員勝つ** (排他が黙って消える)。
  この壊れ方は実行時に検出できない (相手が別の場所を見ていることを知る手段が無い) ので、
  **選べないようにするのが唯一の防御**。`.lockman/` 固定とし、バックアップから外したい
  場合は除外パターンで対応する
...
🚨 **macOS のファイル共有が SMB の byte-range lock を配下の POSIX ロックへ写すかは未確認**。
...
`open(2)` からは指定できない)。
`fcntl` byte-range lock がそれに一番近いが、smbfs が実際にサーバへ送るかは実測待ちで、
黙って無効化されうるので単独の根拠にはできない。
...
🚨 **照合は token だけではない。`renew` は `release` と同じく期限も見る** (issue 312 で追加):
token が一致していても TTL を超えていれば延長せず **exit 4** を返す。まだ誰も引き継いで
いなくても同じ (その lock はもう誰でも引き継げる状態なので、書き直すと `readLock` と
`O_TRUNC` の隙間に入った引き継ぎ相手の lock を上書きする)。判定は `expired` /
`holderTTL` を `release` と共有する。
...
### `with` の終了コードは子と衝突させない

`with` は**子プロセスの終了コードをそのまま返す**。したがってロック側の失敗を 3 / 4 で
表すと、**子が 3 で終わったときに「busy」と区別できない**。`timeout(1)` / `env(1)` と
同じ流儀で、ロック側の失敗は上位の番号へ逃がす:

| `with` の exit | 意味 |
|---|---|
| 子の終了コード | そのまま透過 (0〜120 の範囲は子のもの) |
| 121 | ロックが取れなかった (busy) |
| 122 | 走行中に lease を失った (`--on-lost=kill` で子を止めた) |
| 125 | lockman 自体のエラー (I/O・権限・判定不能) |

`acquire` / `release` / `renew` / `check` 単体は従来どおり 0 / 1 / 3 / 4 を使う。
**両者で番号の意味が違う**ので `--help` に表で併記する。

### パーミッション — sticky bit が付いていると引き継げない

`rename` の可否は**ファイルではなく親ディレクトリの書き込み権限**で決まる。`.lockman/` に
sticky (`+t`) が付いていると**他ユーザーの `lock` を `graveyard/` へ rename できず、
TTL が切れても永久に奪えない**。

- `.lockman/` は **sticky なし・共有前提のモード (0777, umask を明示的に外す)** で作る
- **起動時に sticky を検出したら警告する** (fail はしない。読み取り系は動くため)
- SMB 経由ではユーザーマッピングが異なるので、`lock` は誰でも読める・親は誰でも書ける
  前提にする。ここを絞ると引き継ぎが動かなくなる

### マウントが固まったときに沈黙しない

smbfs はサーバ不達で長時間ブロックする。スクリプトの中で無言のまま固まるのが最悪なので、
**`--io-timeout` (既定 10 秒) を設け、超えたら「判定不能」= exit 1 (`with` は 125) で返す**。
「固まった」と「空いている」を絶対に混同しない。

### 待ち方 (`--wait` 指定時)

- 指数バックオフ 1s → 上限 15s、**±30% のジッタを必ず入れる** (複数マシンが同時に待つと
  同期して叩き合うため)
- **公平性は保証しない**。先に待ち始めた者が勝つ保証は無い (starvation はしうる)。
  待ち行列を持たない設計なので、これは仕様として `--help` に書く

### 使う側の規約

- **再入不可**。同じスクリプトが同じディレクトリを二重に `acquire` すると自分で自分を
  締め出す。token を引き回して 1 回に保つ
- **複数ディレクトリを取るときは絶対パスの辞書順**で取る (デッドロック回避)
- 対象ディレクトリが存在しない / `.lockman/` を作れない → exit 1 (busy に倒さない。
  「そもそも対象が無い」と「使用中」は別物)
- `.lockman/lock` がディレクトリなど想定外の型だった → 壊れているので **busy に倒す** (exit 3)
  + 理由を stderr に出す
...
glogx は popup の体感速度のために「旧版で即起動」を選んでいるが、排他の道具で古い
バイナリが動くのは危険なので同期ビルドにする。

**CI**: `src/lockman/Makefile` (`lint` / `test`) + root Makefile の `GO_PROJECT_DIRS` 登録 +
`.github/workflows/src_lockman.yml` の 3 点セットを同じ変更で入れる。
🚨 **CI で回るのはローカル FS 上のテストだけ** (SMB 共有は CI に無い)。SMB 由来の前提は
CI では一切検証されないことを README とテストの冒頭に書き、実機検証の `human` issue と
セットで扱う。
...
- **経路の違いに引きずられないこと**: 同じ実体を別パスから触らせて (Linux なら bind mount、
  ...
  (メタの置き場所を変えるオプションは作らないので、経路ごとに別のロックを見る事故は
  構造的に起こらない。この「作らない」を守っているかは、オプションの追加時に思い出せるよう
  `--help` とテスト名に残す)
- **`with` の終了コードが子と衝突しない**: 子が exit 3 / exit 4 で終わるケースを走らせ、
  呼び出し側が「busy」と誤読しないこと (子の 3 は 3 のまま、busy は 121)
- **sticky bit の検出**: `.lockman/` に `+t` を付けた状態で警告が出ること。さらに
  **別ユーザーの lock を引き継げない**ことを再現し、警告が空振りでないことを示す
- **I/O が固まったときに沈黙しない**: 応答しないパス (FUSE でも `SIGSTOP` させた
  ヘルパーでもよい) を用意し、`--io-timeout` で exit 1 になること。**「空いている」に
  倒れないこと**が本体
- **待ちのジッタ**: `--wait` で複数プロセスを同時に待たせ、再試行時刻が揃わないこと
...
- クライアント・サーバの実装差 (キャッシュ、再送、ロックの転送可否) を完全には潰せない。
  TTL 下限と fail-closed は「潰しきれない分を安全側に倒す」ための設計

## 未決

着手前に判断が要るものは無い (上の「決定済み」で全て埋まった)。実装中に前提が外れたら
「実測が要る前提」の表の落とし先に従い、判断が変わったら決定済みの節を書き換える。
