...
- 見え方・位置・色の判断は issue 186 (human)。issue 125 には追記していない
  (あちらはウィザードの確認で、独立して消化できる)

### 敵対的レビュー (read-only サブエージェント 1 体、codex は未使用)

2 件出て、どちらも実コードで再現条件を確認できたので直した (commit 4039686 と後続)。

- **P1**: `$STATE_DIR` は全サーバで共有 (`-L` の隔離サーバも同じディレクトリ) なのに、
  `refresh_pane_indicator` は pane id の文字列一致だけで予約を数えていた。pane id は
  サーバごとに振り直されるので、別サーバの予約の時刻がこちらの枠に出る。job に記録済みの
  socket + サーバ pid で絞る形に直した (`fire_claim` が送信前に見る基準と同じ)。
  サーバを同定できないときは何も書かない (fail-closed)
- **P2**: `fire_claim` の表示更新がサーバ同一性チェックの**前**にあった。socket を別サーバが
  握っている状況では無関係な pane に `set-option` していた。チェックの後へ移し、不一致の経路では
  `REPLY_PANE` を空にして `fire_drop` 側の更新も no-op にした
- 壊せなかったと報告された観点: `REPLY_*` の汚染 / 無音契約 / 最早選択のロジック /
  conf の `#,` エスケープ / 書き直した 2 本のテストが vacuous でないこと
- P3 (対応しない): job 1 行目の pane 値を正規表現で検証していない (書き手は `#{pane_id}` の
  出力だけなので実害なし) / job 書き込みの原子性が無い (既存の `read_job` / `jobs_tsv` と
  同じ前提で、今回の変更が持ち込んだものではない)

2 周目の変異検証 4 本 (socket 絞りの削除 / fail-closed の削除 / 更新をチェック前へ戻す /
`REPLY_PANE` の空化の削除) すべてで red。
