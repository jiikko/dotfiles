...
- [x] 268（比較を O(1) 化）— `09ec9586`。2000 件 62.4 → 23.2µs、選択あり 439 → 24.6µs
- [x] 269（比 metric + `check_bench_budgets.sh` の書式）— `79aa1c8f`。
      `issues_view_scale_x100` を導入し、退行 commit `fc0f65ab` で 255 > 150 と赤くなることを実証
- [x] 270（`detail` の遅延化）— `3dd7f772`。合成 6,400 items で 7,153 → 1,464µs。
      `copyText` / `copyPath` は順序の穴があるため見送り、trigger を本文へ
- [x] 271（コメントの訂正）— `48106e0d`。動的な fd 会計は見送り、trigger を本文へ
- [x] 272（`detailsLoading` の解放）— `748b3d69`
- [x] 274（hint の早期 return）— `42a53b80`
- [x] 275（ratelimit のメモ化）— `a1eb7534`

## 記録として残らなかったもの

- 268 の「選択あり」のベンチは **恒久化した**（`BenchmarkIssuesViewFrameSelected2000`）。
  ポインタ受けを誤ると測定が成立しない罠は `b.Fatalf` の番人で塞いだ

## 実装して分かったこと（起票時の記述が誤っていた点）

- **268**: 「`rows` を直接差し替えるテスト/helper は実在しない」は**誤り**。
  `scroll_glide_test.go` が直接差し替えており、issue の推奨（描画ループ内の 3 箇所を外す）を
  実行していたら `displayRows` が空のまま描かれていた
- **274**: 「確保の 88.9%」はこの呼び出しの形では成り立たない（`diskItemKey` の結果は
  エスケープせず **0 allocs**）。実コストは時間（6,400 items で 108.8µs → 3ns）
- **271**: 推奨対応が原理的に無効だった（fd はイベント経路で増える / `issues` 側は
  watch 集合が指紋の入力を兼ねる）
- **272**: 「`spinnerActive()` が下りない」は交絡していた（`fetching()` と
  `usageOv.loading()` が正当に真）。「札が残る」自体は本物
