...
## 敵対的レビュー 2 周目 (2026-09-16。§7「修正した差分にもう 1 周」)

1 周目の指摘を直すために**新しい判定ロジック**を作った (三値 probe / lsof による持ち主の探索 /
削除ヘルパー) ので、打ち切り条件 (判定ロジックを新設していない) に当たらず 2 周目を通した。
指摘 9 件、**採用 8 / 記録 1 / 却下 0**。🚨 **1 周目の修正がまた P1 を 3 件抱えていた**。

| # | 指摘 | 判定 |
|---|---|---|
| P1-C | 三値の `dead` は「client が 3 秒以内に返って標準出力が空」でしかなく、**版ずれ / `tmux` が PATH に無い / fd 枯渇**でも成立する (サーバは生きている)。1 周目 P1-1 と同じ帰結で経路だけが違う | **採用**。しかも**本当に死んだサーバでも client は非 0 で終わる**ので子の rc でも分離できない → 生死の根拠を **「socket の持ち主が居るか」(`lsof`) という観測**へ置き換えた。lsof が無い環境は判定不能として**何も消さず rc=1** (fail-closed) |
| P1-A | `set -e` の呼び出し元 (bell は trap から呼ぶ) では、受けそこねた rc=2 でシェルが即死し、**昇格も stderr の案内も rc=1 も丸ごと飛ぶ** | **採用**。`|| :` で受ける。⑬ で end-to-end を固定 |
| P1-B | EXIT trap 内の `return 1` は終了コードを変えない (実測)。ctrl_v はサーバが生き残っても rc=0 + "All tests passed" | **採用**。`exit 1` にした |
| P2-A | 件数ガードが 15 のまま (実測 21 件) で、⑦〜⑫ を**全部消しても緑** | **採用**。式に足した |
| P2-B | fixture の socket パスが AF_UNIX の 103 バイト上限まで**残り 2 バイト**。長い `$TMPDIR` で 4 件が「socket を作れない」赤になり、**実装の赤と見分けが付かない** | **採用**。base を `/tmp/tth.XXXXXX` に固定し、前提の失敗は `HARNESS` と明示 |
| P3-A | ⑫ の主張が実装より強い (stub をハングさせると正しい実装でも赤) | **採用**。「本番方向の実体へ撃たない」へ狭めた |
| P3-B | INT の trap が return すると bash は**中断地点から実行を再開**する | **採用**。`exit 130` を撃つ |
| P3-C | lsof は symlink を辿らないのではなく**bind した文字列と照合する** (実測)。コメントの理由が誤り | **採用**。理由を実測に合わせた (tmux が bind 前に実パス化するから揃える、が正しい) |
| P3-D | `TT_PROBE_*` のリセットが no-op で、謳う不変条件が無検査 | **記録**。三値 probe を廃したので解消 |

**壊せなかった経路** (2 周目が実験で確認): symlink / hard link による別名で本番 socket の持ち主を
引かせる形は**原理的に成立しない** (lsof は inode でなく bind 名で照合する)。パス側の `default`
除外を消しても `tt__rm_socket` 自身の除外と `comm == tmux` ゲートが残り、多層防御は意図どおり効く。

### 変異検証 (検査 23 件。ケース名ごとの pass/fail で判定)

| 変異 | 結果 |
|---|---|
| 生死を応答から推論する旧設計へ戻す | ⑩ red + ⑪ red |
| 停止要求の rc を受けそこねる (P1-A の退行) | ⑬ red |
| lsof 不在を fail-open へ倒す | ⑭ red |
| パス側 / 名前側の `default` 除外を外す | ⑫ red / ⓪ red |
| `if` を `&& { }` へ戻す | **全緑 = 等価変異** (bash の errexit は AND-OR の最後以外に適用されない。実測) |
