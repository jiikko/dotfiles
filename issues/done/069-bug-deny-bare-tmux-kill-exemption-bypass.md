...
`4dae526` で両方解消。A は `-c` / `-f` / `-T` も値を 1 トークン消費する形にし、免除に数えるのは
`-L` / `-S` のときだけに保った。B は照合前に先頭の連続バックスラッシュを剥がす形。
テストは 56 ケースへ拡張され、A を 4 形・B を 2 形 pin している。

**別セッションによる裏取り** (実装者の主張をそのまま採らない):

- 攻撃ハーネス 9 ケースが 9/9 ok (`-f`/`-c`/`-T` の後の `-L` が allow、`\tmux` が deny)
- 変異 2 種で red を確認: 値消費 (`-c | -f | -T) skip_val=1`) を削ると A のケースが red、
  バックスラッシュ剥がしを削ると B が red。どちらも実装者のテストと私のハーネスの両方が捕まえた
