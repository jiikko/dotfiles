...
| 6. 削除中の `ctrl+g` が `ctrl+c` と同じ扱い | **[244](244-bug-doctor-ctrl-g-not-in-abort-guidance.md) へ切り出した**。`src/glogx/tui.go` / `doctor_delete.go` は issue 242 を進めている別セッションが同時に触っており衝突するため |
...
- **P3-6 は未着手**。[244](244-bug-doctor-ctrl-g-not-in-abort-guidance.md) へ切り出した (242 が done になってから着手する)
- 実機での目視確認は未実施

## この issue を閉じる (2026-09-04)

**監査の全数勘定という役目は果たしており、残る作業は 244 に移した。** 台帳を別に持つと
本体と二重管理になる (issue 207 で実際に更新漏れを起こした) ので、ここで閉じる。

- 起票した 5 件 (231 / 232 / 233 / 234 / 235) は別 issue として追跡されている
- P3 6 件のうち 4 件は修正済み、1 件 (P3-5) は「外さない理由」をコードのコメントに残した
- 残る 1 件 (P3-6) は 244 へ
- 「崩せなかった攻め口」8 件と却下理由は本文に残る (次の監査が同じ指摘を再生成しないため)
