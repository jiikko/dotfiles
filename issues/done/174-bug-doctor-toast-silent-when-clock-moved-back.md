...
### 後続セッションによる確認 (2026-09-03、commit `6bc006c1`)

上の「対応」は予約セッションが worktree で書いたが**未コミットで終了**していた。後続セッションが
master 上で `age >= 0` を外す変異を当て直し、`TestDoctorStartupToast` が red になることを確認して
commit した。上記「172 の記録を参照」とある敵対レビューの記録は 172 には残っていなかった
(予約セッションが書き切る前に終了)。レビューの内容は 172 / 173 の本文に、残った証拠 (コード内コメント
が引く実測値) の範囲で書き直した。

- [x] `LastNotifiedAt` が未来のときトーストが出ることをテストで固定した
- [x] 変異検証: `age >= 0` を外すと沈黙が再現する
