...
- **P1 並行**: 複数セッションを同時に起動すると BSD `ln -sfn` の非アトミックな retry (symlink →
  ...
  **追記**: 状態判定だけでは足りず、b2f8ede の CI (run 33584183538) でテスト 15 が落ちた。自分の ln が
  成功した直後でも、他方が unlink 済み・symlink 未作成の瞬間を見ると link が「無い」。手元でも
  5 並行 x 20 回 (stale 20 件) で 1/100 再現。状態確認を最大 5 回やり直す形にして、5 並行 x 40 回 x 3 で
  0/600。retry を 1 回に戻す変異では負荷試験が再び赤になる (unit テストの 3 x 5 は検出力が弱く、
  手元では緑のまま CI で落ちる形だった。負荷試験そのものは commit しない: 実行時間と flaky 性のため)
