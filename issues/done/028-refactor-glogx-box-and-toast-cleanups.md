...
🚨 以下は着手前にコードを再確認すること (下記は 2026-07-25 時点の評価)。
...
🚨 **ただし「スタックの方が優先度付き単一スロットより要件を満たしている」と最初に書いたのは
...
先に落とし、同じ重要度なら古い方から落とす。🚨 残す枚の並びは元のまま (上が新しい) —
...
- **P3 完了 (案 B)**: `TestBrowseSpinnerActiveSources` を追加し、14 の非同期/アニメ源を 1 つずつ
  ...
  🚨 **ただし issue 本文の「追記漏れが test 失敗として現れる」は不正確**: 新しい源を足して
