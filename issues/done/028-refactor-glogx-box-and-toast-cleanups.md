...
**残件 (別 issue 相当・未対応)**: 「保持」と「表示」が別の場所で決まっている。実際に描かれる枚数は
`boxLines` の描画予算 (`max(page/2, toastBoxLines)`) が握るので、狭い端末では保持していても
画面に出ない (レビュー実測: popup 高さ ≤20 で後続 1 枚、≤28 で後続 2 枚あると最古が描かれない)。
`toastStackMax` は保持数であって表示数ではない、という食い違い。1 本の再現から表示規則の設計を
変えるのは早いので、観測を添えて判断待ちにする。
