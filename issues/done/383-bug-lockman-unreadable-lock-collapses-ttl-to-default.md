...
## 敵対的レビュー 5 周目 (2026-09-18)

**5 周連続で P1**。今回も核 (fail-closed) は無傷で、**3 件とも 4 周目が足した「案内機構」由来**。
指摘 5 件、**採用 3 / 記録 2 / 却下 0**。

| # | 指摘 | 判定 |
|---|---|---|
| P1 | 3 周目が「消した」はずの破壊的助言が、**4 周目の案内先に復活していた**。`acquire` / `with` は `lockman break` を促さない (3 周目 P2-1) が、**その 2 つが名指しする `status` が促していた** (`main.go`)。しかも添えた前提「保持者が居ないことを確かめてから」は**どのコマンドでも確認できない** (保持者の記録は lock の中身にしかなく、それが読めないのがこの状態の定義。status=unreadable / check=busy / acquire=この案内へ戻る)。一過性の窓 (`tryPlace` の O_EXCL fallback は本番 smbfs で 0 バイト lock が見える区間を持つ) で従うと**生きた保持者を剥がす = 二重実行**。`--on-lost=warn` / 素の acquire では被害が無界。🚨 **4 周目 P3-2 の全数勘定「break の案内は production に 2 箇所」は、それを書いた commit 自身が 3 箇所目を足していたので偽**（実数 3: `lock.go` の後始末 / 回収 / `main.go` の status） | **採用**。status から break 節を削り、**材料だけ出して止める**形にした (機構を足さず削る = §0-A)。実数も訂正 |
| P2-1 | `Inspect` が `serverNow` の失敗を**捨てて** `AgeSec` を 0 のままにしていた。唯一の判別材料 (「経過が伸び続けるなら残骸」) が、**3 時間前の残骸を `age=0s` = 「いま書かれたばかり」**と嘘をつく。読める lock では同じ故障が rc=1 + 原因表示で落ちるのに、読めない側だけ無警告。発火条件 (probe を作れない / stat できない = 権限・ENOSPC・詰まったマウント) は**まさに wedge を疑って status を打つ場面と相関する** | **採用**。`AgeSec` をポインタにし、nil = 未取得 → 人間向けは `age=不明` / JSON は欠落。エラーを上へ返す案は採らなかった (4 周目 P1-2「案内先が答えられない」をその故障モードで再生産するため) |
| P2-2 | `--json` の `size_bytes,omitempty` が**値型**だったので、**canonical な unreadable lock (= 0 バイト) でいちばん重要な材料が丸ごと消えて**いた。機械からは「0」「stat 失敗」「古い版」が区別できない。テストは `"unreadable":true` の部分一致しか見ておらず無検査 | **採用**。`SizeBytes` もポインタにした (P2-1 と**同じ表現で両方閉じる**。bool を対にして持つ形は避けた) |
| P3-1 | `cleanupOwn` の `ownErr != nil` / `Lstat` エラー枝は、**結末 (0 バイト lock が残る) が Remove 失敗枝と同じなのに、次の一手を書いていない** (重い方が黙る非対称) | **記録**。出口を足すと機構が 1 つ増えて §7 の 6 周目を呼ぶ (2〜5 周目の P1 が全部この経路)。**残る案内は `lock.go` の 2 箇所だけ**で、どちらも「自分のものだ / 猶予超過だ」という積極的証拠を持つ地点。再開 trigger: 実際に `ownErr != nil` の詰まりが報告されたとき |
| P3-2 | `Inspect` が `readLock` のエラー本文を捨てるので、**「lock がディレクトリ」= 人が動かない限り絶対に解けない**と分かっている原因が、どの出口にも出ない (「一過性か残骸か」の推測文 + 意味のない `size=64B` が出る) | **記録**。分類自体は 3 周目 P1-1 で直っている (実測で確認済み)。原因テキストを通す `Reason` を足すのは「足す」側なので見送る。再開 trigger: ディレクトリ lock の誤診が実際に起きたとき |

### 変異検証 (5 周目の修正分)

| 変異 | 結果 |
|---|---|
| `status` の文面に `lockman break` を戻す | `TestUnreadableLockGuidanceReachesCLI` red |
| 取れなかった age を 0 に丸める | `TestStatusDoesNotFakeAgeWhenServerTimeIsUnavailable` red |
| `SizeBytes` を値型 + omitempty へ戻す | `TestStatusAnswersForUnreadableLock` red (`{"held":true,"unreadable":true,"age_seconds":0}` = 0 バイトが消える) |

🚨 **5 周連続 P1 の構造的な原因が特定できた**: `errUnreadableLock` のコメントは
「break を促さない」を**不変条件として宣言**しているのに、検査していたのは **3 hop 中 2 hop**
(`err.Error()` と `acquire` の stderr) だけで、**破れる hop (status / with の案内先) は未 pin** だった。
文字列を消すだけでは次に案内文を触った人が再生産するので、同じ commit で `guided` (status を実際に
走らせた出力) と `out2` (with の stderr) に break 不在の assert を足した。

### 壊せなかった経路 (レビューが実験で確認)

- **2 段目の新しい fail-closed (`m2 == nil`) の over-block**: 崩せなかった。`m == nil && err == nil` は
  「存在しない」経路だけで、そこは必ず `mtime.IsZero()` なので手前の枝が先に `took=true` を返す。
  正常な引き継ぎ (中身が読める期限切れ lock) は 1 度も止まらず、降りる場合も defer が claim を外すので
  猶予いっぱいの starvation も作れなかった
- **`Inspect` の状態分岐が他のエラーを飲み込まないか**: 崩せなかった。`errBusy` を包む生成点は
  `readLockInfo` の 2 つ (IsDir / Unmarshal 失敗) だけで、`io.ReadAll` の失敗は包まずに raw で返るので
  ちゃんと rc=1 に出る
- **seam を足した後の `tryPlace` の順序**: 崩せなかった。`own` は fd から・AfterCreate hook の前・
  Close の前に取れており、production では両 hook が no-op なので順序は不変
- **write-then-verify の分類漏れ** (素の errBusy で戻るので CLI が無音): **未確認**。到達には
  「自分が置いた直後に第三者が壊す」が要り、発火条件を構成できなかった

### 6 周目は不要 (§7 の打ち切り条件)

打ち切り条件の例外 (a)(b) を満たす: **(a) 5 周目の修正は新しい判定ロジックを 1 つも作っていない**
(文字列の削除 / 構造体タグと型の変更 / printf の出し分け。安全性の分岐は 1 つも増減していない)。
**(b) 各修正を直接の実測で確認した** (変異 3 本で red / `go test -race ./...` 緑)。
...
- [x] 5 周目 (§7) を通した。P1 / P2-1 / P2-2 を採用 (変異 3 本で red)、P3 2 件を記録。
      **6 周目は不要** (打ち切り条件 (a)(b) を満たす。上節に根拠)。4 周目が挙げた攻め口 4 点は
      全部攻めさせ、①②③は「壊せなかった」、④は論理の非対称 (P3-1) として記録した

## 引き継ぎ (2026-09-18 時点。完了)

**状態: 実装・テスト・敵対レビュー 5 周が完了。§7 の打ち切り条件を満たしたのでクローズする。**

- 直った: 中身を読めない lock は引き継がない (fail-closed) / 読めない理由が CLI の出口に出る /
  **どの出口も破壊的操作を促さない** (5 周目 P1。`status` も含めて 3 hop 全部を pin 済み) /
  `status` がその状態に答え、**取れなかった材料は「不明」として出す** (0 に丸めない) /
  `tryPlace` の後始末は自分の実体だけを消す
- テスト: `src/lockman/unreadable_lock_test.go` (**14 本**)。seam は `readLockAfterStatHook` /
  `tryPlaceLinkFn` / `tryPlaceBeforeIdentityHook` / `tryPlaceAfterCreateHook`
- **未検査として残っているもの** (再開 trigger つき):
  - `cleanupOwn` の異常系 3 枝は **coverage 0** (変異検証が原理的に無効。`ownErr != nil` を作る
    seam が要る)。案内の非対称 (5 周目 P3-1) もここ。trigger: 実際に詰まりが報告されたとき
  - `os.SameFile` は ino 再利用・ino==0 を返す FS では inert (**ローカルでは実演不能**。
    実 SMB 共有で `stat -f '%d %i'` を create/delete で繰り返すのが測定手順)
  - ディレクトリ lock の原因テキストが出口に出ない (5 周目 P3-2)。trigger: 誤診が実際に起きたとき
  - write-then-verify の分類漏れ (素の errBusy で無音)。発火条件を構成できなかった

🚨 **踏んだ罠**: 1〜5 周目まで**毎周 P1 が出た**。核 (fail-closed) は 1 周で閉じており、
2 周目以降は**全部「私が足した案内文と後始末」由来**。5 周目の P1 は
「3 周目に消した破壊的助言が、4 周目の案内先に 1 hop 下へ移って復活していた」形で、
**不変条件をコメントで宣言していたのに、破れる hop を pin していなかった**ことが原因。
機構を足すと、その機構に固有の failure mode が生まれて §7 が次の周を要求する。
**足す前に §0-A (作らずに済む構造はないか) を問い、不変条件を宣言したらその場で
「破れる hop」を数えて pin すること。**

🚨 **380 が入ったので前提が変わった**: 0 バイト lock の主要な生成経路 (`Renew` の `O_TRUNC` 窓) は
消えた。本 issue の機構は「外部要因で壊れたとき」の受け皿として残るが、**発火頻度は下がる**。
ただし `tryPlace` の O_EXCL fallback (本番 smbfs 専用の枝) は、取得が飛行中のあいだ
0 バイト lock を他者から見せるので、**この状態自体は正常系でも観測されうる** (5 周目 P1 の前提)。
