...
3 本を `writeAtomic` へ寄せ、`cache_write_discipline_test.go` でソース走査のゲートを足した。
B（例外として明文化）ではなく A を選んだ理由: 3 行の変更で失敗モードが消え、
`writeAtomic` は同じ package に既にあるので追加コストが無い。219 が明示した trigger
（`issues_state.go:71` を直す判断）もこれで閉じる。

🚨 **ゲートの 1 段目は実際の違反を 1 件も捕まえられなかった**。最初は
「同じ関数内に `cacheBaseDir` と `os.WriteFile` が両方ある」で判定したが、
`saveIssuesScreen` 等は `path, _ := issuesStatePath()` と**別関数からパスを受け取る**形なので
0 件になる。3 本とも素の `WriteFile` へ戻す変異が全部緑で通って気づいた。
**パス提供関数を 1 段辿る形**（①`cacheBaseDir` を呼ぶ関数を集める ②それを呼びかつ
`os.WriteFile` も呼ぶ関数を違反にする）へ直して、3 本とも検出されることを確認した。

## 推奨対応（起票時。どちらかを選ぶ判断が要る）
