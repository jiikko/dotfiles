...
- glogx の `n` の敵対レビュー (1 周目): P1 なし。採用 = 読んでから書くまでの並行編集で上書きしない (mtime/サイズの取り直し。窓は縮むだけで 0 ではない) /
  issue ファイル自体が symlink・ハードリンクなら書き換えない / 移動は済みバナーの後始末だけ失敗したとき、一覧が「動いた」として位置を追う。
  変異 5 本 red。記録のみ: 人が行頭に書いた `**着手中…` も解除で消える (CI の定義上バナー扱い) / YAML front matter の前にバナーが入る (該当 issue なし)
- 同 2 周目: P1 なし。「目印を外してからバナーを外す」順だと、後始末の失敗で Status が Next でなくなり、再度の n が skip されて
  古いバナーを UI から外せない (P3-2) / 「err でも Path があれば動いた」の約束が既存の経路で守られていない (P2) → **順序を
  「先にバナーを外し、外せたら目印を外す / 動かす」へ入れ替えて両方を構造で解消** (失敗 = 何も変わらない = 再実行できる)。
  1 周目で足した呼び出し側の分岐は不要になったので削除。変異 3 本 red。判定ロジックの新設が無く直接の実測で確認したため
  3 周目は回さない (adversarial §7 の例外)。残る既知の半端: 入る向きのバナー書き込み失敗 + 巻き戻し失敗の二重失敗
  (失敗時の再スキャンで一覧は正される) / 秒精度の FS では同サイズ・同秒の並行編集を見逃す
