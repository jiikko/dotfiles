...
案 A の実装・敵対的レビュー (opus) の対応・変異検証まで完了したので done へ送る。

- 実測し直した現状: 置き場の job **0 件** / socket ごとの dir **0 個**
  (`find ~/.local/state/tmux-schedule-keys`)。旧共有 dir にも残骸は無く、
  「移さないと決めた古い予約」も存在しない
- 残した P2-4 (使われない socket の dir の残骸) の理由と再開の trigger は、
  `scripts/tmux_schedule_keys.sh` の `STATE_ROOT` 直上のコメントに移した
  (issue は done へ動くので、現場に残す)
- 症状 1〜3 (一覧・取消・失効件数の混ざり) は入れ物の分離 + 前任サーバの失効で塞いだ。
  同じ socket にサーバが立ち直る経路も含めてテストで固定済み
