...
登録した**そのパス (と `.partial`)** だけで、ディレクトリを走査しない。よって母集合の限定・
symlink を辿らない・uid 確認・pid 生存判定・pid 再利用の窓が**最初から存在しない**
...
残る穴は **SIGKILL のみ**で、しかも構造的に狭い: `watch` は `watched -nt state` のときしか
ファイルを作らず (= dotfiles がずれている間だけ)、作られたものは**次の precmd で消費される**。
つまり「dotfiles がずれている最中に、起動から最初のプロンプトまでの間に SIGKILL される」場合に
限って、pid 名の小さなファイルが最大 2 本残る。この頻度は未実測 (件数を主張しない)。
...
テストは 8 assert / 0.8 秒。ベースライン green を 3 回連続で確認 (並行性を含むため)。
**変異 7 本を当て、7 本とも red**。ケース別の pass/fail 一覧で判定した (スイートの rc では読まない):
