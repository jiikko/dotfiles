...
| 日付 | episode | 内容 | 出典 |
|---|---:|---|---|
| 2026-09-04 | 1 | **worktree `~/wt-227228` 上で** 未コミットの修正を復元で消した (7 ファイルに及んだ) | [250](250-retro-fullscreen-registry-and-termsafe-gate-2026-09-04.md) |
| 2026-09-14 | 3 | ルールを読んでいて 3 回 | [374](374-retro-derived-fields-guard-cache-and-mutation-2026-09-14.md) |
| 2026-09-17 | 1 | 足したばかりのテストを 1 ケース目の checkout で消した | [389](389-retro-glogx-cursor-glide-and-shift-space-2026-09-17.md) |
...
([348](348-retro-issue-done-tool-and-truecolor-guard-2026-09-10.md))。**両者は同じ guard で止まらないので分けて数える。**
...
- [x] 🚨 **自作の安全機構なので敵対的レビューを最終ゲートにする** → 5 周 (P1: 3 → 5 → 2 → 2 → 1)。
      5 周目の修正は判定ロジックを新設せず各修正を変異で直接確認したので、§7 の例外で打ち切った (下の進捗)
...
### 敵対的レビュー 4 周目・5 周目 (2026-09-24、dotfiles-d4 が引き継いで実施)

**4 周目 (P1 2 件)** — どちらも「変異は想定どおり red」(rc=0) の偽物:

| 指摘 | 実態 | 対応 |
|---|---|---|
| P1-1 全部の検査を壊す変異でも rc=0 | 帰属は「--expect の行が 1 つ出たか」しか見ていない | **機械では判定しないと決めて宣言を狭めた** (rc=0 は「想定の検査が落ちた」まで)。材料として baseline との出力の差を必ず出す |
| P1-2 挙動を変えない変異でも rc=0 | `add -N` を変異の run の前だけで当て、ログも worktree に置いていたので、2 回の run で木が違った | `add -N` を baseline より前へ、一時ファイルを worktree の gitdir へ |
| P2 群 | HEAD の移動を見ない / 並行 run のプロセスを殺す (pid の接頭辞) / `mv` した --file で rc=8 / `:(icase)` が glob / 構文検査の推定が本物で落ちる / `go build` の例がテストを検査しない | snapshot に HEAD・index / pkill のパターンを escape + 境界 / `--no-renames` (原因は読み方でなく**変異で rename 検出そのものが外れる**こと) / `:(literal)` 優先 / 構文検査を本物に当てて落ちたら rc=3 / 例を差し替え |

**5 周目 (P1 1 件)** — 4 周目で足した「出力の差」の仕組みに攻撃が集中した。§8 に従い、仕組みを積まずに
**素の `diff` へ置き換えて単純化**した (消えた行も出す = fail-fast の runner で走らなくなった検査が見える。
`grep -vxFf` の行数の 2 乗の遅さと NUL で行が出ない件も同時に消えた)。ほかに `go vet` が正当な変異を
構文エラー扱い (`unreachable`) → 推奨を `go test -vet=off -count=1 -run '^$'` へ / 正規化の `ls-files` が
`-z` なし (`core.quotePath=true` で非 ASCII 名が壊れる) / 構文検査の副作用 (`__pycache__`) で 2 回の run の
木が違う → 事前検査を baseline の前へ / 同じ commit の別ブランチへの切り替え → `symbolic-ref` も撮る。

**変異検証**: 4・5 周目の修正ごとに「その修正を戻す」変異を 15 本、この道具自身で当てて全部 red。
途中で 3 回つまずいた (いずれも道具が検出した):
- HEAD の行を外す変異が緑 → 既存ケースの `checkout HEAD~1` は index も変えるので index の hash で捕まっていた。
  HEAD **だけ**が動く形 (空 commit) のケースを足した
- `add -N` を後ろへ戻す変異が緑 → ケース 25 の検証が FAIL を出しても最後に `exit 0` していた
  (テストが何も守っていなかった)。非 0 で終わるよう直した
- `add -N` を**消す**変異は別のケースで落ちた (rc=7) → 変異の当て方の誤り。「移す」形に当て直した

self-test は **25 → 35 ケース** (件数は見出しから数える形に直した)。

**打ち切りの根拠** (§7 の例外): 5 周目の修正は表示を `diff` にした・処理の順番・読み取りのフラグ・境界の
文字で、**判定ロジックを新設していない**。各修正は「戻す」変異で直接確かめた (5 本すべて狙ったケースで red)。

### 残タスク

- [x] **4 周目を回すかの判断** → 回した (上)。5 周目で打ち切り (§7 の例外。根拠は上)
- [x] **収束の傾向への対応** (3 周目時点の宿題: 「宣言 vs 実装の食い違い」が毎周出る) → §8 の「軸を変える」を
      採った。4 周目で**宣言を狭め** (巻き添えの red は機械で判定しない)、5 周目で**仕組みを単純化** (出力の差は
      素の `diff`)。P1 は 3 → 5 → 2 → 2 → 1
- 未確認のまま残すもの (スクリプト冒頭の「何を止めないか」に記載): 検証コマンド自身が作る生成物で 2 回目の木が
  変わる形 / argv にパスを持たない子プロセス / 改行・タブを含むパス / submodule の中身
