...
**tmux の中は `grapheme` 寄り、外 (Apple Terminal) は `wc` 寄り。** 揺れ観測 12 回は両方とも
「揺れなし」なので、端末単体の層は安定していて、差は tmux が幅を書き換えていることによる。

### 結論

この issue の (2) は「`ansi` と `wc` のどちらへ揃えるか」という設問だったが、**どちらへ揃えても
もう一方の環境でずれる**ので、設問自体が成立しない。`dispWidth` の実装は変更しない。
...
「`wc` と一致するなら 2027 の交渉結果を測る価値が出る」としていたが、**`wc` 一択にはならなかった**
ので前提が消えた。2027 対応端末で `got` が変わりうることは変わらないが、揃える先を選ばない以上、
測っても方針は変わらない。**2027 を測る動機が生じるのは「幅の割れる文字を出す」判断へ戻すとき**。

→ **この issue は (1)(2)(3) すべて決着したのでクローズする。**
