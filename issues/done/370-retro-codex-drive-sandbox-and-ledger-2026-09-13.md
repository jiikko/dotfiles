...
決着 2026-09-13 (ユーザー判断で done を承認):

- [x] 反省 1 (`worktree-per-session.md` への追記) — **切り出さない**。逸脱は本文に記録済みで、次に同じ形を踏んだら追記を再検討する
- [x] 反省 5 (敵対レビュー) — **obaket の実測で代替**。issue 369 の受け入れ条件「危険側の観測が 1 件でも出たら 0-a へ戻す」がその出口
