...
すべて対応済み (2026-09-06)。索引としての役目は終わったので done へ移す。

- [x] 279（hint の生成契約を型で閉じる）+ 281（ゲートをレジストリ + sweep へ）— `ace4ee2d` / `890bf886`
- [x] 280（フレーム確保予算に下界）— `a80aecc7`
- [x] 282 / 283 / 284（ゲートが素通しする形の是正）— `a2416246` / `a8ff94b1`
- [x] 285（`hasTerminalControl` を `termsafe/ctlprobe` へ 1 本化）— `a1047540`
- [x] 286（`writeAtomic` 規約 + ソース走査ゲート）— `6c3e1a6c`
- [x] 287（`isDigitKey` の境界）— `a80aecc7`
- [x] `issues/done/201` の done 本文へ「実装は列挙表になっている」を追記 → 281 の本文へ記録
      （201 は既に done なので、乖離の記録は 281 側に置いた）
- [x] `fitHintItems` の doc の乖離（「今の利用者は status viewer だけ」）— 279 の commit で訂正

## 🚨 実装後の敵対的レビューで出た P1（起票時には見えていなかったもの）

`890bf886` で反映済み。**索引の記述として重要なので残す**:

- **hint の固定文字列は 3 本でなく 4 本**だった。job パネルの**カーソル無し**分岐
  （`openPanel` が `panelCursor = -1` から始めるので Enter 直後に必ず通る既定状態）が
  63 桁のまま。既存テストは `panelCursor = 0` を立てるので別分岐しか見ていなかった
- **279 の症状は 1 回目の修正では解決していなかった**。`fitHintItems` が幅に収めた**後**で
  前置（取得中 / gh の警告）を積み、`hintLineText` が末尾 = 出口を落としていた。
  `m.ghErr` は次の取得開始までクリアされないので、gh 未導入 / 未認証 / 非 GitHub repo では
  **常時**その状態で、幅 60〜200 の全帯で出口が消えていた
- **世代化（268）が新しい失敗モードを作っていた**。`&issuesView{rows: list}` の複合リテラルは
  世代が 0 のままなので恒久 stale。**旧実装は全件比較のおかげで偶然自己回復していた**
  （`list-masked-failure-modes-before-removing-guard.md` の形）
- **`ratelimit-dash` の確保予算が壁時計依存**で ~4%/run flake した（メモ化の鍵に `now.Unix()` が
  入るのに fixture が `timeNow` を固定していなかった）
- **`shortestPrio1` の「最短」が 1 本もテストされていなかった**（最長へ変えても全テスト緑）

## 通しての一般化（21 issue の実装を経て）

`issues/done/273` の 5 項目に加えて:

- **自作ゲートは「実際の違反を検出できるか」を必ず植えて確かめる**。286 のゲートは 1 段目で
  実際の違反 3 件を**1 件も検出できなかった**（判定を「同じ関数内」と近似したが、実物は
  別関数からパスを受け取る形だった）
- **canary は本走査と同じ関数を通す**。268 のゲートの初版 canary は `setRows` 自身が必ず
  当たるので、判定が壊れても発火しなかった
- **変異の向きを間違えない**。285 で「オラクルを狭める」変異を当てたが、それは検査が緩む
  方向なのでどこも落ちないのが正しく、それを「波及していない」と読みかけた
- **予算の下限は「退行後の値では落ちる」位置に置く**。今の値から係数を掛けると、
  退行そのものを予算に焼き込む（269 の初版がその形）
