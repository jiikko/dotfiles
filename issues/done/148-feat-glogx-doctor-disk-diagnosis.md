...
**段階 ①②③④ をすべて実装して稼働中。この issue は done。**
以降の章 (1〜6) は**設計時の記録**として残す (当時の判断理由が要るときに読む)。
実装の正本はコードで、使い方の正本は `src/doctor/README.md` と `src/glogx/README.md`。

| 段階 | 内容 | 決着 |
|---|---|---|
| ① | サービス診断 (壊れた launchd 登録) | `src/doctor/svc` + `bin/svcdoctor` |
| ② | ディスク走査 (カタログ + ガード + du 相当) | `src/doctor/disk` + `bin/diskdoctor` |
| ③ | glogx の doctor 画面 (`D`) | `src/glogx/doctor_view.go` ほか |
| ④ | 削除 | engine = `src/doctor/disk/delete.go` / UI = `src/glogx/doctor_delete.go` |

### いまの配置

```
src/doctor/                    ← 1 module (当初は src/svcdoctor と src/diskdoctor の 2 module だった)
  disk/       catalog.go paths.go size.go guard.go scan.go report.go run.go delete.go doc.go
              ↑ delete.go が**唯一**破壊的操作を持つ (不変条件はファイル冒頭が正本)
  svc/        plist.go launchctl.go brew.go scan.go report.go restore.go doc.go  ← 削除経路を持たない
  ...
  cmd/diskdoctor/  cmd/svcdoctor/    ← CLI 本体。**どちらも走査だけで削除しない**
src/glogx/
  doctor_view.go       ← D の画面 (セクション・行・キー)
  doctor_delete.go     ← ④ の導線 (選択 → 確認 → 実行中 → 結果)
  doctor_rowcursor.go  ← 行の上のカーソルと窓 (行の列しか知らない閉じた機械)
  doctor_cache.go      ← snapshot / 再利用 / 起動トースト
  doctor_cleanup.go    ← 走査と削除を看取る latch (issue 211)
  doctor_brew.go       ← brew doctor の転記
```

### ④ (削除) の要点

**破壊的なコードは `disk/delete.go` の 1 本に閉じている。** UI は「どれを選んだか」を渡すだけ。

- 削除の作法はカタログが決める: `rm` / **ゴミ箱へ移動** (`risk: confirm` は必ずこれ) /
  `cli:<コマンド>` (SIP 配下など rm できないもの) / `propose` (表示だけ)
- **削除の直前に走査し直し、その結果だけを対象にする**。渡された Items は「どれを選んだか」の
  指示としてだけ使い、サイズと `(dev, ino)` は測り直した値を使う
- **解放量は再走査の引き算と「実際に触った Item の合計」の小さい方**。消えていなければ
  「未完了」を成功にも失敗にも畳まずに出す
- **記録を残せないなら削除しない**。`cachedir.Base()/doctor-history/` に planned →
  executing → done / aborted の相つきで残る
- **sudo は実行しない**
- UI: `Space` で選択 (`Enter` で開くと**ディレクトリ単位**でも選べる) → `d` で下見 →
  確認プロンプト → `y`。実行中はキーを飲み、Ctrl-C 2 回で ctx を cancel する

### 検証の規模 (④)

| | S1 (engine) | S2 (UI) | 通しレビュー |
|---|---|---|---|
| 敵対的レビュー (opus) | 5 体 (2 周) | 2 体 | 1 体 + issue 反証 1 体 |
| 採用した P1 | 4 | 1 | 4 (P2 相当) |
| 変異 (すべて red) | 33 | 31 | 13 |
...
- **8 章の未決事項**と 7 章の未チェック 2 件は **issue 218 へ切り出した** (どれも
  ユーザー判断か実測の機会待ちで、実装の残りではない)
- doctor まわりで open な issue: 206 (未計測項目) / 207 (反証レビューの運用) /
  216 (テストが実走査を漏らす) / 217 (pullCleanup の取り残し) / 169 (pending)
...
| **done (追加)** | 183 (`Y` のコピー文の裏取りコマンド) も done |

163 自身も **done** (体 3 = 並行・中断・状態機械 は 205 として走らせ、その未達を
206 / 207 に切り出して閉じた)。
...
- [~] (段階 1 では未実装) `sfltool dumpbtm` のパース失敗が **BTM 行を出さないだけ**で済み、
      診断全体を落とさない → **BTM 自体を実装していないので条件が成立しない**。issue 218 へ
...
- [~] **glogx の起動時間が悪化しない** — 起動パスで走査しないことは**テストが固定**しているが、
      **時間そのものは未実測**。issue 218 へ (④ は起動パスを触っていない)
