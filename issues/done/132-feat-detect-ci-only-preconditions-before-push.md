...
> 🚨 **前提の変更 (2026-08-28)**: Linux をサポート対象外にし、CI を macOS runner へ寄せることが
...
| 3' | repo 内に `issues/next/` が存在する | hook の opt-in ガードが全件 no-op になり、テスト 9 件が「発火すべきなのに無出力」 | 個別対応済 (テストを偽 repo で回す形へ)。**仕組みは 3 と同じ (本 issue の残り)** |
...
🚨 **3' は ignore とは無関係に同じ形が出た例** (2026-09-02、retro 195 項目 3)。
`issues/next/` は **空ディレクトリなので git がディレクトリごと載せない**。`next-claim-push` hook は
「`issues/next/` が実在する repo でだけ有効」という opt-in ガードを持つため、CI と新品チェックアウトでは
判定式が壊れていなくても**全件が no-op** になり、テスト 9 件が落ちた。手元では過去の作業で残った
`issues/next/` があり再現しなかった。**「gitignore されているか」ではなく「git に載っているか」で
考えないと同型を取りこぼす** (空ディレクトリ / ignore / untracked のどれでも起こる)。
なお現在は `issues/next/.keep` が置かれて追跡対象になったので、この個別ケースは解消している。4 は**より危険**で、依存が無いと recipe が黙って skip
...
- **手元専用の道具として Docker で CI parity を作る**: docker 依存が増え、1 回の実行が重い。
  ...
  🚨 CI 自体をコンテナ化する案は別物 (上の Phase 2 候補 B) で、こちらは 133 の決定により
