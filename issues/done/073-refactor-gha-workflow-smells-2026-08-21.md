...
## 対応 (2026-08-21, commit a3fa623)

medium 2 件を実装。敵対的レビュー (自作の安全機構なので自己レビューで閉じない規律) で
8 件の素通り経路が出たため、すべて実験で裏取りして塞いだ。

- actionlint を `ACTIONLINT_VERSION: 1.7.12` で pin し、gh api のリトライ 12 行と GH_TOKEN
  依存を削除。取得後は `--version` を pin と突き合わせる (別版の取得と PATH 解決の失敗を
  同時に捕まえる)
- CI の apt パッケージを `Makefile` の `CI_PACKAGES_HEAVY` / `_REST` 出典にし、workflow は
  sed で読む (make 自体が apt の対象)。値は env 経由で受け、許可文字だけを通す
- 乖離を落とすゲート `scripts/check_ci_group_deps.sh` + `make test-ci-group-deps` を新設し
  `make test-lint` に組込。sed 契約の照合 (make の値と sed の結果) / heavy に .bats を
  置かせない / heavy のテストが rest 専用コマンドを呼んでいないかを検査する

**low 2 件 (GIT_CONFIG_* の 5 ファイル重複 / bench.yml の apt-get update 3 回) は未着手**
(trigger 待ちのまま)。

## 残った未対応 (敵対的レビューが指摘した非対称)

- `lint.yml:38` の apt パッケージ (zsh / yamllint / jq / ruby) は依然ハードコードで、
  ゲートが無い。ここから zsh / ruby / yamllint が落ちると `test-zsh-syntax` /
  `test-ruby-syntax` / `test-actionlint` がすべて黙って skip して緑になる。tests.yml で
  塞いだ穴が lint.yml には残っている。**trigger: lint.yml のパッケージを次に触るとき**
- `make test-actionlint` / `test-bats` / `test-shellcheck` は該当コマンド不在で skip して
  緑を返す。CI では pin バイナリが入るので守られるが、「検査できなかった」と「問題なし」を
  区別しない形は残っている
