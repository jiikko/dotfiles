...
| 未達だったもの | 切り出し先 |
|---|---|
| 体 3 (並行・中断・状態機械) が未走行 | [205](205-audit-doctor-red-team-concurrency-state-machine.md) — 攻め口 6 つを移設。⚠️ 180/182 で選べる行と幅の扱いが変わったので、当時の攻め口はそのままでは成立しない旨も書いた |
...
- [x] 6 体の報告が、トピックごとの issue と、この issue の索引に分かれて残っている (体 4 は数字つき)
      — **5 体分。体 3 は 205 へ**
- [x] 起票した issue はそれぞれ反証レビューを通している (P1 は必須) — **P1 2 件は完了。
      残りは着手時に実コードで再現を確かめる形にした (経緯と残りは 207)**
