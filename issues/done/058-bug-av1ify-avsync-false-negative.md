...
方針 1 + 3 + 4 で解決 (方針 2 の双方向化は不採用 — 全件で packet 走査 4 本のコストが常時
掛かる上、宣言ベースの初回判定を素通りするズレは 997d078 も検知しておらず、デグレの範囲は
「降格の正しさ」だけ。この判断は再判定ブロックのコメントに残した):

- `__av1ify_start_time` を新設し、FLAG→ok の降格条件を「packet 実測の終端が揃っている」
  **かつ**「音声開始の関係差 (calc_drift と同じ式を start_time に適用) も閾値内」の 2 条件に
  変更。開始の計算に失敗したら降格しない (検査できなかったときに ok へ倒さない)。
  start_time 欠落は 0 とみなす (mp4 出力のシフトは edit list として必ず書かれるため、
  出力側で取りこぼさない。src 側の欠落 = 0 はコンテナの慣行どおり)
- mock ffprobe に `stream=start_time` の口を追加 (`MOCK_*_START` 系)。これが無いと
  このクラスの回帰テストが書けなかった (issue の「既存テストが守れない理由」)
- Test 18 を新設: issue の実測値 (out 音声 start 4.976 / 終端 19.999) をそのまま使い、
  avsync が FLAG され「時間シフト型」の理由が出ることを assert

検証:

- 実ファイル再現 (issue の -itsoffset ハーネス) で HEAD が `avsync` サフィックス付き NG に
  復帰することを実測 (「音声開始のシフト Δ=4.976009s が閾値超過のため音ズレと判定」)
- 変異検証: 開始シフト判定を潰すと Test 18 だけが red になり、Test 14 (壊れた宣言 duration の
  誤検知是正) は green のまま = 両方向の不変条件を分離して守れている
- avsync テスト 18 本 + av1ify テスト一式 green

未確認のうち「60s 超のシフト」「負方向のシフト」は未対応のまま: 前者は packet_end の走査
区間の話で本件の開始オフセット判定には影響しない (start_time は宣言値で走査に依らない)。
後者は calc_drift が符号なし絶対値で比較するため方向に依らず同じ式で検出される (式上の確認
のみ。実ファイルでは未検証)。
