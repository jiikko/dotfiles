...
## 訂正 (2026-09-18): 常時付与をやめ、検出したソースだけに限定

ユーザーから「従来 OK だったファイルに影響が無いか」と問われて実ファイルで測り直したところ、
当初の実装にデグレが 2 つあった。

1. **avg_frame_rate=0/0 のソースを VFR と誤判定** → `-r 0.000` が渡り ffmpeg が
   `Invalid framerate value` (rc=234) で失敗。Downloads の元ソース 91 本中 1 本 (WMV) が該当
   → `__av1ify_detect_vfr` で 0 以下を「不明」として判定対象外にした
2. **「CFR ソースに cfr は no-op」は誤り**。91 本の先頭 30 秒で 36 本の出力フレーム数が変わった
   (大半は ±1〜3 の末尾端数だが、最大 dup=146 / drop=23)。それらは VFR 判定に掛からないので
   フレーム数チェックが効いたまま残り、従来 OK が check_ng に転ぶ恐れがあった
   → `-fps_mode cfr` は **VFR 検出かつ --fps 未指定** のときだけ付与。それ以外の引数は改修前と同一

あわせて `_validate_mp4.zsh` へ足した "non monotonically increasing dts" も取り下げた
(従来 OK の出力や validate-mp4 単体利用で新たに NG が出るため。理由はコード側の注記)。

- 変異検証: cfr 常時付与への戻し / VFR 時の cfr 除去 / 0/0 ガード除去 / validate への dts 再追加 の
  4 本すべて red を確認
