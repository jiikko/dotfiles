...
- **内容**: `crash-analyzer`, `smoke-test-runner`, `tt-api-expert`, `xcodebuild-runner` はトリガー未登録。名前を知らないと利用できない（2026-07-17 訂正: `statusline-setup` は Claude Code 標準のハーネス組み込みエージェントでトリガー登録自体が不要のため対象から除外）
- **推奨**: 残り 4 件は全て TT 固有なので項目 13（プロジェクトローカル化）とセットで扱う。カタログは項目 21 参照

### ~~9. `_common/output-format-template.md` と `quality-checklist.md` が活用されていない~~ 対応済み 2026-07-17（両ファイルを削除）
- 2026-07-16 の「2 エージェントが参照」は事実誤認だった: その 2 エージェント（architecture-reviewer / swift-language-expert）が参照しているのは `language-adaptation.md` / `tool-selection-strategy.md` であり、当該 2 ファイルへの参照は **0 件**。完全未使用のため削除で解消
...
### ~~11. シンボリックリンク依存で断絶リスクがある~~ 対応済み 2026-07-17（`tests/claude/test_dangling_symlinks.sh` を新設し make test で常時検出。$HOME 直下 + ~/.claude 配下の dotfiles 由来リンクを走査。導入時点で実際に残骸 2 件（~/.ideavimrc, ~/.gvimrc）を検出・掃除済み）
...
### 17. Language Adaptation の参照方法が統一されていない — 要再調査（2026-07-16 確認: `@../_common/language-adaptation.md` 参照は 2 エージェントのみ。他エージェントは settings.json の `language: 日本語` で足りている可能性があり、参照追加より「_common ファイル自体の要否」を先に判断すべき。項目 9 の未使用 2 ファイルは削除済み (2026-07-17) だが、こちらは実参照 2 件があるため要否判断が別途必要）
...
残っていた 9 項目を実態と突き合わせた。**対応 1 / 決着 3 / trigger 待ち 5。**

### 対応した: 項目 21 (agents の索引が無い)

`_claude/agents/README.md` を新設した (31 件)。言語・フレームワーク別 / 設計・レビュー /
実行・調査 / macOS 周辺 / **ThumbnailThumb 専用** / ハーネス組み込み、の 6 群。
`_claude/CLAUDE.md` の「スキルファイル参照」節から辿れるようにした
(あの表は skill が主役で agent は一部しか載っておらず、**agent の入口が無かった**)。

🚨 **索引がずれると無いより悪い** (載っていない agent は「存在しない」と読まれる) ので、
`tests/claude/test_agents_index.sh` で両方向の乖離を検出する。変異 4 本で red を確認:

| 変異 | 結果 |
|---|---|
| 索引から 1 件消す (登録漏れ) | red |
| 実体の無い名前を索引に足す (削除残り) | red |
| 見出しの件数だけずらす | red |
| 表の書式を変えて抽出を空にする | red (**測れていないのに緑にしない**) |

### 決着: 項目 17 (Language Adaptation の参照方法)

**現状維持で決着。** 起票時の前提「`@../_common/language-adaptation.md` 参照が 2 エージェント」は
**もう成立していない** — 実測 2026-09-04 で `@../_common/` 形式の参照は **0 件**。
2026-09-02 に「この形式は展開されずリテラルのまま届く」と実測され、参照が外された後だった。

`_common/` の 3 ファイルの現状:

| ファイル | 参照元 | 判断 |
|---|---|---|
| `task-detached-deadlock-pattern.md` | agent 4 件が「Read すること」で明示参照 | **生きている** (この形式は機能する) |
| `language-adaptation.md` | agent / skill からの参照 **0 件** | **残す**。下記 |
| `tool-selection-strategy.md` | 同 **0 件** | **残す**。下記 |

参照 0 件でも消さないのは、**両ファイルの現在の価値が「`@` 参照は展開されない」という
実測の記録そのもの**だから。消すと、次に共通文書を作る人が同じ罠を踏む
(項目 9 で消した 2 ファイルは注記を持たない純粋な未使用だったので、状況が違う)。
その警告は新設した `agents/README.md` の「保守」節にも書いた。

⚠️ `language-adaptation.md` の「ユーザーの言語を検出し、日本語でなければ英語」は
`settings.json` の `"language": "日本語"` で固定されている現状とはずれている
(結果は同じなので実害なし)。

### 決着: 項目 8 / 13 (TT 固有の agent と skill がグローバルに在る)

実測: `crash-analyzer` / `smoke-test-runner` / `tt-api-expert` / `xcodebuild-runner` の 4 件は
実在し、**CLAUDE.md のトリガー表には 1 件も載っていない**。`perf-analysis` skill も
グローバルに残っている (description に「ThumbnailThumb 専用」を明記して緩和済み)。

**この repo からは動かせない** (移動先は ThumbnailThumb の `.claude/`)。
新設した索引で **「🚨 ThumbnailThumb 専用 (他プロジェクトでは使えない)」の節に分けて明示**し、
移動が本来の置き場所であることも書いた。名前を知らないと呼べない問題 (項目 8 の本体) は
索引で解消したので、**残るのは移動だけ**。trigger は「TT 側を触るとき」。

### trigger 待ち (5 項目。理由は起票時のまま有効)

| 項目 | 状態 | trigger |
|---|---|---|
| 10 audit の直接実行フローが薄い | 緩和済み (自動フォールバックは在る) | 直接実行で調査指針の薄さが実害になったとき |
| 14 forge が audit の内部パスを直接参照 | 緩和済み (フォールバック手順あり) | forge の内部構造を触る改修が来たとき |
| 15 出力 JSON スキーマの非互換 | 現存 (理論上の懸念) | 統合で実害が観測されたとき |
| 18 audit の 2 段階質問 | 構造が変わったが 2 段階のまま | 実運用で冗長と感じたとき |
| 22 forge examples が Electron / SwiftUI のみ | ハブ化で構造は改善、サンプルは未追加 | Go / Rails / Node.js で forge を使って例が要ると感じたとき |

いずれも**先回りで直さない** ([`verify-design-intent-before-refactor.md`](../../_claude/rules/verify-design-intent-before-refactor.md)
の「実変更 trigger 待ち」)。この表があるので、trigger が来たときに再発見できる。

## この issue を閉じる理由

残件は「trigger 待ち 5 件」と「TT 側でしか動かせない移動 1 件」だけで、**この repo で
今できることは無くなった**。2026-02-19 起票の監査ドキュメントを開いたまま持つより、
trigger を上の表に残して閉じる方が見つけやすい。
