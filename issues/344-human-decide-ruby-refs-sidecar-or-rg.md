...
### 「rg のパターンを絞る」は何に効くか — before/after の実測

🚨 **構文で絞る案（`\.load\b`）はほぼ無意味だった。** 最初にこれを提案したが、数えたら外れていた。

| 手段 | 総数 | 本命 | ノイズ率 |
|---|---|---|---|
| 現行（`-w load`） | 243 行 | 20 | **91%** |
| `\.load\b`（レシーバ付きだけ） | 161 行 | 19 | **88%** |
| **定義元クラス名を含むファイルに絞る** | **20 行** | 20 | **0%** |

理由は内訳にある。243 行の構成は **レシーバ付き `.load` が 161 行**、`def load` 19、コメント 28、文字列 5。
つまり `\.load\b` が落とすのは `def load` とコメントと bare `load(...)` で、
**最大のノイズ源である `@form.load`（67 行、すべて `test/models/loaders/checkout_loader_test.rb`）は
レシーバ付きなのでそのまま残る**。91% → 88% にしかならない。

`load` を定義しているクラスは **19 個**（うち 11 個が `Loaders::*Loader#load` の兄弟）。
`.load` のレシーバ表記も 23 種（`@form` 67 / `@loader` 28 / `loader` 11 / …）で、
**`@loader` ですら 6 つの別 Loader のテストで使われている**。構文で分けられる問題ではない。

#### 効いた絞り方: 定義元クラス名を含むファイルに限定する

`rg -l 'StockEventLoader'` で 4 ファイルに絞ってから `load` を引くと **20 行**、うち 20 行が本命
（`@loader.load` 13 / `loader.load` 5 / `posted_stock_event.rb` の 1 / 定義 1）。
3 ファイルすべてが `Loaders::StockEventLoader.new(...)` を書いているので、絞り込みが効く。

汎化の確認として、継承されたメソッド `wrap_error`（`BaseLoader` で定義、9 サブクラスが継承）でも試した:
24 行 → 10 行。落ちた 2 ファイルは `app/controllers/concerns/json_io/api3/form.rb:54` と
`test/support/api3_error_inspector.rb:2` で、**どちらも同名の別定義**なので落として正しい
（サブクラスは `class XLoader < Loaders::BaseLoader` と書くためクラス名を含み、生き残る）。

🚨 **想定される失敗モード（未実測）**: 呼び出し側がクラス名を一度も書かない場合
（DI / factory / `const_get` / polymorphic なコレクション経由）は**取りこぼす**。
この repo の `load` では `posted_stock_event.rb` がクラス名を書いていたので当たったが、
一般に recall を落とす手であることは変わらない。採用するなら
「絞った結果が 0 件なら自動で絞らない版へ落とす」等の逃げ道が要る。

### 判定基準の表への当てはめ

表の最終行は「sidecar より **rg のパターンを絞る**方が安い」と言っているが、
**絞り方の中身までは決めていなかった**。上の実測で、効くのは構文（`\.load\b`）ではなく
**スコープ（定義元クラス名を含むファイル）**だと分かった。
なお `-w` は「部分一致にすると別メソッドを大量に拾う」という既存判断で選ばれているので、
そこを触るなら `references_action` の純関数テスト + 変異検証が付く。**実装は未着手**。
...
- [ ] 「定義元クラス名を含むファイルに絞る」を `<C-k>` に入れるか、ノイズを受容するかの判断（ユーザー待ち）
- [ ] 入れる場合、絞り込みが 0 件になったときのフォールバックをどうするか
