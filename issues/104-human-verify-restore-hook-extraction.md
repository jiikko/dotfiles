...
## 隔離サーバで代替できないか（2026-09-04 に調査した。答え: **できない**）

「`-L` の隔離テストサーバで復元を回せば本番ログを汚さずに機械で確認できるのでは」と考えたが、
**構造的に成立しない**。

`scripts/tmux_log_restore_hook.sh` は **default socket でだけ書く**ゲートを持っている
（`tests/tmux/test_log_restore_hook.sh` のケース 3「非 default socket では 1 行も書かない」が
それを pin しており、**それが issue 079 の抽出の目的そのもの**）。隔離サーバで復元を回しても
意図的に 1 行も書かないので、「hook が実際に走ってログが出る」ことは観測できない。

つまり確認は **default socket = 本番サーバで復元を 1 周させる**しかない。この issue は
人に回すのが正しい（`_claude/rules/tmux-probe-requires-socket-isolation.md` の
「human に回す前に隔離サーバで測れないか一度問う」に対する答えを、ここに残しておく）。
