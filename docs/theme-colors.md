...
エンベロープ (~0.25 秒)** で遷移する (例: 残光 hot 201 → 127→90 (紫が沈む) → 52→130→166 (点火)
→ 202 / 消灯 16 → 52→130→166 → 202)。既に暗い起点 (消灯・残光の尾) は沈む工程を自動スキップ。
v1 の「前の色→現在地色の直行色相スイープ」は全フレーム高輝度のまま色相だけ変わり
「分かりにくい」実感だったため V 字へ (人の注意は色相差より輝度差に反応する。2026-07-16)。
...
- 選択タブの現在地色は ローズ→ショッキングピンク→Coral→蛍光オレンジ 202 と変遷 (いずれも 2026-07-16)。
  戻す/変えるときは `accent.current_accent` と tmux `@cur-accent` の対で (経緯: issues/017-feat-claude-code-orange-theme-2026-07-16.md)
