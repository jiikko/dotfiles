...
| [`glogx-ui-guide.md`](glogx-ui-guide.md) | glogx 全画面 (と pro-con) に共通する操作感とキー語彙 (vim 層 / emacs 別名層 / 動作層、開閉・破壊的操作・案内の規律、`J`/`K` 項目送り、入力欄の編集キー、pro-con の例外) | 新しいキーを足す / 画面を足す前に読む。個別キーの一覧は `src/glogx/README.md` |
