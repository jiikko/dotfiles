...
本文 pager で `u` を押すと、本文に現れる http(s) URL を**出現順に 1 つずつ**ブラウザで開く
(押すたびに次へ、末尾で先頭に戻る)。通知行が「何番目/全体」と URL を出す。

番号を振って選ばせないのは、本文 pager に行カーソルが無く、番号の対応を出せる場所が通知行
1 行しかないため (実測: URL を持つ 6 issue で 1 件あたり平均 5 本あり幅に収まらない)。順送りなら
状態は位置 1 つで済む。抽出は整形前の生ソースから行う — 折り返しで URL が行をまたいでも割れない。
