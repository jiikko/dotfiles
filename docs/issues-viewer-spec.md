...
| 開いている間の**別プロセスの編集** | 1 秒周期の mtime ポーリングで変化を検出したら同じ経路で読み直す |

ポーリングは viewer を開いている間だけ回し、閉じたら止める。ディレクトリの mtime (追加・削除・
`done/` への移動) とファイルの mtime + サイズ (本文・front matter の書き換え) の両方を見る:
ディレクトリの mtime は本文の書き換えでは動かない。変化を検出した次の周期でも同じなら読む
(書きかけのファイルを掴まない)。実装は `src/glogx/issues_watch.go`。
