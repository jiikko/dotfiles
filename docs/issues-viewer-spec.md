...
issue を選ぶと本文が**左からにゅっと開いて画面の 8 割**を占め、`h`/`q` で閉じるときは同じ動きを
**逆再生**する (Notion の peek のような見え方。ユーザー要望 2026-07-31)。全画面で置き換えないのは、
「どの一覧のどこから開いたか」が画面から消えるため — 右側に一覧が残れば開閉が位置関係として見える。
...
- 本文は**最終幅で整形して切るだけ**にする。途中幅で整形し直すと毎フレーム折り返しが変わって
  文字が踊り、幅ごとにキャッシュを持つ `Body` が毎フレーム再整形される
- 境界は `▏`。`│` は本文 pager のスクロールバーと隣り合って `││` に見える (実測)
