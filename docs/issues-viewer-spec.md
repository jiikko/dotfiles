...
issue を選ぶと本文が**画面の右外から飛び出して左へ滑り込み**、画面の 8 割 + 5 桁を占めて止まる
(比率 `issuesDrawerRatio` + 固定の上乗せ `issuesDrawerExtra`。上乗せは一覧の最小幅 `issuesDrawerMinList`
を確保できる余裕があるときだけ効く — 下限が効いて上乗せ前より狭くなるのは本末転倒なので、
比率ぶんより狭くはしない)。
