...
からの経過を `@fade-step-secs`（現 3 秒）刻みの「段（bucket）」に落とし、明るい紫から 5 段かけて
...
| 段 (bucket) | 条件（step=3 秒時） | 表示 (bg × fg) |
|---|---|---|
| 実行中 | 前面でプロセスが動いている pane を持つ (@busy) | **最明 colour201 × 黒。終わるまで常時** |
| 0 | 〜3 秒 | colour201（明るい紫）× 黒 |
| 1 | 3〜6 秒 | colour164 × 黒 |
| 2 | 6〜9 秒 | colour127 × 明灰 colour252（紫は緑成分ゼロで輝度が低く、黒字はここから読めない） |
| 3 | 9〜12 秒 | colour90 × 明灰 |
| 4 | 12〜15 秒 | colour53（闇へ）× 明灰 |
| 5（消灯）| 15 秒以上 or 実行履歴なし | 背景なし（バー地に溶ける）× colour240 |

> step=3 秒なので 15 秒（5 段）かけて消灯する。刻みを変えるなら定数 `@fade-step-secs` だけを触る
...
- スタンプの throttle は 3 秒（プロンプト毎に tmux client を fork しない方針。連続作業中でも
  3 秒に 1 回の fork に上限が付くので CPU は軽微）。⚠️ throttle は `@fade-step-secs`（3 秒）以下で
