...
🚨 当初この節に「v2 のエンジンは `clipperhouse/displaywidth` で測る」と書いたが**誤り**（セルフレビューで訂正）。`displaywidth` は `x/ansi` v0.11 以降が内部で使うライブラリで、独立した実装ではない。実際に変わったのは**ライブラリではなく幅モデル (`ansi.Method`)**:
