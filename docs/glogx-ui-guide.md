...
見ているだけの画面 (`pro-con --view`) では書き込みの操作 (`n` `i` `r` `d` `+` `w` `x`) をすべて暗くし、押した時点で断る
(入力欄を開いてから断ると書いた文が無駄になる)。模擬と本物のモードはどれも受ける (backend.Accepter)。
...
- `o` (ブラウザで開く) と `b` (半ページ上) は**空けてある**。追加オーダーは `+`、btw は `w` (what's up)。`?` はレーンの意味の表 (2026-09-24 に btw から移した)
