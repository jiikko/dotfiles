...
| 予約入力 (N 時間 M 分後に send-keys) | `prefix + m` / `prefix + Enter` / `prefix + C-m`（ウィザード / 予約一覧・取消） | display-popup + gum input/choose/confirm / run-shell -b で sleep / send-keys -l / display -p で自 pane 固定 | `scripts/tmux_schedule_keys.sh` |
