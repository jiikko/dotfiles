...
1. **軽量化を急ぐ**: 重い Vimscript プラグインは順次 Lua 版へ（候補例: vim-toggle）。必要性や労力を見極めつつ進める。
...
- **kana/vim-operator-user の整理**: Vimscript 依存が多く遅延ロードも効きづらい。mini 系モジュールや Lua 実装へ段階的に移行したい。
## 推奨プラグイン候補（リサーチ結果）
