...
#
# 🚨 検査できなかったときに緑を返さないこと。依存コマンドの不在・読めないファイル・grep の
...
# ---- 依存コマンドの健全性 -------------------------------------------------------------
# 🚨 固定パターンだけで試さないこと。パターン依存の失敗 (メタ文字で exit 2) は
# 下の grep_rc で個別に見る。
...
for d in $dirs; do
  ...
  for f in "${files[@]}"; do
    ...
    for cmd in $only_rest; do
      ...
      # 🚨 heredoc 内の散文は静的には除外できない (実測で偽陽性になる)。逃げ道として
