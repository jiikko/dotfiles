...
#
# 🚨 名前は tests_ だが **scripts/ と bin/ も見る**。`scripts/discover_shell_scripts.sh` の
...
#   🚨 「zsh のテストを bash で呼ぶな」を検査する形にはしていない。呼び方は人と runner の側の
...
#   🚨 最初は「`||` / `&&` / `;` を含む行は rc を見ている」としていたが、**`cd "$X" || true` /
...
#
# 🚨 **行ベースの検査**なので、heredoc の本文やサブシェル `( ... )` の中の行頭 `cd` も対象になる
...
#
# 🚨 **この検査が構造的に見落とすもの** (予算を置くなら見落としを先に書く):
...
# 🚨 system grep を使う。ugrep 経由の grep は `$` を行末アンカーと解釈して 0 件を返す
...
#      🚨 ブロックの中に exit / return が**無い**形 (`|| { echo warn; }`) は許さない
